...
>
> **护型选择（§6.3.2）：** 只做 甲/乙(ZA>0)/己；乙(ZA<0)/丙 持仓等待方向；戊(ZA>0) 观察；丁/戊(ZA<0) 放弃。
>
> **关键增强信号（§6.3.3 + §6.3.12.1）：** DJA丘等1（31.4%）+ BSHA5（39.4%）+ **升连≥2%（92.75%）**——升连必须≥2%，否则无优势（首升连无限制68.84%≈基线，涨幅<2% 52.07%反而更差）。
>
> **阅读指引：** 第一~五章是各维度分析（护型/等型/正交动力学/三态理论），第六章是决策收敛（§6.3 定型操作 + §6.3.11 四类区域 + §6.3.12 操作理念），第七~八章是跨周期验证 + 数据规范。**核心决策在第六章，前面各章是数据支撑。**
...
- [六、操盘计划](#六操盘计划)
    ...
    - [6.2 操盘决策速查（介入点/退出/流程/印证/收益）](#62-操盘决策速查介入点退出流程印证收益)
        - [6.2.1 退出情况分类（按转坏信号强度，从轻到重）](#621-退出情况分类按转坏信号强度从轻到重)
        - [6.2.2 完整操盘流程（分情况指南）](#622-完整操盘流程分情况指南)
            - [6.2.2.1 情况A：DJC丘内（DXZC>0）——主战场](#6221-情况adjc丘内dxzc0主战场)
            - [6.2.2.2 情况B：DJC丘外（DXZC<0）——观察区](#6222-情况bdjc丘外dxzc0观察区)
        - [6.2.3 与 5.3.3 位置决策表相互印证](#623-与-533-位置决策表相互印证)
        - [6.2.4 收益估算与风险提示](#624-收益估算与风险提示)
    - [6.3 四层漏斗定型操作规则（综合所有分析，2026-09-08 定稿，2026-09-12 统一为四层）](#63-四层漏斗定型操作规则综合所有分析2026-09-08-定稿2026-09-12-统一为四层)
        - [6.3.1 第1层：DXCD 选择（上/忐 首选，忠 谨慎，中/下/忑 放弃）](#631-第1层dxcd-选择上忐-首选忠-谨慎中下忑-放弃)
        - [6.3.2 第2层：DXZC>0 区域护型选择——定型操作的核心层](#632-第2层dxzc0-区域护型选择定型操作的核心层)
        - [6.3.3 第3层：介入时机（DJA丘 + 日等型 + BSHA5）](#633-第3层介入时机dja丘-日等型-bsha5)
        - [6.3.4 定型操作规则（最终收敛）](#634-定型操作规则最终收敛)
            - [6.3.4.1 护型操作决策流程图（可视化总结，2026-09-10，2026-09-11 补全DXCD分支）：](#6341-护型操作决策流程图可视化总结2026-09-102026-09-11-补全dxcd分支)
            - [6.3.4.2 流程图遍历验证（随机10个代码，2026-09-11）：](#6342-流程图遍历验证随机10个代码2026-09-11)
        - [6.3.5 定型操作期望收益（2026-09-08 验证，BSHA 用列19修正后）：](#635-定型操作期望收益2026-09-08-验证bsha-用列19修正后)
        - [6.3.6 持有条件样本占总天数比例（2026-09-08 验证）：](#636-持有条件样本占总天数比例2026-09-08-验证)
            - [6.3.6.1 高波池（Qic+Qim+Qit，3451只）：](#6361-高波池qicqimqit3451只)
            - [6.3.6.2 低波池（Qe+Qin+Qd+Qif+Qst，3732只）：](#6362-低波池qeqinqdqifqst3732只)
        - [6.3.7 操作区域"少而精"原则——不要随便退出，阴柱可介入，仅诱多退出（2026-09-08 验证）：](#637-操作区域少而精原则不要随便退出阴柱可介入仅诱多退出2026-09-08-验证)
        - [6.3.8 股性分析：持有条件比例是否定义股票性格（2026-09-08 验证）：](#638-股性分析持有条件比例是否定义股票性格2026-09-08-验证)
            - [6.3.8.1 股性稳定性（关键测试）：](#6381-股性稳定性关键测试)
            - [6.3.8.2 按持有比例分组：收益与波动（高波池）：](#6382-按持有比例分组收益与波动高波池)
            - [6.3.8.3 极端对比（最高10% vs 最低10%）：](#6383-极端对比最高10-vs-最低10)
        - [6.3.9 入管依据验证（旧准则）：DTZA 持续天数（升管/跌管）作为入管信号（2026-09-11 验证）：](#639-入管依据验证旧准则dtza-持续天数升管跌管作为入管信号2026-09-11-验证)
        - [6.3.10 四类区域的数学理论基础：均线系统几何结构（2026-09-17 定稿）](#6310-四类区域的数学理论基础均线系统几何结构2026-09-17-定稿)
        - [6.3.11 四类区域全集划分：禁区/执念/鸡肋/主战场（2026-09-16 定稿）](#6311-四类区域全集划分禁区执念鸡肋主战场2026-09-16-定稿)
        - [6.3.12 用户操作理念：沃土→甲乙己不破DJB→升管→垒升→剔诱多（2026-09-16 定稿）](#6312-用户操作理念沃土甲乙己不破djb升管垒升剔诱多2026-09-16-定稿)
            - [6.3.12.1 操作逻辑明确化：沃土 + DXZC>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）](#63121-操作逻辑明确化沃土-dxzc0甲乙己-优选升连2-不破djb不出2026-09-16-定稿)
...
> **🔗 本章结论 → 第六章入口：** DJC丘生命周期（甲启动→乙主升→丙预警）是第六章"介入时机"的底层依据——**DJC丘首/初介入选甲/己（§6.3.3 等1 31.4%），主升持有乙，顶部/尾部 DXZA>0 与 DXZB>0 双降警惕退出（§6.3.4 规则6）**。DJC丘（DXZC>0）是四类区域"主战场"的前提（§6.3.11）。
...
> - 己 DXZC>0 时升连占比 **66.1%**（与 §6.3.4.1 的 66.8% 一致）——**不是"都是升连"**，但升系（升连+升孕+升吞）合计 **82.9%**，绝大多数是升系柱排。
...
> - 己/甲 DXZC>0 时，次日冲高率约 21-23%，与基线（23.6%）接近——**护型本身不保证次日冲高**，需结合介入时机（DJA丘等1/2/3 + BSHA5，§6.3.3）和升连≥2%（§6.3.12.1）提升胜率。
...
> **🔗 本章结论 → 第六章入口：** 护型分析（甲/乙ZA>0 可做、乙ZA<0 交叉路口、丙持仓或退出、戊ZA>0 观察、丁/戊ZA<0 放弃）是第六章"护型选择"（§6.3.2）的直接数据支撑——**只做 甲/乙(ZA>0)/己，其余按角色处理**。DXZC>0 限定（上/忐/忠）对应四类区域"主战场"（§6.3.11）。
...
> **🔗 本章结论 → 第六章入口：** 等型分析（等1 介入点 31.4%、等2 30.3%、等3 持有期 29.5%）是第六章"介入时机"（§6.3.3）的直接数据支撑——**DJA丘等1/2/3 + BSHA5 触发器**。等6/等7（下跌通道）非介入点，应回避。
...
> **🔗 本章结论 → 第六章入口：** DXAB 正交动力学（转负概率、持续时间、快速负交）是第六章"护型选择 + 退出信号"的数据支撑——**DXAB>0 正交是护型可做的前提（§6.3.2），快速负交（DXCD中/下/忑）是退出预警（§6.3.4 规则）**。突破日护型坡度（甲最强）辅助介入时机选择。
...
> 5. **操作规则总纲（与§6.3.4定型操作规则统一）：** 只做 DXCD=上/忐（DXZC>0）→ 只做 甲/乙(ZA>0)/己 三个护型 → 乙(ZA<0)/丙 持仓不退出等待转确定状态、空仓等待选方向 → 戊(ZA>0) 观察等待转己/甲 → 丁/戊(ZA<0) 直接放弃 → 介入时机 DJA丘+日等型（等1最强）+BSHA5。**§5.2特有的入管判定过滤（DTZA=1不考虑入管/DTZA=2看升连跌孕/DTZA=3排除阴阳阴）+ 升管内跌排减仓 + 乙ZA<0/丙+DTZA≤-2跌管退出 + DXZC<0必须退出** 是§6.3.4的补充细化（风险管理层）。
>
...
> **🔗 本章结论 → 第六章入口：** 柱与DJA三态理论 + 操作规则优化是第六章"操作理念"（§6.3.12）的直接数据支撑——**升管（触哼入管，§5.1.4.1）、垒升（柱体≥2%，§5.3）、剔诱多（跌连-非触顶，§6.3.7.3）、不破DJB不出（DXZB退出规则，§5.2.9）** 正是 §6.3.12 操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）的微观层。**本章是第六章操作理念的"微观细节库"。**
...
>
> **第1步 · 选区（§6.3.11 四类区域，DXZE×CD 二维）：** 判断当前处于哪个区域——**主战场（DXZE>0+CD上/忐/忠）/ 鸡肋（DXZE<0+CD上/忐/忠）/ 执念（(CD中)+(DXZE>0+CD下/忑)）/ 禁区（DXZE<0+CD下/忑）**。只在**主战场**重仓，鸡肋可持有不宜过早介入，执念观察，禁区坚决不碰。
>
> **第2步 · 选点（§6.3 四层漏斗，DXCD→护型→介入时机）：** 在选定的区域内，用四层漏斗选具体介入点——**第1层 DXCD（上/忐 首选，忠 谨慎）、第2层 护型（甲乙己）、第3层 介入时机（DJA丘等1/2/3 + BSHA5）、第4层 微观（升管/垒升/剔诱多）**。
>
> **关系：** 四类区域（§6.3.11）是**顶层选区**（决定"做不做、重仓还是轻仓"），四层漏斗（§6.3）是**底层选点**（决定"具体买哪个、何时买"）。**先选区（大势/土质），再选点（护型/时机/微观）**——不要在恶土（禁区/执念）里谈微观，这正是 §6.3.12 操作理念"抓大看小"的核心。
...
> [!IMPORTANT]
> **升连位置护型发现（2026-09-08，见2.4.2.10）：** DXZC>0 时升连位置，甲（47.7%）最多、乙（41.0%）次之。**但按 DXCD 分化：DXCD=上 时乙反而最多（48.4% > 甲38.3%），且甲/乙都强（33%+）可介入；DXCD=忐 时甲更强（27.6%）；DXCD=忠 时升连弱（20-22%），不可靠。** 升连介入优先选 DXCD=上（甲/乙都强）；DXCD=忠 谨慎（归主战场/鸡肋，但升连弱，需叠加升连≥2%才介入，见 §6.3.11）。
...
### 6.3 四层漏斗定型操作规则（综合所有分析，2026-09-08 定稿，2026-09-12 统一为四层）
...
第1层 真假正交：DXCD上/忐（真正交）vs 中/下/忑（假正交），排除假正交
   ↓
第2层 大区域：DJC丘（DXZC>0），取DXCD=上/忐（首选），忠谨慎（归主战场/鸡肋，需叠加升连≥2%），排除中/下/忑
   ↓
...
> **⚠️ 结构调整（2026-09-12）：** 原"三层漏斗"把DXCD并入第1层大区域，未单独区分"真假正交"。**统一为四层漏斗（与§6.1一致）**——**第1层真假正交（DXCD）**、第2层大区域（DJC丘）、第3层护型过滤、第4层介入时机。§6.3聚焦第3层护型选择的详细评定（见4.8.2），§6.1聚焦第1层真假正交（见4.1）。

#### 6.3.1 第1层：DXCD 选择（上/忐 首选，忠 谨慎，中/下/忑 放弃）
...
> **数据支撑（详细见 §6.1 核心框架四层漏斗 + §2.9.3 按DXCD拆分）：** DXZC>0（上/忐/忠）时甲/乙ZA>0 主导且持久；DXZC<0（中/下/忑）时丁/戊ZA<0 主导，甲/乙ZA>0 几乎消失（乙ZA>0 仅0.03-0.7%）。**限定 DXZC>0 是有意义的**（见 §2.9.3）。
>
> **⚠️ 忠的归属（2026-09-16 统一）：** 忠 归**主战场**（DXZE>0）/ **鸡肋**（DXZE<0），遵循 VBA 仓日类定义（忠 ∈ {上,忐,忠}，见 §6.3.11）。但忠 = DXZC>0 且 DXZD<0（价格在 DJD=EMA60 之下），**上升受限、升连弱（20-22%）**——操作时需谨慎，**必须叠加"升连≥2%"（§5.3.9）才介入**，否则忠的升连不提升盈利性。上/忐 仍是首选（升连强、持久），忠 是次选（可做但需谨慎）。

#### 6.3.2 第2层：DXZC>0 区域护型选择——定型操作的核心层
> [!IMPORTANT]
...
#### 6.3.3 第3层：介入时机（DJA丘 + 日等型 + BSHA5）
> **介入时机完整数据见 §6.2 主介入点（等1/等2/等3 细分）+ §6.2（BSHA5 触发器 + 等5超跌反弹）。** 下表为收敛结论：
...
##### 6.3.4.1 护型操作决策流程图（可视化总结，2026-09-10，2026-09-11 补全DXCD分支）：
> **说明：** 下图将 §6.3 定型操作框架的完整决策逻辑可视化为流程图——先按 DXCD 判断区域，DXZC>0（上/忐）区域内按护型判断操作，可做的护型进入介入时机判断，持有的护型监控退出信号。**（若在支持 Mermaid 的查看器中打开可交互渲染；也可打开 `____temp/护型流程图.html` 查看 HTML 版。）**
...
##### 6.3.4.2 流程图遍历验证（随机10个代码，2026-09-11）：
...
> - **DXCD=忠（DXZC>0 但 DXZD<0）未覆盖 7.7%**——**已补全操作建议：谨慎介入**（归主战场/鸡肋，遵循 VBA 定义，见 §6.3.11）。忠虽是 DXZC>0，但 DXZD<0 意味着价格在 DJD(EMA60) 之下，上升受限、升连弱——**需叠加升连≥2%（§5.3.9）才介入**，否则忠的升连不提升盈利性。
>
> **✅ 补全后流程图覆盖：** DXCD=上/忐（约40%）按护型操作；DXCD=忠（约8%）谨慎介入（需叠加升连≥2%）；DXCD=中/下/忑（约52%）明确回避不属于操作范围。**每个柱都能找到对应操作指南（或明确的不操作指令）。**

#### 6.3.5 定型操作期望收益（2026-09-08 验证，BSHA 用列19修正后）：
> **口径说明：** 本节测算的是"定型操作组合"（DXCD=上/忐 × 甲/乙ZA>0/己 × DJA丘 + BSHA5）的收益；**§6.2 测算的是"DXZC>0+剔忠+广义正交"口径（DJC+剔忠）**。两者过滤不同，数据互补，可对照：（§6.2 主介入点 29.8%、年化30-60%）vs（本节定型组合 29.68-42.5%、年化18-32%）——**定型组合更"少而精"，年化更保守但每次胜率更高**。
...
##### 6.3.6.2 低波池（Qe+Qin+Qd+Qif+Qst，3732只）：
...
#### 6.3.7 操作区域"少而精"原则——不要随便退出，阴柱可介入，仅诱多退出（2026-09-08 验证）：
...
#### 6.3.8 股性分析：持有条件比例是否定义股票性格（2026-09-08 验证）：
...
#### 6.3.9 入管依据验证（旧准则）：DTZA 持续天数（升管/跌管）作为入管信号（2026-09-11 验证）：
...
#### 6.3.10 四类区域的数学理论基础：均线系统几何结构（2026-09-17 定稿）
...
> **结论：** 四类区域的数学基础 = **均线系统几何结构**。方向由长均线（EMA60）决定（P>EMA60=升势，P<EMA60=跌势），待定状态（中偏升/忠偏跌）由"跌破/站上短均线但长均线方向相反"决定，强度由均线排列（多头/空头）决定。**DXAB 在各区域内做优选，不改变区域划分。** 此数学框架是 §6.3.11 概率划分的理论基石。

#### 6.3.11 四类区域全集划分：禁区/执念/鸡肋/主战场（2026-09-16 定稿）
...
#### 6.3.12 用户操作理念：沃土→甲乙己不破DJB→升管→垒升→剔诱多（2026-09-16 定稿）
...
| 层 | 理念 | 对应已验证结论 | 数据 |
|:--|:--|:--|:--|
| ① 沃土 | DXZE>0 + DXCD上/忐 | §6.3.11 主战场 | +4.98pp（23.6%→28.6%）|
| ② 甲乙己 | 只做甲乙己护型 | §6.3.2 | P≥3% 27-30%，唯一可做三护型 |
...
| ⑤ 剔诱多 | 跌连-非触顶 | §6.3.7.3 | 24.9% 最明确诱多，退出 |
...
> **⚠️ 唯一需注意的取舍：** 沃土（35.6%）→ 甲乙己 → 升管 → 垒升 → 剔诱多，可操作范围收敛到极小，**交易频率低（§6.3.5 每年仅 10-14 次/股），但每次胜率高**。这是"少而精"的代价，需接受。

> **一句话：** 此理念 = §5.2.9"抓大看小"五层决策框架的完整落地——**先大势（DXZE>0）→ 沃土（DXCD上/忐）→ 主升区（DXZC>0）→ 护型（甲乙己，不破DJB不出）→ 微观（升管/垒升/剔诱多）**，每层都已有全量数据支撑，非新理念。

##### 6.3.12.1 操作逻辑明确化：沃土 + DXZC>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）
...
| + 沃土（DXZE>0+CD上/忐） | ~73% | +4.98pp | §6.3.11 主战场 |
...
>
> **⚠️ 注意：** 沃土+甲乙己把样本收敛到极小（§6.3.5 每年仅 10-14 次/股），再叠加"升连涨幅≥2%"（样本56万），可操作频率进一步降低——但每次胜率极高（92.75%）。这是"少而精"的极致形态，需接受低交易频率换取高胜率。
