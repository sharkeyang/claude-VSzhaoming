# -*- coding: utf-8 -*-
"""
验证 DXZC>0 + DTZD>0 持有到条件不满足的胜率（均线价进出）
===========================================================
- 入场条件满足时，次日以均线价（前一日收盘价）入场
- 出场条件触发时，次日以均线价（前一日收盘价）出场
- 胜率口径：持有期总收益>0
- 高波池 Qic+Qim+Qit
"""
...
OUTF = io.open('____temp/_DXZC_DTZD_持有期胜率_v2.txt', 'w', encoding='utf-8')
def out(s=''):
    OUTF.write(s + '\n')
...
for fname in os.listdir('昭明算展/谕组日'):
    ...
    try:
        ...
        with open(p, 'r', encoding='gbk', errors='replace') as f:
            ...
            # 策略1: ZC>0 入场，ZC<=0 出场
            # 均线价进出：条件满足的次日以均线价入场，条件触发的次日以均线价出场
            in_trade = False
            entry_price = None
            entry_idx = -1
            for i in range(len(prices)):
                if not in_trade:
                    # 等待入场信号：当日 ZC>0，次日以均线价入场
                    if i + 1 < len(prices) and zcs[i] > 0:
                        in_trade = True
                        entry_idx = i + 1
                        entry_price = prices[i]  # 均线价 = 前一日收盘价
                else:
                    # 检查出场信号：当日 ZC<=0，次日以均线价出场
                    if zcs[i] <= 0:
                        # 出场：以均线价（前一日收盘价）出场
            ...
            for i in range(len(prices)):
                if not in_trade:
                    if i + 1 < len(prices) and zcs[i] > 0 and dtzds[i] > 0:
                        in_trade = True
                        entry_idx = i + 1
            ...
            for i in range(len(prices)):
                if not in_trade:
                    if i + 1 < len(prices) and zes[i] > 0 and zcs[i] > 0:
                        in_trade = True
                        entry_idx = i + 1
                        entry_price = prices[i]
                else:
