...
MAX_HOLD = 20
results = defaultdict(list)  # mode -> [rets]
...
print('总文件:', n)
print('\n=== 不同退出规则风险指标 ===')
names = {'full':'完整规则','reduce_only':'仅减仓','exit_only':'仅跌管退出','none':'无退出'}
for mode in ['full','reduce_only','exit_only','none']:
    rets = results[mode]
    rets.sort()
    ...
    print(f'{names[mode]:<12}: n={len(rets)}, 成功率={wins/len(rets)*100:.2f}%, 平均收益={sum(rets)/len(rets):.3f}%, 平均亏损={avg_loss:.3f}%, 最大亏损={max_loss:.3f}%, 最差5%均值={avg_worst5:.3f}%')
