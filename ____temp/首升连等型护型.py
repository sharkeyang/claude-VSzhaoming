...
for i, f in enumerate(files):
    df = pd.read_csv(f, encoding='gbk', header=None, usecols=[5,9,10,13,14,26,44])
    df.columns = ['涨幅','DXAB','柱排','日ZA','日ZC','次日高幅','日等型']
    ...
    for hx in ['a','b','r']:
        for e in ['等1','等2','等3','等4','等5','等6','等7']:
            g = sub[(sub['护型码']==hx) & (sub['日等型']==e)]
            add(f'护型{hx}×{e}', g)
    if (i+1) % 2000 == 0:
        print(f'  已处理 {i+1}/{len(files)}', flush=True)
print('\n' + '='*70)
print('首升连+涨幅≥2% × 日等型 × 护型')
print('='*70)
rows = []
for cls, (n, p3) in agg.items():
    if n >= 1000: rows.append((cls, n, p3/n*100))
rows.sort(key=lambda r: -r[2])
print(f"{'组合':<20}{'样本':>10}{'5天≥3%':>10}")
