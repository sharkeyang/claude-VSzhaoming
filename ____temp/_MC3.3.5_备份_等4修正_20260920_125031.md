...
- [六、操盘计划](#六操盘计划)
    ...
    - [6.8 第3层：介入时机（选时机）——升管坐标定位（新框架）](#68-第3层介入时机选时机升管坐标定位新框架)
        ...
        - [6.8.4 等4：出管/跌管区域（乙DXZA<0）](#684-等4出管跌管区域乙dxza0)
...
| **忠** | P > EMA26，EMA26 < EMA60 | 价格在 EMA26 之上、EMA60 之下（均线空头）|
| **忑** | P < EMA60 | 价格在 EMA60 之下，均线空头 |

> **VBA 判定逻辑（神谕.bas:4806-4823）：** DXCD 由 BTCD（EMA26 vs EMA60）、BTZC（P vs EMA26）、BTZD（P vs EMA60）三者决定：
...
| **忠** | P>EMA26，EMA26<EMA60 | **跌势（偏跌）** | 16.4-29.5% | 16.5-31.6% |
| **忑** | P<EMA60 | **跌势** | 2.4-4.1% | 0.6-3.3% |
...
> - **忠（P>EMA26，EMA26<EMA60）偏跌**：站上短均线但跌破长均线，次日ZD>0 仅 16-32% —— **用户判断"忠更可能跌势"正确**。
...
> **框架定位（2026-09-20）：** 第3层在"选区域（第1层 DXCD）→ 选护型（第2层 甲乙己）"之后，解决**何时介入**。核心思路：在升管（DXAB甲乙己 + 升管入管）中，用 **（DXAB + 等型）** 作为坐标定位介入时机。等型坐标：**等1（起点）/等2（早期）/等3（成熟）/等4（出管）**。
...
#### 6.8.4 等4：出管/跌管区域（乙DXZA<0）

> **等4 整体偏弱（20-23%），非主介入区。** 等7（超卖反弹）反而最高（22.92%），但样本少。
...
> **结论：** 等7 反而最高（22.92%，超卖反弹），但样本少；等4 整体偏弱（20-23%），**非主介入区**。
...
| **等4** | 出管/跌管 | 首下破反弹；**等6第二柱阳柱+次日再站DJA(37.37%)**；整体回避 | 跌破DJA无反弹离场 | 20-23% |

> **核心结论：** 介入时机坐标 = **等3 扩管（管宽≥5 + 触顶 + 升排）** 是最强介入点（升连 33.11%）；等2 靠触顶增强；等1 靠第2柱确认（维持升管29.39% vs 回落2.58%）；等4 整体回避（仅等6第二柱阳柱+次日再站DJA 37.37%可介入）。**管宽哼JA（col23）是决定性维度**——窄管(0-5)极弱应回避，扩管（≥5+推高顶）升排最强，管宽哼JA空（管宽消失）是转弱信号。
