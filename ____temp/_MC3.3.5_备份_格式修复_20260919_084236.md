...
- [四、等型分析](#四等型分析)
    ...
    - [4.3 等2：关注柱排方向](#43-等2关注柱排方向)
        ...
        - [4.3.2 等2阳柱的划分（升孕 vs 升排 vs 跌排，2026-09-08 验证）](#432-等2阳柱的划分升孕-vs-升排-vs-跌排2026-09-08-验证)
            ...
            - [4.3.2.11 乙(DXZA>0)DXZA=1+前跌排+升吞 是否诱多（2026-09-08 验证）](#43211-乙dxza0dxza1前跌排升吞-是否诱多2026-09-08-验证)
            - [4.3.2.11.1 乙(DXZA>0) DXZA=1 + 前跌排 + 升吞：](#432111-乙dxza0-dxza1-前跌排-升吞)
            - [4.3.2.11.3 结论：](#432113-结论)
...
- [六、操盘计划](#六操盘计划)
    ...
    - [6.2 第1层：DXCD 选择（上/忐 首选，忠 谨慎，中/下/忑 放弃）](#62-第1层dxcd-选择上忐-首选忠-谨慎中下忑-放弃)
        ...
        - [6.2.1.1 四类区域全集划分：禁区/执念/鸡肋/主战场（2026-09-16 定稿）> **课题（用户提出，2026-09-16）：** 用 DXZE、DXCD 两个维度，把全部区域划分为 禁区/执念/鸡肋/主战场 四类，要求**全集划分**（覆盖所有区域，无遗漏、无重叠）。遵循 VBA 仓日类定义（[神谕.bas:2653-2657](昭明计划VS优化_vba/IQQQ跨码_D据擎2神谕.bas#L2653-L2657)）。](#6211-四类区域全集划分禁区执念鸡肋主战场2026-09-16-定稿-课题用户提出2026-09-16-用-dxzedxcd-两个维度把全部区域划分为-禁区执念鸡肋主战场-四类要求全集划分覆盖所有区域无遗漏无重叠遵循-vba-仓日类定义神谕bas2653-2657昭明计划vs优化_vbaiqqq跨码_d据擎2神谕basl2653-l2657)
    ...
    - [6.12 旧内容：操盘决策速查（介入点/退出/流程/印证/收益）](#612-旧内容操盘决策速查介入点退出流程印证收益)
        ...
        - [6.12.6 持有条件样本占总天数比例（2026-09-08 验证）：> **课题（用户提出，2026-09-08）：** 统计符合持有条件（DXCD=上/忐 + 甲/乙ZA>0/己 + 日ZA>0）的样本占总天数的比例，覆盖高波池/低波池；针对单股统计该比例的分布规律（最高/最低/中位/统计指标）。](#6126-持有条件样本占总天数比例2026-09-08-验证-课题用户提出2026-09-08-统计符合持有条件dxcd上忐-甲乙za0己-日za0的样本占总天数的比例覆盖高波池低波池针对单股统计该比例的分布规律最高最低中位统计指标)
            ...
            - [6.12.6.2 低波池（Qe+Qin+Qd+Qif+Qst，3732只）：> **⚠️ 口径说明（2026-09-12）：** 低波池构成含 **Qst（ST/退市）**——ST/退市股票是**应剔除的**，不全是真正的低波池。Qst 的持有比例可能异常（ST长期停牌/退市导致持有条件失真）。**严格低波池应剔除 Qst**（Qe+Qin+Qd+Qif），但当前统计含 Qst（3732只）。下表数据含 Qst，仅供参考。](#61262-低波池qeqinqdqifqst3732只-口径说明2026-09-12-低波池构成含-qstst退市st退市股票是应剔除的不全是真正的低波池qst-的持有比例可能异常st长期停牌退市导致持有条件失真严格低波池应剔除-qstqeqinqdqif但当前统计含-qst3732只下表数据含-qst仅供参考)
        ...
        - [6.12.9 入管依据验证（旧准则）：DTZA 持续天数（升管/跌管）作为入管信号（2026-09-11 验证）：> **⚠️ 此为准入管依据（DTZA≥3），已被 §5.1.4.1 新准则（触哼入管）替代。新准则见 §5.1.4.1，比较优劣见 §5.1.4.3。**](#6129-入管依据验证旧准则dtza-持续天数升管跌管作为入管信号2026-09-11-验证-此为准入管依据dtza3已被-5141-新准则触哼入管替代新准则见-5141比较优劣见-5143)
...
##### 4.3.2.11 乙(DXZA>0)DXZA=1+前跌排+升吞 是否诱多（2026-09-08 验证）
...
##### 4.3.2.11.1 乙(DXZA>0) DXZA=1 + 前跌排 + 升吞：
