...
口径：条件满足时入场，条件不满足（ZE<=0或ZC<=0）时退出。
胜率定义：持有期内至少有一天涨幅>0（有浮盈）。
收益：持有期涨幅累加。
"""
...
for i, f in enumerate(files):
    ...
    for j in range(len(df)):
        ...
        if in_cycle and (not cond or j == len(df)-1):
            agg[board]['n'] += 1
            agg[board]['sum_ret'] += cycle_sum
            agg[board]['rets'].append(cycle_sum)
            if has_positive:
                agg[board]['n_win'] += 1
            else:
                agg[board]['n_loss'] += 1
...
with open('____temp/board_full_final.txt', 'w', encoding='utf-8') as f:
    f.write(result)
print(f'\n结果已保存: ____temp/board_full_final.txt')