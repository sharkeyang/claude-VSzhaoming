...
>
> **关键增强信号（§6.4 + §6.1.2）：** DJA丘等1（31.4%）+ BSHA5（39.4%）+ **升连≥2%（92.75%）**——升连必须≥2%，否则无优势（首升连无限制68.84%≈基线，涨幅<2% 52.07%反而更差）。
>
> **阅读指引：** 第一~五章是各维度分析（护型/等型/正交动力学/三态理论），第六章是决策收敛（§6.1 定型操作 + §6.2.1 四类区域 + §6.1.1 操作理念），第七~八章是跨周期验证 + 数据规范。**核心决策在第六章，前面各章是数据支撑。**
...
- [DXAB广义正交护型 × 日等型 交叉分析报告](#dxab广义正交护型--日等型-交叉分析报告)
  ...
  - [四、DXAB正交动力学](#四dxab正交动力学)
    ...
    - [4.8 DXAB 由负转正信号的综述](#48-dxab-由负转正信号的综述)
      - [4.8.1 DXAB 由负转正信号在 DXEF/DXCD 下的重要性（次日冲高率）：](#481-dxab-由负转正信号在-dxefdxcd-下的重要性次日冲高率)
      - [4.8.2 是否会延续（转正后 N 天仍保持 BTAB\>0）：](#482-是否会延续转正后-n-天仍保持-btab0)
      - [4.8.3 可能持续多少时间（转正后 BTAB\>0 持续天数 × DXEF/DXCD）：](#483-可能持续多少时间转正后-btab0-持续天数--dxefdxcd)
      - [4.8.4 遇见阴柱如何处理：](#484-遇见阴柱如何处理)
      - [4.8.5 遇见 DXZA\<0 如何处理：](#485-遇见-dxza0-如何处理)
      - [4.8.6 遇见 DXZB\<0 如何处理：](#486-遇见-dxzb0-如何处理)
      - [4.8.7 综合结论：DXAB 由负转正信号的操作指南](#487-综合结论dxab-由负转正信号的操作指南)
      - [4.8.8 DXAB 由负转正后的特权（2026-09-08 验证）](#488-dxab-由负转正后的特权2026-09-08-验证)
        - [4.8.8.1 DXAB 负转正后阴柱回调至 DJA 附近（日ZA≤1 且阴柱），次日冲高率：](#4881-dxab-负转正后阴柱回调至-dja-附近日za1-且阴柱次日冲高率)
        - [4.8.8.2 回调后 N 天内转负概率：](#4882-回调后-n-天内转负概率)
        - [4.8.8.3 回调后 3 天内是否再次上升：](#4883-回调后-3-天内是否再次上升)
        - [4.8.8.4 结论：DXAB 负转正后的特权取决于 DXZE](#4884-结论dxab-负转正后的特权取决于-dxze)
      - [4.8.9 DXAB 负转正特权深化 + 转负比例 + 升连上影（2026-09-08 验证）](#489-dxab-负转正特权深化--转负比例--升连上影2026-09-08-验证)
        - [4.8.9.1 DXAB 负转正后特权（按 DXEF 直接分）：](#4891-dxab-负转正后特权按-dxef-直接分)
        - [4.8.9.2 DXAB 负转正后转负比例（从1天开始罗列，分 DXZE/DXEF）：](#4892-dxab-负转正后转负比例从1天开始罗列分-dxzedxef)
        - [4.8.9.3 升连过程中上影阳柱/跌孕（用收盘涨幅\>0概率判断诱多）：](#4893-升连过程中上影阳柱跌孕用收盘涨幅0概率判断诱多)
        - [4.8.9.4 DJA 之上跌连后升吞：](#4894-dja-之上跌连后升吞)
        - [4.8.9.5 乙(DXZA\>0)DXZA=1+前跌排+升吞 是否诱多（用收盘涨幅\>0概率修正）：](#4895-乙dxza0dxza1前跌排升吞-是否诱多用收盘涨幅0概率修正)
  ...
  - [六、操盘计划](#六操盘计划)
    - [6.1 三层漏斗总览（核心框架）](#61-三层漏斗总览核心框架)
      - [6.1.1 指导思想：用户操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）\> **课题（用户提出，2026-09-16）：** 在沃土，寻找 DXAB（甲乙己，不破DJB不出）+ 在升管 + 垒升，剔除诱多。**逐层评论确认：此理念 = §5.2.9"抓大看小"五层决策框架的完整落地，每一层都已有全量数据支撑，非新理念，而是对已验证结论的有机整合。**](#611-指导思想用户操作理念沃土甲乙己不破djb升管垒升剔诱多-课题用户提出2026-09-16-在沃土寻找-dxab甲乙己不破djb不出-在升管--垒升剔除诱多逐层评论确认此理念--529抓大看小五层决策框架的完整落地每一层都已有全量数据支撑非新理念而是对已验证结论的有机整合)
        - [6.1.2 操作逻辑明确化：沃土 + DXZC\>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）](#612-操作逻辑明确化沃土--dxzc0甲乙己--优选升连2-不破djb不出2026-09-16-定稿)
    - [6.2 第1层：DXCD 选择（上/忐 首选，忠 谨慎，中/下/忑 放弃）](#62-第1层dxcd-选择上忐-首选忠-谨慎中下忑-放弃)
      - [6.2.1 四类区域：数学基础 + 全集划分](#621-四类区域数学基础--全集划分)
      - [6.2.1.1 四类区域全集划分：禁区/执念/鸡肋/主战场（2026-09-16 定稿）\> **课题（用户提出，2026-09-16）：** 用 DXZE、DXCD 两个维度，把全部区域划分为 禁区/执念/鸡肋/主战场 四类，要求**全集划分**（覆盖所有区域，无遗漏、无重叠）。遵循 VBA 仓日类定义（神谕.bas:2653-2657）。](#6211-四类区域全集划分禁区执念鸡肋主战场2026-09-16-定稿-课题用户提出2026-09-16-用-dxzedxcd-两个维度把全部区域划分为-禁区执念鸡肋主战场-四类要求全集划分覆盖所有区域无遗漏无重叠遵循-vba-仓日类定义神谕bas2653-2657)
    - [6.3 第2层：DXZC\>0 区域护型选择——定型操作的核心层\> \[!IMPORTANT\]](#63-第2层dxzc0-区域护型选择定型操作的核心层-important)
    - [6.4 第3层：介入时机（选时机）](#64-第3层介入时机选时机)
      - [6.4.1 介入时机（DJA丘 + 日等型 + BSHA5）\> **介入时机完整数据见 §6.12 主介入点（等1/等2/等3 细分）+ §6.12（BSHA5 触发器 + 等5超跌反弹）。** 下表为收敛结论：](#641-介入时机dja丘--日等型--bsha5-介入时机完整数据见-612-主介入点等1等2等3-细分-612bsha5-触发器--等5超跌反弹-下表为收敛结论)
      - [6.4.2 升管判断（触哼入管 §5.1.4.1，2026-09-17 新准则）](#642-升管判断触哼入管-51412026-09-17-新准则)
      - [6.4.3 垒升判断（柱体≥2%，§5.3）](#643-垒升判断柱体253)
    - [6.5 定型操作规则（最终收敛）\> **✅ 操作方法定型：**](#65-定型操作规则最终收敛--操作方法定型)
      - [6.5.1 护型操作决策流程图（可视化总结，2026-09-10，2026-09-11 补全DXCD分支）：\> **说明：** 下图将 §6.1 定型操作框架的完整决策逻辑可视化为流程图——先按 DXCD 判断区域，DXZC\>0（上/忐）区域内按护型判断操作，可做的护型进入介入时机判断，持有的护型监控退出信号。**（若在支持 Mermaid 的查看器中打开可交互渲染；也可打开 `____temp/护型流程图.html` 查看 HTML 版。）**](#651-护型操作决策流程图可视化总结2026-09-102026-09-11-补全dxcd分支-说明-下图将-61-定型操作框架的完整决策逻辑可视化为流程图先按-dxcd-判断区域dxzc0上忐区域内按护型判断操作可做的护型进入介入时机判断持有的护型监控退出信号若在支持-mermaid-的查看器中打开可交互渲染也可打开-____temp护型流程图html-查看-html-版)
      - [6.5.2 流程图遍历验证（随机10个代码，2026-09-11）：\> **课题（用户提出，2026-09-11）：** 按照护型操作决策流程图，遍历随机10个代码，是否能应对每个柱的操作？如果发现找不到操作指南的柱，就进行报告。](#652-流程图遍历验证随机10个代码2026-09-11-课题用户提出2026-09-11-按照护型操作决策流程图遍历随机10个代码是否能应对每个柱的操作如果发现找不到操作指南的柱就进行报告)
    - [6.12 旧内容：操盘决策速查（介入点/退出/流程/印证/收益）](#612-旧内容操盘决策速查介入点退出流程印证收益)
      ...
      - [6.12.3 与 5.3.3 位置决策表相互印证](#6123-与-533-位置决策表相互印证)
      - [6.12.4 收益估算与风险提示](#6124-收益估算与风险提示)
      - [6.12.5 定型操作期望收益（2026-09-08 验证，BSHA 用列19修正后）：\> **口径说明：** 本节测算的是"定型操作组合"（DXCD=上/忐 × 甲/乙ZA\>0/己 × DJA丘 + BSHA5）的收益；**§6.12 测算的是"DXZC\>0+剔忠+广义正交"口径（DJC+剔忠）**。两者过滤不同，数据互补，可对照：（§6.12 主介入点 29.8%、年化30-60%）vs（本节定型组合 29.68-42.5%、年化18-32%）——**定型组合更"少而精"，年化更保守但每次胜率更高**。](#6125-定型操作期望收益2026-09-08-验证bsha-用列19修正后-口径说明-本节测算的是定型操作组合dxcd上忐--甲乙za0己--dja丘--bsha5的收益612-测算的是dxzc0剔忠广义正交口径djc剔忠两者过滤不同数据互补可对照612-主介入点-298年化30-60vs本节定型组合-2968-425年化18-32定型组合更少而精年化更保守但每次胜率更高)
      - [6.12.6 持有条件样本占总天数比例（2026-09-08 验证）：\> **课题（用户提出，2026-09-08）：** 统计符合持有条件（DXCD=上/忐 + 甲/乙ZA\>0/己 + 日ZA\>0）的样本占总天数的比例，覆盖高波池/低波池；针对单股统计该比例的分布规律（最高/最低/中位/统计指标）。](#6126-持有条件样本占总天数比例2026-09-08-验证-课题用户提出2026-09-08-统计符合持有条件dxcd上忐--甲乙za0己--日za0的样本占总天数的比例覆盖高波池低波池针对单股统计该比例的分布规律最高最低中位统计指标)
        - [6.12.6.1 高波池（Qic+Qim+Qit，3451只）：| 指标 | 值 |](#61261-高波池qicqimqit3451只-指标--值-)
        - [6.12.6.2 低波池（Qe+Qin+Qd+Qif+Qst，3732只）：\> **⚠️ 口径说明（2026-09-12）：** 低波池构成含 **Qst（ST/退市）**——ST/退市股票是**应剔除的**，不全是真正的低波池。Qst 的持有比例可能异常（ST长期停牌/退市导致持有条件失真）。**严格低波池应剔除 Qst**（Qe+Qin+Qd+Qif），但当前统计含 Qst（3732只）。下表数据含 Qst，仅供参考。](#61262-低波池qeqinqdqifqst3732只-️-口径说明2026-09-12-低波池构成含-qstst退市st退市股票是应剔除的不全是真正的低波池qst-的持有比例可能异常st长期停牌退市导致持有条件失真严格低波池应剔除-qstqeqinqdqif但当前统计含-qst3732只下表数据含-qst仅供参考)
      - [6.12.7 操作区域"少而精"原则——不要随便退出，阴柱可介入，仅诱多退出（2026-09-08 验证）：\> **课题（用户提出，2026-09-08）：** 操作区域（DXCD=上/忐 + DXZC\>0 + 甲/乙ZA\>0/己/乙ZA\<0）在整个单只股票存续时间实际非常小，所以不要随便退出。除了极少数可明确识别的诱多（要特别总结），其他都要持有，且可以在阴柱介入。](#6127-操作区域少而精原则不要随便退出阴柱可介入仅诱多退出2026-09-08-验证-课题用户提出2026-09-08-操作区域dxcd上忐--dxzc0--甲乙za0己乙za0在整个单只股票存续时间实际非常小所以不要随便退出除了极少数可明确识别的诱多要特别总结其他都要持有且可以在阴柱介入)
      - [6.12.8 股性分析：持有条件比例是否定义股票性格（2026-09-08 验证）：\> **课题（用户提出，2026-09-08）：** 能否用持有条件比例定义股性（股票性格）？高比例=适合持有（有操盘手/游资/机构持有）=好股；低比例=杂毛=完全不适合持有。](#6128-股性分析持有条件比例是否定义股票性格2026-09-08-验证-课题用户提出2026-09-08-能否用持有条件比例定义股性股票性格高比例适合持有有操盘手游资机构持有好股低比例杂毛完全不适合持有)
        - [6.12.8.1 股性稳定性（关键测试）：| 指标 | 值 |](#61281-股性稳定性关键测试-指标--值-)
        - [6.12.8.2 按持有比例分组：收益与波动（高波池）：| 分组 | 股票数 | P(≥3%) | 均高幅 | 日均涨幅 | 波动(σ) |](#61282-按持有比例分组收益与波动高波池-分组--股票数--p3--均高幅--日均涨幅--波动σ-)
        - [6.12.8.3 极端对比（最高10% vs 最低10%）：| 组 | P(≥3%) | 日均涨幅 | 波动 |](#61283-极端对比最高10-vs-最低10-组--p3--日均涨幅--波动-)
      - [6.12.9 入管依据验证（旧准则）：DTZA 持续天数（升管/跌管）作为入管信号（2026-09-11 验证）：\> **⚠️ 此为准入管依据（DTZA≥3），已被 §5.1.4.1 新准则（触哼入管）替代。新准则见 §5.1.4.1，比较优劣见 §5.1.4.3。**](#6129-入管依据验证旧准则dtza-持续天数升管跌管作为入管信号2026-09-11-验证-️-此为准入管依据dtza3已被-5141-新准则触哼入管替代新准则见-5141比较优劣见-5143)
...
> - 己/甲 DXZC>0 时，次日冲高率约 21-23%，与基线（23.6%）接近——**护型本身不保证次日冲高**，需结合介入时机（DJA丘等1/2/3 + BSHA5，§6.4）和升连≥2%（§6.1.2）提升胜率。
...
> - **操作含义：** 升连位置 DXCD=上 时乙/甲都强（33%+）可介入；DXCD=忐 时甲更强（27.6%）；DXCD=忠 时升连弱（20-22%），不可靠。
>
> **⚠️ 涨幅限制（2026-09-16 补充）：** 本节"升连位置"数据**未区分涨幅**。实际介入必须叠加"升连≥2%"（§5.3.9）——首升连无限制（68.84%）≈ 基线（68.86%）无优势，涨幅<2%的升连（52.07%）反而更差，**升连+涨幅≥2% 才达 92.75%**。上表 P(≥3%) 是"升连位置"口径，实际介入需在升连基础上再筛涨幅≥2%。
...
#### 4.8.1 DXAB 由负转正信号在 DXEF/DXCD 下的重要性（次日冲高率）：
...
#### 4.8.3 可能持续多少时间（转正后 BTAB>0 持续天数 × DXEF/DXCD）：
...
##### 4.8.8.4 结论：DXAB 负转正后的特权取决于 DXZE
...
##### 4.8.9.2 DXAB 负转正后转负比例（从1天开始罗列，分 DXZE/DXEF）：
...
##### 4.8.9.3 升连过程中上影阳柱/跌孕（用收盘涨幅>0概率判断诱多）：
...
##### 4.8.9.5 乙(DXZA>0)DXZA=1+前跌排+升吞 是否诱多（用收盘涨幅>0概率修正）：
...
| **管中** | 巨宽（连阳触顶）/中宽（阴阳交错或缓慢升连）/窄管（十字星低波）| 镜面 |
...
| 模式 | 次日P3% | 5天P3% | 操作 |
|:--|--:|--:|:--|
| **巨宽（连阳触顶）** | **26.33** | 74.44 | 垒升典型，可介入 |
| **中宽（阴阳交错/缓慢升连）** | 22.57 | 70.43 | 可介入 |
| **窄管（十字星低波）** | **14.82** | **51.61** | **应回避**（无有效升排）|

> **✅ 管中模式区分度大：** 巨宽（26.33%）> 中宽（22.57%）> 窄管（14.82%），差11.5pp。**窄管应回避**——这是旧准则（DTZA≥3）没有的维度。
...
**结论：新准则（触哼入管）在入管质量（P3更高）、误判控制（出管率更低）、管中细分（巨宽/中宽/窄管）、出管预警（连阴/阴阳阴×触顶）四个方面全面优于旧准则（DTZA≥3）。新准则作为入管判定标准。**
...
> **🔗 本章结论 → 第六章入口：** 柱与DJA三态理论 + 操作规则优化是第六章"操作理念"（§6.1.1）的直接数据支撑——**升管（触哼入管，§5.1.4.1）、垒升（柱体≥2%，§5.3）、剔诱多（跌连-非触顶，§6.12.7）、不破DJB不出（DXZB退出规则，§5.2.9）** 正是 §6.1.1 操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）的微观层。**本章是第六章操作理念的"微观细节库"。**
...
| 层 | 变量 | 作用 | 数据支撑 |
|:--|:--|:--|:--|
| **第1层 DXCD选择** | DXCD上/忐 vs 中/下/忑 + DJC丘（DXZC>0）| 选区域（真假正交+大区域）| 真正交28.8% vs 假正交19.7%；+7.20pp |
| **第2层 护型选择** | DXAB广义正交 | 选护型（排除下破DJC高发的非广义）| 非广义下破33.6% vs 广义7.5% |
| **第3层 介入时机** | DJA丘（日ZA>0）+ 日等型 | 选时机 | 等1 31.4%最强 |

> **⚠️ 三层漏斗（2026-09-18 统一）：** 原"四层漏斗"（真假正交/大区域/护型过滤/介入时机）中，**真假正交（DXCD）与大区域（DJC丘DXZC>0）是同一维度**（DXCD隐含DXZC），合并为**第1层 DXCD选择（选区域）**。三层漏斗 = **第1层DXCD选择（选区域）→ 第2层护型选择（选护型）→ 第3层介入时机（选时机）**。



> [!IMPORTANT]
> **三层漏斗总纲（原§6.2 三层漏斗定型操作规则，并入6.1）：** 三层漏斗把可操作范围收敛到极小，与上方核心框架表一致。
...
**操作框架 = 三层漏斗（与§6.1核心框架一致），把可操作范围收敛到极小：**

```
第1层 DXCD选择：DXCD上/忐（真正交）vs 中/下/忑（假正交）+ DJC丘（DXZC>0），排除假正交，取上/忐（首选），忠谨慎（归主战场/鸡肋，需叠加升连≥2%），排除中/下/忑
   ↓
...
> **⚠️ 三层漏斗（2026-09-18 统一）：** 原"四层漏斗"把真假正交（DXCD）与大区域（DJC丘DXZC>0）拆成两层，但两者是同一维度（DXCD隐含DXZC），合并为**第1层 DXCD选择（选区域）**。三层漏斗 = **第1层DXCD选择（选区域）→ 第2层护型选择（选护型）→ 第3层介入时机（选时机）**。§6.3聚焦第2层护型选择的详细评定（见4.8.2），§6.2聚焦第1层DXCD选择（见4.1）。


#### 6.1.1 指导思想：用户操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）> **课题（用户提出，2026-09-16）：** 在沃土，寻找 DXAB（甲乙己，不破DJB不出）+ 在升管 + 垒升，剔除诱多。**逐层评论确认：此理念 = §5.2.9"抓大看小"五层决策框架的完整落地，每一层都已有全量数据支撑，非新理念，而是对已验证结论的有机整合。**

**理念五层（与 §5.2.9 决策顺序一致，每层已验证）：**

| 层 | 理念 | 对应已验证结论 | 数据 |
|:--|:--|:--|:--|
| ① 沃土 | DXZE>0 + DXCD上/忐 | §6.2.1 主战场 | +4.98pp（23.6%→28.6%）|
| ② 甲乙己 | 只做甲乙己护型 | §6.3 | P≥3% 27-30%，唯一可做三护型 |
| ②' 不破DJB不出 | 持有底线 = DXZB>0（不破DJB）| **§5.2.9 已全量验证** | 四象限 69.96% 坚定持有 / 41.62% 退出 |
| ③ 升管 | 触哼入管（§5.1.4.1 新准则）| §5.1.4.1 | 次日P3 25.14%（vs 旧准则23.00%）|
| ④ 垒升 | 柱体≥2% | §5.3 | 升管内部最强，升孕≥2%获利98.07% |
| ⑤ 剔诱多 | 跌连-非触顶 | §6.12.7 | 24.9% 最明确诱多，退出 |
...
##### 6.1.2 操作逻辑明确化：沃土 + DXZC>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）
...
| 层 | 5天≥3% | 相对基线 | 说明 |
|:--|:--:|:--:|:--|
| 基线（甲乙己ZC>0） | 68.86% | — | §5.3.9 基线 |
| + 沃土（DXZE>0+CD上/忐） | ~73% | +4.98pp | §6.2.1 主战场 |
| + 升连涨幅≥2% | **92.75%** | +23.9pp | §5.3.9 核心贡献 |
| + 不破DJB持有 | 69.96%（坚定持有象限） | 持有期保护 | §5.2.9 防过早退出 |
...
> **一句话：** 操作逻辑明确为 **沃土 + DXZC>0甲乙己 + 升连涨幅≥2%（垒升）+ 不破DJB不出**，盈利性 5天≥3% 约 **92.75%**（§5.3.9 验证过的最强可执行策略）。**升连必须≥2%** 是决定性修正——不加涨幅限制的升连（68.84%）与基线持平，涨幅<2%的升连（52.07%）反而更差。



### 6.2 第1层：DXCD 选择（上/忐 首选，忠 谨慎，中/下/忑 放弃）

| DXCD | DXZC | 甲/乙ZA>0 行占比 | 甲持续 | 结论 |
|:--|:--:|:--:|:--:|:--|
| **上** | >0 | 30.0%+35.3%=**65.3%** | 5.8天 | ✅ 首选（多头主导，样本最多）|
| **忐** | >0 | 55.6%+30.7%=**86.3%** | 4.1天 | ✅ 首选（甲/乙ZA>0 占比最高）|
| 忠 | >0 | 57.0%+17.9%=74.9% | 2.7天 | ⚠️ **谨慎**（归主战场/鸡肋，但升连弱20-22%，需叠加升连≥2%）|
| 中/下/忑 | <0 | 甲0.23-5.0% | 1.2-1.8天 | ❌ 放弃（甲/乙ZA>0 几乎消失）|

> **数据支撑（详细见 §6.1 核心框架三层漏斗 + §2.9.3 按DXCD拆分）：** DXZC>0（上/忐/忠）时甲/乙ZA>0 主导且持久；DXZC<0（中/下/忑）时丁/戊ZA<0 主导，甲/乙ZA>0 几乎消失（乙ZA>0 仅0.03-0.7%）。**限定 DXZC>0 是有意义的**（见 §2.9.3）。
>
> **⚠️ 忠的归属（2026-09-16 统一）：** 忠 归**主战场**（DXZE>0）/ **鸡肋**（DXZE<0），遵循 VBA 仓日类定义（忠 ∈ {上,忐,忠}，见 §6.2.1）。但忠 = DXZC>0 且 DXZD<0（价格在 DJD=EMA60 之下），**上升受限、升连弱（20-22%）**——操作时需谨慎，**必须叠加"升连≥2%"（§5.3.9）才介入**，否则忠的升连不提升盈利性。上/忐 仍是首选（升连强、持久），忠 是次选（可做但需谨慎）。
...
**核心数学特性（EMA）：** 当价格 P > EMA 时，EMA 向上移动（新价格高于均值拉高均值），趋势向上；反之向下。**"价格在 EMA 之上 → 保持升势"是 EMA 的数学特性，成立。**

**DXCD 六类的数学定义（P、JC=EMA26、JD=EMA60 三者排列，源自神谕.bas:4806-4823）：**
...
**待定状态（一条均线之上一条之下）的方向倾向：**

| 待定 | 数学条件 | 方向倾向 | 原因 |
|:--|:--|:--|:--|
| **中** | EMA26>P>EMA60 | **偏升** | 跌破短均线（短期回调），站上长均线（长期升势）|
| **忠** | P>EMA26，EMA26<EMA60 | **偏跌** | 站上短均线（短期反弹），跌破长均线（长期跌势）|
...
#### 6.2.1.1 四类区域全集划分：禁区/执念/鸡肋/主战场（2026-09-16 定稿）> **课题（用户提出，2026-09-16）：** 用 DXZE、DXCD 两个维度，把全部区域划分为 禁区/执念/鸡肋/主战场 四类，要求**全集划分**（覆盖所有区域，无遗漏、无重叠）。遵循 VBA 仓日类定义（[神谕.bas:2653-2657](昭明计划VS优化_vba/IQQQ跨码_D据擎2神谕.bas#L2653-L2657)）。
...
### 6.3 第2层：DXZC>0 区域护型选择——定型操作的核心层> [!IMPORTANT]
...
### 6.4 第3层：介入时机（选时机）
#### 6.4.1 介入时机（DJA丘 + 日等型 + BSHA5）> **介入时机完整数据见 §6.12 主介入点（等1/等2/等3 细分）+ §6.12（BSHA5 触发器 + 等5超跌反弹）。** 下表为收敛结论：
...
#### 6.5.1 护型操作决策流程图（可视化总结，2026-09-10，2026-09-11 补全DXCD分支）：> **说明：** 下图将 §6.1 定型操作框架的完整决策逻辑可视化为流程图——先按 DXCD 判断区域，DXZC>0（上/忐）区域内按护型判断操作，可做的护型进入介入时机判断，持有的护型监控退出信号。**（若在支持 Mermaid 的查看器中打开可交互渲染；也可打开 `____temp/护型流程图.html` 查看 HTML 版。）**
...
#### 6.5.2 流程图遍历验证（随机10个代码，2026-09-11）：> **课题（用户提出，2026-09-11）：** 按照护型操作决策流程图，遍历随机10个代码，是否能应对每个柱的操作？如果发现找不到操作指南的柱，就进行报告。
>
...
> - **DXCD=忠（DXZC>0 但 DXZD<0）未覆盖 7.7%**——**已补全操作建议：谨慎介入**（归主战场/鸡肋，遵循 VBA 定义，见 §6.2.1）。忠虽是 DXZC>0，但 DXZD<0 意味着价格在 DJD(EMA60) 之下，上升受限、升连弱——**需叠加升连≥2%（§5.3.9）才介入**，否则忠的升连不提升盈利性。
>
...
#### 6.12.5 定型操作期望收益（2026-09-08 验证，BSHA 用列19修正后）：> **口径说明：** 本节测算的是"定型操作组合"（DXCD=上/忐 × 甲/乙ZA>0/己 × DJA丘 + BSHA5）的收益；**§6.12 测算的是"DXZC>0+剔忠+广义正交"口径（DJC+剔忠）**。两者过滤不同，数据互补，可对照：（§6.12 主介入点 29.8%、年化30-60%）vs（本节定型组合 29.68-42.5%、年化18-32%）——**定型组合更"少而精"，年化更保守但每次胜率更高**。
...
#### 6.12.6 持有条件样本占总天数比例（2026-09-08 验证）：> **课题（用户提出，2026-09-08）：** 统计符合持有条件（DXCD=上/忐 + 甲/乙ZA>0/己 + 日ZA>0）的样本占总天数的比例，覆盖高波池/低波池；针对单股统计该比例的分布规律（最高/最低/中位/统计指标）。

##### 6.12.6.1 高波池（Qic+Qim+Qit，3451只）：| 指标 | 值 |
|:--|:--:|
...
##### 6.12.6.2 低波池（Qe+Qin+Qd+Qif+Qst，3732只）：> **⚠️ 口径说明（2026-09-12）：** 低波池构成含 **Qst（ST/退市）**——ST/退市股票是**应剔除的**，不全是真正的低波池。Qst 的持有比例可能异常（ST长期停牌/退市导致持有条件失真）。**严格低波池应剔除 Qst**（Qe+Qin+Qd+Qif），但当前统计含 Qst（3732只）。下表数据含 Qst，仅供参考。
...
#### 6.12.7 操作区域"少而精"原则——不要随便退出，阴柱可介入，仅诱多退出（2026-09-08 验证）：> **课题（用户提出，2026-09-08）：** 操作区域（DXCD=上/忐 + DXZC>0 + 甲/乙ZA>0/己/乙ZA<0）在整个单只股票存续时间实际非常小，所以不要随便退出。除了极少数可明确识别的诱多（要特别总结），其他都要持有，且可以在阴柱介入。
>
> **6.3.7.1 操作区域占比（高波池）：**
| 区域 | 天数 | 占比 |
|:--|:--:|:--:|
...
> **6.3.7.3 诱多识别（操作区域内 柱排×触顶 P≥3%，从低到高）：**
| 柱排×触顶 | n | P(≥3%) | 判断 |
|:--|:--:|:--:|:--|
...
#### 6.12.8 股性分析：持有条件比例是否定义股票性格（2026-09-08 验证）：> **课题（用户提出，2026-09-08）：** 能否用持有条件比例定义股性（股票性格）？高比例=适合持有（有操盘手/游资/机构持有）=好股；低比例=杂毛=完全不适合持有。

##### 6.12.8.1 股性稳定性（关键测试）：| 指标 | 值 |
|:--|:--:|
| 前一半持有比例均值 | 33.9% |
| 后一半持有比例均值 | 35.5% |
| **时间相关性(皮尔逊)** | **-0.156** |
...
##### 6.12.8.2 按持有比例分组：收益与波动（高波池）：| 分组 | 股票数 | P(≥3%) | 均高幅 | 日均涨幅 | 波动(σ) |
|:--|:--:|:--:|:--:|:--:|:--:|
...
##### 6.12.8.3 极端对比（最高10% vs 最低10%）：| 组 | P(≥3%) | 日均涨幅 | 波动 |
|:--|:--:|:--:|:--:|
| 最低10% | 22.6% | 0.0% | 4.5% |
| 最高10% | **24.5%** | **0.1%** | **3.1%** |
...
#### 6.12.9 入管依据验证（旧准则）：DTZA 持续天数（升管/跌管）作为入管信号（2026-09-11 验证）：> **⚠️ 此为准入管依据（DTZA≥3），已被 §5.1.4.1 新准则（触哼入管）替代。新准则见 §5.1.4.1，比较优劣见 §5.1.4.3。**
>
