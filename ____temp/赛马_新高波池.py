# -*- coding: utf-8 -*-
"""
策略赛马 — 新高波池口径，位置+BSHA5+连 组合排名
输出格式：排名 | 位置+条件 | 样本 | H1 | H2 | H3
"""
import csv, os, sys
    ...
    try:
        with open(os.path.join('昭明算展/谕组日', fname), 'r', encoding='gbk', errors='replace') as f:
            ...
            for row in r:
                ...
                # 位置+ZC>0+BSHA5+连+门
                if cond_zc_pos and cond_bsha5 and cond_ly and cond_men:
                    add(f'ZC>0+BSHA5+连+门|{cls_name}', nxt_gf, hit1, hit2, hit3)

                # 位置+ZC>0+甲乙己+BSHA5
                if cond_zc_pos and hx in ('甲','乙','己') and cond_bsha5:
                    add(f'ZC>0+甲乙己+BSHA5|{cls_name}', nxt_gf, hit1, hit2, hit3)

                # 位置+ZC>0+甲乙己+BSHA5+连
                if cond_zc_pos and hx in ('甲','乙','己') and cond_bsha5 and cond_ly:
                    add(f'ZC>0+甲乙己+BSHA5+连|{cls_name}', nxt_gf, hit1, hit2, hit3)

                # 跨等型汇总
                if cond_bsha5:
                    add('BSHA5|全量', nxt_gf, hit1, hit2, hit3)
                if cond_bsha5 and cond_ly:
                    add('BSHA5+连|全量', nxt_gf, hit1, hit2, hit3)
                if cond_bsha5 and cond_ly and cond_men:
                    add('BSHA5+连+门|全量', nxt_gf, hit1, hit2, hit3)
                if cond_zc_pos and cond_bsha5:
                    add('ZC>0+BSHA5|全量', nxt_gf, hit1, hit2, hit3)
                if cond_zc_pos and cond_bsha5 and cond_ly:
                    add('ZC>0+BSHA5+连|全量', nxt_gf, hit1, hit2, hit3)
                if cond_zc_pos and hx in ('甲','乙','己') and cond_bsha5:
                    add('ZC>0+甲乙己+BSHA5|全量', nxt_gf, hit1, hit2, hit3)
    except Exception:
        pass
...
print()
print(f'=== 策略赛马排名（H2 ≥ {BENCH}%，n≥{MIN_N}）===')
