...
- [四、操盘计划](#四操盘计划)
    ...
    - [4.8 四层漏斗定型操作规则（综合所有分析，2026-09-08 定稿，2026-09-12 统一为四层）](#48-四层漏斗定型操作规则综合所有分析2026-09-08-定稿2026-09-12-统一为四层)
...
| **乙(ZA<0)** | →乙65.7%、→丙31.9%、→丁2.3% | ⚠️ **交叉路口**——转正30.6%/维持34.1%/转丙32.2%，**持仓不退出等待转确定状态（乙ZA>0或丙）再决定，空仓等待选方向**（见3.3.1.2.1） |
...
| ⚠️ **交叉路口** | **乙(ZA<0)** | 转正30.6%/维持34.1%/转丙32.2%，**持仓不退出等待转确定状态（乙ZA>0或丙）再决定，空仓等待选方向**（见3.3.1.2.1） |
...
> 5. **操作规则总纲（与§4.8.4定型操作规则统一）：** 只做 DXCD=上/忐（DXZC>0）→ 只做 甲/乙(ZA>0)/己 三个护型 → 乙(ZA<0)/丙 持仓不退出等待转确定状态、空仓等待选方向 → 戊(ZA>0) 观察等待转己/甲 → 丁/戊(ZA<0) 直接放弃 → 介入时机 DJA丘+日等型（等1最强）+BSHA5。**§2.10特有的入管判定过滤（DTZA=1不考虑入管/DTZA=2看升连跌孕/DTZA=3排除阴阳阴）+ 升管内跌排减仓 + 乙ZA<0/丙+DTZA≤-2跌管退出 + DXZC<0必须退出** 是§4.8.4的补充细化（风险管理层）。
>
...
### 4.8 四层漏斗定型操作规则（综合所有分析，2026-09-08 定稿，2026-09-12 统一为四层）
...
**操作框架 = 四层漏斗（与§4.1核心框架一致），把可操作范围收敛到极小：**
...
> **⚠️ 结构调整（2026-09-12）：** 原"三层漏斗"把DXCD并入第1层大区域，未单独区分"真假正交"。**统一为四层漏斗（与§4.1一致）**——**第1层真假正交（DXCD）**、第2层大区域（DJC丘）、第3层护型过滤、第4层介入时机。§4.8聚焦第3层护型选择的详细评定（见4.8.2），§4.1聚焦第1层真假正交（见4.1）。
