# -*- coding: utf-8 -*-
"""验证区分乙(DXZA)/戊(DXZA)的次日高幅阈值概率
乙拆乙(ZA>0)/乙(ZA<0)，戊拆戊(ZA>0)/戊(ZA<0)
"""
import csv, io, os
...
# stats[hx] = [总数, ≥0, ≥1, ≥2, ≥3]
stats = {}
for hx in ['甲', '乙(ZA>0)', '乙(ZA<0)', '己', '丙', '戊(ZA>0)', '戊(ZA<0)', '丁']:
    stats[hx] = [0, 0, 0, 0, 0]
        ...
        with io.open(path, 'r', encoding='gbk') as f:
            ...
            for row in reader:
                ...
                try:
                    ...
                    continue
                rows.append((hx, za, zc, row[COL_GAOFU]))
            for i in range(len(rows) - 1):
                ...
                # 拆分乙/戊
                if hx == '乙':
                    hx = '乙(ZA>0)' if za > 0 else '乙(ZA<0)'
                elif hx == '戊':
                    hx = '戊(ZA>0)' if za > 0 else '戊(ZA<0)'
                if hx not in stats:
                    continue
                try:
...
print('\n=== 各护型 DXZC>0 次日高幅阈值概率 ===')
print(f'{"护型":<12}{"总数":<12}{"≥0%":<10}{"≥1%":<10}{"≥2%":<10}{"≥3%":<10}')
for hx in ['甲', '乙(ZA>0)', '乙(ZA<0)', '己', '丙', '戊(ZA>0)', '戊(ZA<0)', '丁']:
    t, g0, g1, g2, g3 = stats[hx]
    if t:
        print(f'{hx:<12}{t:<12}{g0/t*100:<10.1f}{g1/t*100:<10.1f}{g2/t*100:<10.1f}{g3/t*100:<10.1f}')
    else:
        print(f'{hx:<12} 无样本')