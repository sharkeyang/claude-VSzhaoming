# -*- coding: utf-8 -*-
"""Q4修正4：用柱排字符串形态字母尾部判断单柱形态，匹配用户3种组合
单柱形态 = 柱排字符串形态字母的最后一个字母：
  Q=升连 O=升吞 o=升孕 W=跌连 V=跌吞 v=跌孕
用户组合（三根单柱形态）：
...
for i, f in enumerate(files):
    ...
    df['形态尾'] = df['柱排'].apply(last_form_letter)
    ...
    for j in range(n-2):
        ...
        t0, t1, t2 = fl[j], fl[j+1], fl[j+2]
        for k, pat in targets.items():
            if t0==pat[0] and t1==pat[1] and t2==pat[2]:
                a = agg[k]
...
print('\n' + '='*70)
print('Q4修正4: 用户3种阴阳阴形态组合（柱排形态尾字母）')
print('='*70)
