# -*- coding: utf-8 -*-
"""
周级别 WXZC+WXAB 全维度分析.py
==============================
在周级别上重跑日类 DXZC+DXAB 的所有核心分析：
1. 护型排序（WXZC>0 和 WXZC≤0）
2. 护型转移概率
3. 护型×WXCD协同
4. 护型持续天数
5. 护型×柱排
...
for i, f in enumerate(files):
    ...
    for j in range(len(df)):
        ...
        if hu == '?': continue
        zc_gt0 = zc[j] > 0
        key = (hu, 'ZC>0' if zc_gt0 else 'ZC≤0')
        agg_1[key]['n'] += 1
...
print(f'{"护型":<6s} {"上":>12s} {"中":>12s} {"下":>12s} {"忐":>12s} {"忠":>12s} {"忑":>12s}')
print('-'*80)
for hu in ['甲','乙','丙','丁','戊','己']:
    parts = []
...
for i, f in enumerate(files):
    ...
    for j in range(len(df)):
        hu = get_wxab_hu(wxab[j])
        zp = zhupai[j] if not pd.isna(zhupai[j]) else '?'
        if hu == '?': continue
        key = (hu, zp)
        agg_5[key]['n'] += 1
