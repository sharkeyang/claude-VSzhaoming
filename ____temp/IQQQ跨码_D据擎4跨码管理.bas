...
'========================================================================================
    ...
    WSTO.Range(WSTO.Cells(1, 1), WSTO.Cells(1, 10)).Merge
    末行 = 2
...
'========================================================================================
    ...
    '--- 概览/持仓构成/大盘仓位数字格式 ---
    For X = 5 To 28
        With WSTO.Rows(X): .HorizontalAlignment = xlRight: End With
    Next
'========================================================================================
'输出：大盘仓位限制
'========================================================================================
    ...
    If 指正分 > 指负分 Then 大盘判断 = "向好": 总上限 = 1
    If 指正分 = 指负分 Then 大盘判断 = "震荡": 总上限 = 0.66
    If 指正分 < 指负分 Then 大盘判断 = "向差": 总上限 = 0.33
    '--- 输出 ---
    大盘行 = 20
    ...
    Dim 福月基比 As Double: 福月基比 = IIf(福仓额 > 0, 福月基额 / 福仓额, 0)
    Dim 福周冲比 As Double: 福周冲比 = IIf(福仓额 > 0, 福周冲额 / 福仓额, 0)
    Dim 福日冲比 As Double: 福日冲比 = IIf(福仓额 > 0, 福日冲额 / 福仓额, 0)
    大盘行 = 22
    ...
    WSTO.Cells(大盘行, 7).Value = IIf(福月基比 >= 0.2, "OK", "!低")
    WSTO.Cells(大盘行, 7).Font.Color = IIf(福月基比 >= 0.2, 常色主绿, 常色主红)
    大盘行 = 24
    WSTO.Cells(大盘行, 1).Value = "  ├─周冲(<=40%)"
    WSTO.Cells(大盘行, 3).Value = Round(福周冲额, 1) & "(" & Format(福周冲比, "0%") & ")"
    WSTO.Cells(大盘行, 7).Value = IIf(福周冲比 <= 0.4, "OK", "!超")
    WSTO.Cells(大盘行, 7).Font.Color = IIf(福周冲比 <= 0.4, 常色主绿, 常色主红)
    大盘行 = 25
    WSTO.Cells(大盘行, 1).Value = "  └─日冲(<=40%)"
    WSTO.Cells(大盘行, 3).Value = Round(福日冲额, 1) & "(" & Format(福日冲比, "0%") & ")"
    WSTO.Cells(大盘行, 7).Value = IIf(福日冲比 <= 0.4, "OK", "!超")
    WSTO.Cells(大盘行, 7).Font.Color = IIf(福日冲比 <= 0.4, 常色主绿, 常色主红)
    '彦账户
    Dim 彦上限 As Double: 彦上限 = 彦总资 * 总上限
    Dim 彦月基比 As Double: 彦月基比 = IIf(彦仓额 > 0, 彦月基额 / 彦仓额, 0)
    Dim 彦周冲比 As Double: 彦周冲比 = IIf(彦仓额 > 0, 彦周冲额 / 彦仓额, 0)
    Dim 彦日冲比 As Double: 彦日冲比 = IIf(彦仓额 > 0, 彦日冲额 / 彦仓额, 0)
    大盘行 = 26
    ...
    WSTO.Cells(大盘行, 7).Value = IIf(彦月基比 >= 0.5, "OK", "!低")
    WSTO.Cells(大盘行, 7).Font.Color = IIf(彦月基比 >= 0.5, 常色主绿, 常色主红)
    大盘行 = 28
    WSTO.Cells(大盘行, 1).Value = "  └─周冲(<=50%)"
    WSTO.Cells(大盘行, 5).Value = Round(彦周冲额, 1) & "(" & Format(彦周冲比, "0%") & ")"
    WSTO.Cells(大盘行, 7).Value = IIf(彦周冲比 <= 0.5, "OK", "!超")
    WSTO.Cells(大盘行, 7).Font.Color = IIf(彦周冲比 <= 0.5, 常色主绿, 常色主红)
    '--- 右对齐 ---
    For X = 21 To 28
        With WSTO.Rows(X): .HorizontalAlignment = xlRight: End With
    Next
...
'========================================================================================
    ...
    WSTO.Cells(末行, 7).Value = "月基带周"
    WSTO.Cells(末行, 8).Value = "月基策略"
    ...
    For Each 行业 In 行业典集.Keys
        ...
        If 行业指数典集.Exists(行业) Then
            ...
            If 典指位.Exists(指数信息(1)) Then
                行号指 = 典指位(指数信息(1))
                WSTO.Cells(末行, 7).Value = 谕组(行号指, 位谕of月基带周)
                WSTO.Cells(末行, 8).Value = 谕组(行号指, 位谕of月基策略)
    ...
    WSTO.Columns(7).ColumnWidth = 10  '月基带周
    WSTO.Columns(8).ColumnWidth = 10  '月基策略
    WSTO.Columns(9).ColumnWidth = 16 '月基带日
    WSTO.Columns(10).ColumnWidth = 40 '持仓股票
    WSTO.Rows(30).RowHeight = 20  '行业表头行高
'========================================================================================
'输出：二B、板块轮动状态
'========================================================================================
    末行 = 末行 + 1
...
'========================================================================================
    ...
    Next
    '--- 行业分布表头(行30)灰底 ---
    With WSTO.Range(WSTO.Cells(30, 1), WSTO.Cells(30, 10)).Interior
        .Color = 常色九灰
    End With
    '--- 大盘仓位限制数据行(21-28)浅灰间隔 ---
    For X = 21 To 28
        If X Mod 2 = 0 Then
            With WSTO.Range(WSTO.Cells(X, 1), WSTO.Cells(X, 7)).Interior
                .Color = RGB(245, 245, 245)
    ...
    For X = 31 To 末行整
        If X Mod 2 = 0 Then
            With WSTO.Range(WSTO.Cells(X, 1), WSTO.Cells(X, 10)).Interior
                .Color = RGB(242, 248, 255)  '极淡蓝
            End With
        End If
    Next
    '--- 标题行(行1)下边框 ---
    With WSTO.Range(WSTO.Cells(1, 1), WSTO.Cells(1, 10)).Borders(xlEdgeBottom)
        .LineStyle = xlContinuous
        .Weight = xlMedium
        .ColorIndex = 0  '黑
    End With
    ...
    For Each 节行 In Array(3, 11, 20, 29)
        With WSTO.Range(WSTO.Cells(节行, 1), WSTO.Cells(节行, 10)).Borders(xlEdgeBottom)
            .LineStyle = xlContinuous
            .Weight = xlMedium
            .Color = 常色五靛
        End With
