...
>
> **护型选择（§6.4）：** 只做 甲/乙(ZA>0)/己；乙(ZA<0)/丙 持仓等待方向；戊(ZA>0) 观察；丁/戊(ZA<0) 放弃。
>
> **关键增强信号（§6.5 + §6.1.1）：** DJA丘等1（31.4%）+ BSHA5（39.4%）+ **升连≥2%（92.75%）**——升连必须≥2%，否则无优势（首升连无限制68.84%≈基线，涨幅<2% 52.07%反而更差）。
>
> **阅读指引：** 第一~五章是各维度分析（护型/等型/正交动力学/三态理论），第六章是决策收敛（§6.2 定型操作 + §6.3.1 四类区域 + §6.1 操作理念），第七~八章是跨周期验证 + 数据规范。**核心决策在第六章，前面各章是数据支撑。**
...
- [六、操盘计划](#六操盘计划)
    - [6.1 指导思想：用户操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）](#61-指导思想用户操作理念沃土甲乙己不破djb升管垒升剔诱多)
        - [6.1.1 操作逻辑明确化：沃土 + DXZC>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）](#611-操作逻辑明确化沃土-dxzc0甲乙己-优选升连2-不破djb不出2026-09-16-定稿)
    - [6.2 三层漏斗总览（核心框架）](#62-三层漏斗总览核心框架)
    - [6.3 第1层：DXCD 选择（上/忐 首选，忠 谨慎，中/下/忑 放弃）](#63-第1层dxcd-选择上忐-首选忠-谨慎中下忑-放弃)
        - [6.3.1 四类区域：数学基础 + 全集划分](#631-四类区域数学基础-全集划分)
            - [6.3.1.1 四类区域全集划分：禁区/执念/鸡肋/主战场（2026-09-16 定稿）](#6311-四类区域全集划分禁区执念鸡肋主战场2026-09-16-定稿)
    - [6.4 第2层：DXZC>0 区域护型选择——定型操作的核心层](#64-第2层dxzc0-区域护型选择定型操作的核心层)
    - [6.5 第3层：介入时机（DJA丘 + 日等型 + BSHA5）](#65-第3层介入时机dja丘-日等型-bsha5)
    - [6.6 定型操作规则（最终收敛）](#66-定型操作规则最终收敛)
        - [6.6.1 护型操作决策流程图（可视化总结，2026-09-10，2026-09-11 补全DXCD分支）：](#661-护型操作决策流程图可视化总结2026-09-102026-09-11-补全dxcd分支)
        - [6.6.2 流程图遍历验证（随机10个代码，2026-09-11）：](#662-流程图遍历验证随机10个代码2026-09-11)
    - [6.7 定型操作期望收益（2026-09-08 验证，BSHA 用列19修正后）：](#67-定型操作期望收益2026-09-08-验证bsha-用列19修正后)
    - [6.8 持有条件样本占总天数比例（2026-09-08 验证）：](#68-持有条件样本占总天数比例2026-09-08-验证)
        - [6.8.1 高波池（Qic+Qim+Qit，3451只）：](#681-高波池qicqimqit3451只)
        - [6.8.2 低波池（Qe+Qin+Qd+Qif+Qst，3732只）：](#682-低波池qeqinqdqifqst3732只)
    - [6.9 操作区域"少而精"原则——不要随便退出，阴柱可介入，仅诱多退出（2026-09-08 验证）：](#69-操作区域少而精原则不要随便退出阴柱可介入仅诱多退出2026-09-08-验证)
    - [6.10 股性分析：持有条件比例是否定义股票性格（2026-09-08 验证）：](#610-股性分析持有条件比例是否定义股票性格2026-09-08-验证)
        - [6.10.1 股性稳定性（关键测试）：](#6101-股性稳定性关键测试)
        - [6.10.2 按持有比例分组：收益与波动（高波池）：](#6102-按持有比例分组收益与波动高波池)
        - [6.10.3 极端对比（最高10% vs 最低10%）：](#6103-极端对比最高10-vs-最低10)
    - [6.11 入管依据验证（旧准则）：DTZA 持续天数（升管/跌管）作为入管信号（2026-09-11 验证）：](#611-入管依据验证旧准则dtza-持续天数升管跌管作为入管信号2026-09-11-验证)
...
> **🔗 本章结论 → 第六章入口：** DJC丘生命周期（甲启动→乙主升→丙预警）是第六章"介入时机"的底层依据——**DJC丘首/初介入选甲/己（§6.5 等1 31.4%），主升持有乙，顶部/尾部 DXZA>0 与 DXZB>0 双降警惕退出（§6.6 规则6）**。DJC丘（DXZC>0）是四类区域"主战场"的前提（§6.3.1）。
...
> - 己 DXZC>0 时升连占比 **66.1%**（与 §6.6.1 的 66.8% 一致）——**不是"都是升连"**，但升系（升连+升孕+升吞）合计 **82.9%**，绝大多数是升系柱排。
...
> - 己/甲 DXZC>0 时，次日冲高率约 21-23%，与基线（23.6%）接近——**护型本身不保证次日冲高**，需结合介入时机（DJA丘等1/2/3 + BSHA5，§6.5）和升连≥2%（§6.1.1）提升胜率。
...
> **🔗 本章结论 → 第六章入口：** 护型分析（甲/乙ZA>0 可做、乙ZA<0 交叉路口、丙持仓或退出、戊ZA>0 观察、丁/戊ZA<0 放弃）是第六章"护型选择"（§6.4）的直接数据支撑——**只做 甲/乙(ZA>0)/己，其余按角色处理**。DXZC>0 限定（上/忐/忠）对应四类区域"主战场"（§6.3.1）。
...
> **🔗 本章结论 → 第六章入口：** 等型分析（等1 介入点 31.4%、等2 30.3%、等3 持有期 29.5%）是第六章"介入时机"（§6.5）的直接数据支撑——**DJA丘等1/2/3 + BSHA5 触发器**。等6/等7（下跌通道）非介入点，应回避。
...
> **🔗 本章结论 → 第六章入口：** DXAB 正交动力学（转负概率、持续时间、快速负交）是第六章"护型选择 + 退出信号"的数据支撑——**DXAB>0 正交是护型可做的前提（§6.4），快速负交（DXCD中/下/忑）是退出预警（§6.6 规则）**。突破日护型坡度（甲最强）辅助介入时机选择。
...
> 5. **操作规则总纲（与§6.6定型操作规则统一）：** 只做 DXCD=上/忐（DXZC>0）→ 只做 甲/乙(ZA>0)/己 三个护型 → 乙(ZA<0)/丙 持仓不退出等待转确定状态、空仓等待选方向 → 戊(ZA>0) 观察等待转己/甲 → 丁/戊(ZA<0) 直接放弃 → 介入时机 DJA丘+日等型（等1最强）+BSHA5。**§5.2特有的入管判定过滤（DTZA=1不考虑入管/DTZA=2看升连跌孕/DTZA=3排除阴阳阴）+ 升管内跌排减仓 + 乙ZA<0/丙+DTZA≤-2跌管退出 + DXZC<0必须退出** 是§6.6的补充细化（风险管理层）。
>
...
> **来源：** 原 C6 §6.6.1.7 ⑪ 垒升专题（2026-09-15 移入）。**数据：甲乙DXZA>0己，DXZC>0，全量7455文件。**
...
> **🔗 本章结论 → 第六章入口：** 柱与DJA三态理论 + 操作规则优化是第六章"操作理念"（§6.1）的直接数据支撑——**升管（触哼入管，§5.1.4.1）、垒升（柱体≥2%，§5.3）、剔诱多（跌连-非触顶，§6.9.3）、不破DJB不出（DXZB退出规则，§5.2.9）** 正是 §6.1 操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）的微观层。**本章是第六章操作理念的"微观细节库"。**
...
>
> **第1步 · 选区（§6.3.1 四类区域，DXZE×CD 二维）：** 判断当前处于哪个区域——**主战场（DXZE>0+CD上/忐/忠）/ 鸡肋（DXZE<0+CD上/忐/忠）/ 执念（(CD中)+(DXZE>0+CD下/忑)）/ 禁区（DXZE<0+CD下/忑）**。只在**主战场**重仓，鸡肋可持有不宜过早介入，执念观察，禁区坚决不碰。
>
> **第2步 · 选点（§6.2 三层漏斗，DXCD→护型→介入时机）：** 在选定的区域内，用三层漏斗选具体介入点——**第1层 DXCD（上/忐 首选，忠 谨慎）、第2层 护型（甲乙己）、第3层 介入时机（DJA丘等1/2/3 + BSHA5，叠加微观升管/垒升/剔诱多）**。
>
> **关系：** 四类区域（§6.3.1）是**顶层选区**（决定"做不做、重仓还是轻仓"），三层漏斗（§6.2）是**底层选点**（决定"具体买哪个、何时买"）。**先选区（大势/土质），再选点（护型/时机/微观）**——不要在恶土（禁区/执念）里谈微观，这正是 §6.1 操作理念"抓大看小"的核心。

### 6.1 指导思想：用户操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）
...
| 层 | 理念 | 对应已验证结论 | 数据 |
|:--|:--|:--|:--|
| ① 沃土 | DXZE>0 + DXCD上/忐 | §6.3.1 主战场 | +4.98pp（23.6%→28.6%）|
| ② 甲乙己 | 只做甲乙己护型 | §6.4 | P≥3% 27-30%，唯一可做三护型 |
...
| ⑤ 剔诱多 | 跌连-非触顶 | §6.9.3 | 24.9% 最明确诱多，退出 |
...
> **⚠️ 唯一需注意的取舍：** 沃土（35.6%）→ 甲乙己 → 升管 → 垒升 → 剔诱多，可操作范围收敛到极小，**交易频率低（§6.7 每年仅 10-14 次/股），但每次胜率高**。这是"少而精"的代价，需接受。

> **一句话：** 此理念 = §5.2.9"抓大看小"五层决策框架的完整落地——**先大势（DXZE>0）→ 沃土（DXCD上/忐）→ 主升区（DXZC>0）→ 护型（甲乙己，不破DJB不出）→ 微观（升管/垒升/剔诱多）**，每层都已有全量数据支撑，非新理念。

#### 6.1.1 操作逻辑明确化：沃土 + DXZC>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）
...
| + 沃土（DXZE>0+CD上/忐） | ~73% | +4.98pp | §6.3.1 主战场 |
...
>
> **⚠️ 注意：** 沃土+甲乙己把样本收敛到极小（§6.7 每年仅 10-14 次/股），再叠加"升连涨幅≥2%"（样本56万），可操作频率进一步降低——但每次胜率极高（92.75%）。这是"少而精"的极致形态，需接受低交易频率换取高胜率。
...
> [!IMPORTANT]
> **三层漏斗总纲（原§6.3 三层漏斗定型操作规则，并入6.1）：** 三层漏斗把可操作范围收敛到极小，与上方核心框架表一致。
...
**操作框架 = 三层漏斗（与§6.2核心框架一致），把可操作范围收敛到极小：**
...
> **⚠️ 三层漏斗（2026-09-18 统一）：** 原"四层漏斗"把真假正交（DXCD）与大区域（DJC丘DXZC>0）拆成两层，但两者是同一维度（DXCD隐含DXZC），合并为**第1层 DXCD选择（选区域）**。三层漏斗 = **第1层DXCD选择（选区域）→ 第2层护型选择（选护型）→ 第3层介入时机（选时机）**。§6.4聚焦第2层护型选择的详细评定（见4.8.2），§6.3聚焦第1层DXCD选择（见4.1）。


### 6.3 第1层：DXCD 选择（上/忐 首选，忠 谨慎，中/下/忑 放弃）
...
> **数据支撑（详细见 §6.2 核心框架三层漏斗 + §2.9.3 按DXCD拆分）：** DXZC>0（上/忐/忠）时甲/乙ZA>0 主导且持久；DXZC<0（中/下/忑）时丁/戊ZA<0 主导，甲/乙ZA>0 几乎消失（乙ZA>0 仅0.03-0.7%）。**限定 DXZC>0 是有意义的**（见 §2.9.3）。
>
> **⚠️ 忠的归属（2026-09-16 统一）：** 忠 归**主战场**（DXZE>0）/ **鸡肋**（DXZE<0），遵循 VBA 仓日类定义（忠 ∈ {上,忐,忠}，见 §6.3.1）。但忠 = DXZC>0 且 DXZD<0（价格在 DJD=EMA60 之下），**上升受限、升连弱（20-22%）**——操作时需谨慎，**必须叠加"升连≥2%"（§5.3.9）才介入**，否则忠的升连不提升盈利性。上/忐 仍是首选（升连强、持久），忠 是次选（可做但需谨慎）。

#### 6.3.1 四类区域：数学基础 + 全集划分
...
> **结论：** 四类区域的数学基础 = **均线系统几何结构**。方向由长均线（EMA60）决定（P>EMA60=升势，P<EMA60=跌势），待定状态（中偏升/忠偏跌）由"跌破/站上短均线但长均线方向相反"决定，强度由均线排列（多头/空头）决定。**DXAB 在各区域内做优选，不改变区域划分。** 此数学框架是 §6.3.1 概率划分的理论基石。


##### 6.3.1.1 四类区域全集划分：禁区/执念/鸡肋/主战场（2026-09-16 定稿）
...
### 6.4 第2层：DXZC>0 区域护型选择——定型操作的核心层
> [!IMPORTANT]
...
#### 6.6.1 护型操作决策流程图（可视化总结，2026-09-10，2026-09-11 补全DXCD分支）：
> **说明：** 下图将 §6.2 定型操作框架的完整决策逻辑可视化为流程图——先按 DXCD 判断区域，DXZC>0（上/忐）区域内按护型判断操作，可做的护型进入介入时机判断，持有的护型监控退出信号。**（若在支持 Mermaid 的查看器中打开可交互渲染；也可打开 `____temp/护型流程图.html` 查看 HTML 版。）**
...
>
> **己的升连比例（坡度特征）：** 己在 DXZC>0 时**升连比例最高（66.8%）**——己一旦出现几乎都是连续阳线（坡度陡），印证"己=冲高后急跌"的直觉。但己占区域内升连仅7.5%（"陡但少"），出现频率低，需快进快出（见 §6.4 第2层护型选择）。

#### 6.6.2 流程图遍历验证（随机10个代码，2026-09-11）：
...
> - **DXCD=忠（DXZC>0 但 DXZD<0）未覆盖 7.7%**——**已补全操作建议：谨慎介入**（归主战场/鸡肋，遵循 VBA 定义，见 §6.3.1）。忠虽是 DXZC>0，但 DXZD<0 意味着价格在 DJD(EMA60) 之下，上升受限、升连弱——**需叠加升连≥2%（§5.3.9）才介入**，否则忠的升连不提升盈利性。
>
> **✅ 补全后流程图覆盖：** DXCD=上/忐（约40%）按护型操作；DXCD=忠（约8%）谨慎介入（需叠加升连≥2%）；DXCD=中/下/忑（约52%）明确回避不属于操作范围。**每个柱都能找到对应操作指南（或明确的不操作指令）。**

### 6.7 定型操作期望收益（2026-09-08 验证，BSHA 用列19修正后）：
...
#### 6.8.2 低波池（Qe+Qin+Qd+Qif+Qst，3732只）：
...
### 6.9 操作区域"少而精"原则——不要随便退出，阴柱可介入，仅诱多退出（2026-09-08 验证）：
...
### 6.10 股性分析：持有条件比例是否定义股票性格（2026-09-08 验证）：
...
### 6.11 入管依据验证（旧准则）：DTZA 持续天数（升管/跌管）作为入管信号（2026-09-11 验证）：
...
> [!IMPORTANT]
> **升连位置护型发现（2026-09-08，见2.4.2.10）：** DXZC>0 时升连位置，甲（47.7%）最多、乙（41.0%）次之。**但按 DXCD 分化：DXCD=上 时乙反而最多（48.4% > 甲38.3%），且甲/乙都强（33%+）可介入；DXCD=忐 时甲更强（27.6%）；DXCD=忠 时升连弱（20-22%），不可靠。** 升连介入优先选 DXCD=上（甲/乙都强）；DXCD=忠 谨慎（归主战场/鸡肋，但升连弱，需叠加升连≥2%才介入，见 §6.3.1）。
