...
old = '末行 = 5' + NL + '    WSTO.Cells(末行, 1).Value = "总资产(千元)"'
new = 'Dim 概览始 As Integer: 概览始 = 末行 + 1' + NL + old
assert old in text, 'Phase 1.1: old not found!'
...
for n in range(5, 17):
    ...
    if old in text:
        ...
        print(f'WARN: 末行={n} pattern not found, trying alternate...')
        # 尝试不带缩进
        old = '末行 = ' + str(n) + NL
        new = '末行 = 末行 + 1' + NL
        assert old in text, f'Phase 1.2: 末行={n} not found!'
        text = text.replace(old, new, 1)
...
assert old in text, 'Phase 2.1: not found!'
text = text.replace(old, new, 1)
...
for old, new in repls:
    ...
    if count == 0:
        print(f'WARN: Phase 2.2: "{old}" not found, count={count}')
    else:
        text = text.replace(old, new, 1)

# 2.3 替换第一个 大盘行=19（变量声明区）→ 大盘行 = 大盘始 + 1
old = '    大盘行 = 19' + NL + '    \'福账户'
new = '    大盘行 = 大盘始 + 1' + NL + '    \'福账户'
if old in text:
    text = text.replace(old, new, 1)
else:
    print('WARN: 大盘行=19 + 福账户 comment not found, trying WSTO.Cells pattern...')
    old = '    大盘行 = 19' + NL + '    WSTO.Cells(大盘行, 1).Value = "福账户限额"'
    new = '    大盘行 = 大盘行 + 1' + NL + '    WSTO.Cells(大盘行, 1).Value = "福账户限额"'
    assert old in text, 'Phase 2.3: 大盘行=19 not found!'
    text = text.replace(old, new, 1)
...
for n in range(20, 29):
    ...
    if old in text:
        ...
        print(f'WARN: 大盘行={n} pattern not found')
        # 尝试不带后续缩进
        old = '    大盘行 = ' + str(n) + NL
        new = '    大盘行 = 大盘行 + 1' + NL
        assert old in text, f'Phase 2.3b: 大盘行={n} not found!'
        text = text.replace(old, new, 1)
...
assert old in text, 'Phase 3.1: not found!'
text = text.replace(old, new, 1)

# 3.2 替换 末行=33 → 末行=末行+1
old = '    末行 = 33' + NL + '    WSTO.Cells(末行, 1).Value = "行业"'
new = '    末行 = 末行 + 1' + NL + '    WSTO.Cells(末行, 1).Value = "行业"'
assert old in text, 'Phase 3.2: not found!'
text = text.replace(old, new, 1)

# 3.3 替换 末行=34 → 末行=末行+1
old = '    末行 = 34' + NL + '    For Each 行业 In 行业典集.Keys'
new = '    末行 = 末行 + 1' + NL + '    For Each 行业 In 行业典集.Keys'
assert old in text, 'Phase 3.3: not found!'
text = text.replace(old, new, 1)
