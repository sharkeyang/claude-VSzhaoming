...
for i,f in enumerate(files):
    ...
    try: df = pd.read_csv(f, encoding='gbk', usecols=['高幅','DXAB','柱排','波型','盈提示','顶型','日等型'])
    except: continue
    ...
    for j in range(n-1):
        ...
        if '甲' in dxab[j] and '甲' not in dxab[j+1]:
            agg_总.add(nhr)
...
print(f'甲->非甲 总样本: {agg_总.n}, P3={agg_总.p3/agg_总.n*100:.2f}%')
