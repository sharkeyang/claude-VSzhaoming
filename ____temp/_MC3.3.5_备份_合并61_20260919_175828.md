...
- [六、操盘计划](#六操盘计划)
    - [6.1 三层漏斗总览（核心框架）](#61-三层漏斗总览核心框架)
        - [6.1.1 指导思想：用户操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）](#611-指导思想用户操作理念沃土甲乙己不破djb升管垒升剔诱多)
            - [6.1.1.1 抓大看小：决策顺序 + DXZB 退出规则（原§5.2.9，2026-09-15 全量验证）](#6111-抓大看小决策顺序-dxzb-退出规则原5292026-09-15-全量验证)
        - [6.1.2 三层漏斗操作规则（原三层漏斗总纲）](#612-三层漏斗操作规则原三层漏斗总纲)
    ...
    - [6.2 第1层：选区（四类区域，DXZE×CD）](#62-第1层选区四类区域dxzecd)
        - [6.2.1 四类区域：数学基础 + 全集划分](#621-四类区域数学基础-全集划分)
            - [6.2.1.1 待定状态（一条均线之上一条之下）的方向倾向](#6211-待定状态一条均线之上一条之下的方向倾向)
...
> **🔗 本章结论 → 第六章入口：** 柱与DJA三态理论 + 操作规则优化是第六章"操作理念"（§6.1.1）的直接数据支撑——**升管（触哼入管，§5.1.4.1）、垒升（柱体≥2%，§5.3）、剔诱多（跌连-非触顶，§6.12.7）、不破DJB不出（DXZB退出规则，§6.1.1.1）** 正是 §6.1.1 操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）的微观层。**本章是第六章操作理念的"微观细节库"。**
...
> **三层漏斗 vs 五层理念（§6.1.1.1 抓大看小）的关系：** 两套框架互补，不冲突。
> - **三层漏斗**（第1层选区→第2层护型选择→第3层介入时机）= 操盘计划的**选点框架**（决定具体买哪个、何时买）。
> - **五层理念**（§6.1.1.1 抓大看小：大势→沃土→主升区→护型→微观）= **决策顺序**（先大势再微观，抓大看小）。
> - **关系：** 五层理念是"抓大看小"的完整决策链条（从大势到微观），三层漏斗是"选点"的具体框架（选区域→选护型→选时机）。**先按五层理念抓大看小（大势→沃土→主升区），再按三层漏斗选点（护型→时机）**。详见 §6.1.1.1。

#### 6.1.1 指导思想：用户操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）


> **课题（用户提出，2026-09-16）：** 在沃土，寻找 DXAB（甲乙己，不破DJB不出）+ 在升管 + 垒升，剔除诱多。**逐层评论确认：此理念 = §6.1.1.1"抓大看小"五层决策框架的完整落地，每一层都已有全量数据支撑，非新理念，而是对已验证结论的有机整合。**

**理念五层（与 §6.1.1.1 决策顺序一致，每层已验证）：**

| 层 | 理念 | 对应已验证结论 | 数据 |
|:--|:--|:--|:--|
| ① 沃土 | DXZE>0 + DXCD上/忐 | §6.2.1 主战场 | +4.98pp（23.6%→28.6%）|
| ② 甲乙己 | 只做甲乙己护型 | §6.3 | P≥3% 27-30%，唯一可做三护型 |
| ②' 不破DJB不出 | 持有底线 = DXZB>0（不破DJB）| **§6.1.1.1 已全量验证** | 四象限 69.96% 坚定持有 / 41.62% 退出 |
| ③ 升管 | 触哼入管（§5.1.4.1 新准则）| §5.1.4.1 | 次日P3 25.14%（vs 旧准则23.00%）|
| ④ 垒升 | 柱体≥2% | §5.3 | 升管内部最强，升孕≥2%获利98.07% |
| ⑤ 剔诱多 | 跌连-非触顶 | §6.12.7 | 24.9% 最明确诱多，退出 |

> **核心：②' 不破DJB不出 = §6.1.1.1 DXZB 退出规则（已全量验证，非待验证）。**
...
> **一句话：** 此理念 = §6.1.1.1"抓大看小"五层决策框架的完整落地——**先大势（DXZE>0）→ 沃土（DXCD上/忐）→ 主升区（DXZC>0）→ 护型（甲乙己，不破DJB不出）→ 微观（升管/垒升/剔诱多）**，每层都已有全量数据支撑，非新理念。

##### 6.1.1.1 抓大看小：决策顺序 + DXZB 退出规则（原§5.2.9，2026-09-15 全量验证）
...
| + 不破DJB持有 | 69.96%（坚定持有象限） | 持有期保护 | §6.1.1.1 防过早退出 |
...
| 忠 | >0 | 57.0%+17.9%=74.9% | 2.7天 | ❌ **舍弃**（DXCD<0 可能多次 DXZC 正交，等变上/忐再介入）|
...
>
> **⚠️ 舍弃忠（用户，2026-09-19）：** 虽然 VBA 定义忠 ∈ {上,忐,忠} 归主战场/鸡肋，但**操作上舍弃忠**——忠虽然 DXZC 转正是介入机会的典型特征，但因为 DXCD<0 可能存在多次 DXZC 正交（假介入机会），所以要**等待 DXCD 变为忐或上 再介入**。**只在已变为上/忐 之后考虑介入，不要在忠 里猜测何时变好。**
...
> **醍醐灌顶（用户，2026-09-19）：不要再在 DXCD 下/中/忑/忠 时猜测何时变上/忐，只在已经变为上/忐 之后再考虑介入。**
>
> **忠/中的方向倾向（看 DXCD 与 DXZC/DXZD 的交叉个数对比）：** 当 DXCD 维持状态时，可能出现多次 DXZC 交叉——**忠 的方向倾向**（DXCD<0 但 DXZC 可能多次转正，是"假介入机会"）。同理可推断**中 的方向倾向**。
>
> **⚠️ 舍弃忠：** 忠 虽然 DXZC 转正是介入机会的典型特征，但因为 DXCD<0 可能存在多次 DXZC 正交，所以要**等待 DXCD 变为忐或上 再介入**。**要舍弃（忠）！**

#### 6.2.1.2 在各区域内考虑 DXAB（不改变区域划分，只做优选）
...
#### 6.2.1.3 四类区域全集划分：禁区/执念/鸡肋/主战场（2026-09-16 定稿）
...
>
> **⚠️ 舍弃忠（用户，2026-09-19）：** 忠 虽然 DXZC 转正是介入机会的典型特征，但因为 DXCD<0 可能存在多次 DXZC 正交（假介入机会），所以**操作上舍弃忠**——**等待 DXCD 变为忐或上 再介入**，不要在忠 里猜测何时变好。
