...
- [六、操盘计划](#六操盘计划)
    ...
    - [6.2 第1层：选区（四类区域，DXZE×CD）](#62-第1层选区四类区域dxzecd)
...
> [!IMPORTANT]
> ## 🎯 第六章使用顺序：一个三层漏斗，第1层就是选区（2026-09-16 澄清）
>
...
| 层 | 变量 | 作用 | 数据支撑 |
|:--|:--|:--|:--|
| **第1层 选区** | DXZE×DXCD 四类区域 | 选区（主战场/鸡肋/执念/禁区）| 主战场重仓，禁区不碰 |
| **第2层 护型选择** | DXAB广义正交 | 选护型（排除下破DJC高发的非广义）| 非广义下破33.6% vs 广义7.5% |
| **第3层 介入时机** | DJA丘（日ZA>0）+ 日等型 | 选时机 | 等1 31.4%最强 |

> **⚠️ 三层漏斗（2026-09-18 统一）：** 原"四层漏斗"（真假正交/大区域/护型过滤/介入时机）中，**真假正交（DXCD）与大区域（DJC丘DXZC>0）是同一维度**（DXCD隐含DXZC），合并为**第1层 选区（四类区域）**。三层漏斗 = **第1层选区（四类区域，DXZE×CD）→ 第2层护型选择（选护型）→ 第3层介入时机（选时机）**。

**操作细则（三层漏斗把可操作范围收敛到极小）：**

```
第1层 选区：四类区域（DXZE×CD），主战场重仓/鸡肋持有/执念观察/禁区不碰
   ↓
...
> - **三层漏斗**（第1层选区→第2层护型选择→第3层介入时机）= 操盘计划的**选点框架**（决定具体买哪个、何时买）。
...
### 6.2 第1层：选区（四类区域，DXZE×CD）

> **第1层选区的核心是四类区域（§6.2.1，DXZE×CD 二维，12格全覆盖）：主战场/鸡肋/执念/禁区。** 只在主战场重仓，鸡肋可持有不宜过早介入，执念观察，禁区坚决不碰。下方 DXCD 选择表是四类区域"土质"维度的数据支撑（上/忐/忠=可做，中/下/忑=不可做，与四类区域一致，**不重复计算**）——DXCD 已隐含在四类区域中，不再作为独立一层。

| DXCD | DXZC | 甲/乙ZA>0 行占比 | 甲持续 | 结论 |
|:--|:--:|:--:|:--:|:--|
| **上** | >0 | 30.0%+35.3%=**65.3%** | 5.8天 | ✅ 首选（多头主导，样本最多）|
| **忐** | >0 | 55.6%+30.7%=**86.3%** | 4.1天 | ✅ 首选（甲/乙ZA>0 占比最高）|
| 忠 | >0 | 57.0%+17.9%=74.9% | 2.7天 | ❌ **舍弃**（DXZD<0 上升受限、升连弱，等变上/忐再介入）|
...
>
> **⚠️ 舍弃忠（用户，2026-09-19）：** 虽然 VBA 定义忠 ∈ {上,忐,忠} 归主战场/鸡肋，但**操作上舍弃忠**——忠虽然单看 DXZC 恒正，但 DXCD<0（忐忠忑）期间 DXZC 反复交叉（97.4%），**忠 的 DXZC 转正是假信号**；且忠 DXZD<0（价格在 EMA60 之下），**上升受限、升连弱（20-22%）、P≥3% 弱**，所以要**等待 DXCD 变为忐或上 再介入**。**只在已变为上/忐 之后考虑介入，不要在忠 里猜测何时变好。**
...
>
> **醍醐灌顶：对于忠，虽然 DXZC 转正是介入机会的典型特征，但因为 DXCD<0 可能存在多次 DXZC 正交，所以要等待 DXCD 变为忐或上 再介入。要舍弃（忠）！**
>
...
>
> **⚠️ 舍弃忠：** 忠 虽然单看 DXZC 恒正，但 DXCD<0（忐忠忑）期间 DXZC 反复交叉（97.4%），**忠 的 DXZC 转正是假信号**；且忠 DXZD<0（价格在 EMA60 之下），**上升受限、升连弱（20-22%）、P≥3% 弱**——所以**操作上舍弃忠**，**等待 DXCD 变为忐或上 再介入**。**要舍弃（忠）！**
...
>
> **⚠️ 舍弃忠（用户，2026-09-19）：** 忠 虽然单看 DXZC 恒正，但 DXCD<0（忐忠忑）期间 DXZC 反复交叉（97.4%），**忠 的 DXZC 转正是假信号**；且忠 DXZD<0（价格在 EMA60 之下），**上升受限、升连弱（20-22%）、P≥3% 弱**，所以**操作上舍弃忠**——**等待 DXCD 变为忐或上 再介入**，不要在忠 里猜测何时变好。
...
> **✅ 操作方法定型：**
> 1. **只做 DXCD=上/忐**（DXZC>0，首选）；**忠 谨慎**（归主战场/鸡肋，但升连弱，需叠加升连≥2%才介入）；排除中/下/忑。
...
flowchart TD
    ...
    Q0 -->|忠 DXZC>0但DXZD<0| ZHONG[忠<br>DXZD<0 谨慎介入]
...
> | **忠** | DXZC>0 但 DXZD<0 | ⚠️ **谨慎介入**（归主战场/鸡肋，但升连弱，需叠加升连≥2%）|
...
> - **DXCD=忠（DXZC>0 但 DXZD<0）未覆盖 7.7%**——**已补全操作建议：谨慎介入**（归主战场/鸡肋，遵循 VBA 定义，见 §6.2.1）。忠虽是 DXZC>0，但 DXZD<0 意味着价格在 DJD(EMA60) 之下，上升受限、升连弱——**需叠加升连≥2%（§5.3.8）才介入**，否则忠的升连不提升盈利性。
>
> **✅ 补全后流程图覆盖：** DXCD=上/忐（约40%）按护型操作；DXCD=忠（约8%）谨慎介入（需叠加升连≥2%）；DXCD=中/下/忑（约52%）明确回避不属于操作范围。**每个柱都能找到对应操作指南（或明确的不操作指令）。**
...
| **第1层 选区（主战场 DXZE>0+上/忐）** | 4,534,155 | 28.6% | 2.4% |
| **第2层 +护型甲乙己** | 3,324,398 | 29.7% | 2.5% |
| **第3层 +DJA丘** | 3,324,398 | 29.7% | 2.5% |
| **+BSHA5 触发器** | 1,190,418 | **42.5%** | **3.3%** |

> **各层递进价值：** 第1层选区（主战场 DXZE>0+上/忐，四类区域"主战场"子集）提升 +4.98pp（23.6%→28.6%），护型甲乙己 +1.09pp，BSHA5 触发器 **+12.79pp**（29.7%→42.5%）。**BSHA5 是最大增量。**
>
...
> - **管宽哼JA 定义（VBA权威）：** 管宽哼JA = (最近5日最高价 / EMA5 − 1) × 100，即**最近5日最高价相对 EMA5 的偏离百分比**。
> - **🔍 为什么能 >60（2026-09-20 全量验证）：** 管宽哼JA >60 主要出现在**北交所（bj）股票**——北交所无10%涨跌停限制（单日可涨30%+），连续大涨后价格远高于 EMA5。全量分布：bj 占比 4.07%（n=116,522），sh 0.23%（n=4,490,606），sz 0.31%（n=5,297,014）。**沪深股票受涨跌停限制，管宽哼JA 很难 >60**。所以扩管>60 的强信号（升连76.49%）主要来自北交所高波动股，需注意市场差异。
...
> [!IMPORTANT]
> **升连位置护型发现（2026-09-08，见2.4.2.10）：** DXZC>0 时升连位置，甲（47.7%）最多、乙（41.0%）次之。**但按 DXCD 分化：DXCD=上 时乙反而最多（48.4% > 甲38.3%），且甲/乙都强（33%+）可介入；DXCD=忐 时甲更强（27.6%）；DXCD=忠 时升连弱（20-22%），不可靠。** 升连介入优先选 DXCD=上（甲/乙都强）；DXCD=忠 谨慎（归主战场/鸡肋，但升连弱，需叠加升连≥2%才介入，见 §6.2.1）。
