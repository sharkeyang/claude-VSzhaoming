...
#### 6.8.1 第1层：DXCD 选择（上/忐 首选，忠 谨慎，中/下/忑 放弃）
...
>
> **⚠️ 忠的归属（2026-09-16 统一）：** 忠 归**主战场**（DXZE>0）/ **鸡肋**（DXZE<0），遵循 VBA 仓日类定义（忠 ∈ {上,忐,忠}，见 §6.8.11）。但忠 = DXZC>0 且 DXZD<0（价格在 DJD=EMA60 之下），**上升受限、升连弱（20-22%）**——操作时需谨慎，**必须叠加"升连≥2%"（§5.3.9）才介入**，否则忠的升连不提升盈利性。上/忐 仍是首选（升连强、持久），忠 是次选（可做但需谨慎）。
...
> - **DXCD=忠（DXZC>0 但 DXZD<0）未覆盖 7.7%**——**已补全操作建议：谨慎介入**（归主战场/鸡肋，遵循 VBA 定义，见 §6.8.11）。忠虽是 DXZC>0，但 DXZD<0 意味着价格在 DJD(EMA60) 之下，上升受限、升连弱——**需叠加升连≥2%（§5.3.9）才介入**，否则忠的升连不提升盈利性。
>
...
#### 6.8.9 入管依据验证（旧准则）：DTZA 持续天数（升管/跌管）作为入管信号（2026-09-11 验证）：
...
#### 6.8.10 四类区域的数学理论基础：均线系统几何结构（2026-09-17 定稿）
...
> **结论：** 四类区域的数学基础 = **均线系统几何结构**。方向由长均线（EMA60）决定（P>EMA60=升势，P<EMA60=跌势），待定状态（中偏升/忠偏跌）由"跌破/站上短均线但长均线方向相反"决定，强度由均线排列（多头/空头）决定。**DXAB 在各区域内做优选，不改变区域划分。** 此数学框架是 §6.8.11 概率划分的理论基石。

#### 6.8.11 四类区域全集划分：禁区/执念/鸡肋/主战场（2026-09-16 定稿）
...
#### 6.8.12 用户操作理念：沃土→甲乙己不破DJB→升管→垒升→剔诱多（2026-09-16 定稿）
...
| 层 | 理念 | 对应已验证结论 | 数据 |
|:--|:--|:--|:--|
| ① 沃土 | DXZE>0 + DXCD上/忐 | §6.8.11 主战场 | +4.98pp（23.6%→28.6%）|
...
##### 6.8.12.1 操作逻辑明确化：沃土 + DXZC>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）
...
| + 沃土（DXZE>0+CD上/忐） | ~73% | +4.98pp | §6.8.11 主战场 |
...
>
> **⚠️ 注意：** 沃土+甲乙己把样本收敛到极小（§6.8.5 每年仅 10-14 次/股），再叠加"升连涨幅≥2%"（样本56万），可操作频率进一步降低——但每次胜率极高（92.75%）。这是"少而精"的极致形态，需接受低交易频率换取高胜率。
