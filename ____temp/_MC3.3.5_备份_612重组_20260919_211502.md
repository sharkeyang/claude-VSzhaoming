...
- [六、操盘计划](#六操盘计划)
    ...
    - [6.12 旧内容：操盘决策速查（介入点/退出/流程/印证/收益）](#612-旧内容操盘决策速查介入点退出流程印证收益)
        ...
        - [6.12.5 定型操作期望收益（2026-09-08 验证，BSHA 用列19修正后）：](#6125-定型操作期望收益2026-09-08-验证bsha-用列19修正后)
        - [6.12.6 持有条件样本占总天数比例（2026-09-08 验证）：](#6126-持有条件样本占总天数比例2026-09-08-验证)
            - [6.12.6.1 高波池（Qic+Qim+Qit，3451只）：](#61261-高波池qicqimqit3451只)
            - [6.12.6.2 低波池（Qe+Qin+Qd+Qif+Qst，3732只）：](#61262-低波池qeqinqdqifqst3732只)
        - [6.12.7 操作区域"少而精"原则——不要随便退出，阴柱可介入，仅诱多退出（2026-09-08 验证）：](#6127-操作区域少而精原则不要随便退出阴柱可介入仅诱多退出2026-09-08-验证)
        - [6.12.8 股性分析：持有条件比例是否定义股票性格（2026-09-08 验证）：](#6128-股性分析持有条件比例是否定义股票性格2026-09-08-验证)
            - [6.12.8.1 股性稳定性（关键测试）：](#61281-股性稳定性关键测试)
            - [6.12.8.2 按持有比例分组：收益与波动（高波池）：](#61282-按持有比例分组收益与波动高波池)
            - [6.12.8.3 极端对比（最高10% vs 最低10%）：](#61283-极端对比最高10-vs-最低10)
        - [6.12.9 入管依据验证（旧准则）：DTZA 持续天数（升管/跌管）作为入管信号（2026-09-11 验证）：](#6129-入管依据验证旧准则dtza-持续天数升管跌管作为入管信号2026-09-11-验证)
...
> **🔗 本章结论 → 第六章入口：** 柱与DJA三态理论 + 操作规则优化是第六章"操作理念"（§6.1.1）的直接数据支撑——**升管（触哼入管，§5.1.4.1）、垒升（柱体≥2%，§5.3）、剔诱多（跌连-非触顶，§6.12.7）、不破DJB不出（DXZB退出规则，§6.1.1）** 正是 §6.1.1 操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）的微观层。**本章是第六章操作理念的"微观细节库"。**
...
#### 6.12.5 定型操作期望收益（2026-09-08 验证，BSHA 用列19修正后）：
...
#### 6.12.7 操作区域"少而精"原则——不要随便退出，阴柱可介入，仅诱多退出（2026-09-08 验证）：
