...
> | 区域 | 条件 | 操作 |
> |:--|:--|:--|
> | **主战场** | DXZE>0 + CD上/忐/忠（忠需谨慎）| 唯一重仓区 |
> | **鸡肋** | DXZE<0 + CD上/忐/忠 | 可持有，不宜过早介入 |
> | **执念** | (CD中) + (DXZE>0 + CD下/忑) | 观察不介入 |
> | **禁区** | DXZE<0 + CD下/忑 | 坚决不介入 |
>
> **护型选择（§6.3）：** 只做 甲/乙(ZA>0)/己；乙(ZA<0)/丙 持仓等待方向；戊(ZA>0) 观察；丁/戊(ZA<0) 放弃。
>
> **关键增强信号（§6.4 + §6.1.1）：** DJA丘等1（31.4%）+ BSHA5（39.4%）+ **升连≥2%（92.75%）**——升连必须≥2%，否则无优势（首升连无限制68.84%≈基线，涨幅<2% 52.07%反而更差）。
>
...
- [六、操盘计划](#六操盘计划)
    ...
    - [6.1.1.1 综合退出规则（与已有规则不抵触）](#6111-综合退出规则与已有规则不抵触)
        - [6.1.3 操作逻辑明确化：沃土 + DXZC>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）](#613-操作逻辑明确化沃土-dxzc0甲乙己-优选升连2-不破djb不出2026-09-16-定稿)
    - [6.1.3.1 盈利性评估（逐层叠加，5天≥3%）](#6131-盈利性评估逐层叠加5天3)
    - [6.2 第1层：选土（四类区域，DXZE×CD）](#62-第1层选土四类区域dxzecd)
        - [6.2.1 四类区域：数学基础 + 全集划分](#621-四类区域数学基础-全集划分)
        - [6.2.1.1 均线系统（6条 EMA，价格 P 相对各均线的交叉天数）](#6211-均线系统6条-ema价格-p-相对各均线的交叉天数)
        - [6.2.1.2 核心数学特性（EMA）](#6212-核心数学特性ema)
        - [6.2.1.3 DXCD 六类的数学定义（P、JC=EMA26、JD=EMA60 三者排列，源自神谕.bas:4806-4823）](#6213-dxcd-六类的数学定义pjcema26jdema60-三者排列源自神谕bas4806-4823)
        - [6.2.1.4 核心洞察：长均线（EMA60）是趋势锚，短均线（EMA26）容易被破](#6214-核心洞察长均线ema60是趋势锚短均线ema26容易被破)
        - [6.2.1.5 方向判断（P vs EMA60，全量验证，次日ZD>0 = 次日价格仍站上EMA60）](#6215-方向判断p-vs-ema60全量验证次日zd0-次日价格仍站上ema60)
    ...
    - [6.4 第3层：介入时机（选时机）](#64-第3层介入时机选时机)
        - [6.4.1 介入时机（DJA丘 + 日等型 + BSHA5）](#641-介入时机dja丘-日等型-bsha5)
        - [6.4.2 升管判断（触哼入管 §5.1.4.1，2026-09-17 新准则）](#642-升管判断触哼入管-51412026-09-17-新准则)
    - [6.4.2.1 入管判定（只针对 DXAB 甲乙己，不考虑 DXZC）](#6421-入管判定只针对-dxab-甲乙己不考虑-dxzc)
    - [6.4.2.2 管中模式（扩管/平管/窄管）](#6422-管中模式扩管平管窄管)
    - [6.4.2.3 出管判定（连阴/阴阳阴 × 下一柱触顶）](#6423-出管判定连阴阴阳阴-下一柱触顶)
    - [6.4.2.4 出管边界优化（DJB 宽松边界，2026-09-18）](#6424-出管边界优化djb-宽松边界2026-09-18)
        - [6.4.3 垒升判断（柱体≥2%，§5.3）](#643-垒升判断柱体253)
    - [6.4.3.1 介入时机：只在垒升位置介入](#6431-介入时机只在垒升位置介入)
    - [6.4.3.2 持有策略：升连成群结队，可期待连续](#6432-持有策略升连成群结队可期待连续)
    - [6.4.3.3 启动信号：升连链首柱是升吞最强（§5.3.6）](#6433-启动信号升连链首柱是升吞最强536)
    - [6.4.3.4 跌连处理：DXZC>0 跌连不成群结队（§5.3.5.2）](#6434-跌连处理dxzc0-跌连不成群结队5352)
    - [6.4.3.5 持续持有 vs 波段（§5.3.1.1）](#6435-持续持有-vs-波段5311)
    - [6.4.3.6 镜面映射的全局指导（§5.3.5.2）](#6436-镜面映射的全局指导5352)
    ...
    - [6.8 第3层：介入时机（选时机）——升管坐标定位（新框架）](#68-第3层介入时机选时机升管坐标定位新框架)
        - [6.8.0 框架总览：操盘工具 + 坐标定位工具](#680-框架总览操盘工具-坐标定位工具)
        ...
        - [6.8.4 等5/6/7：出管/跌管区域（乙DXZA<0）](#684-等567出管跌管区域乙dxza0)
        - [6.8.5 介入点/止损条件汇总表](#685-介入点止损条件汇总表)
        - [6.8.6 跌管（丙丁戊）镜面映射](#686-跌管丙丁戊镜面映射)
...
> **🔗 本章结论 → 第六章入口：** DJC丘生命周期（甲启动→乙主升→丙预警）是第六章"介入时机"的底层依据——**DJC丘首/初介入选甲/己（§6.4 等1 31.4%），主升持有乙，顶部/尾部 DXZA>0 与 DXZB>0 双降警惕退出（§6.5 规则6）**。DJC丘（DXZC>0）是四类区域"主战场"的前提（§6.2.1）。
...
> - 己/甲 DXZC>0 时，次日冲高率约 21-23%，与基线（23.6%）接近——**护型本身不保证次日冲高**，需结合介入时机（DJA丘等1/2/3 + BSHA5，§6.4）和升连≥2%（§6.1.1）提升胜率。
...
> **🔗 本章结论 → 第六章入口：** 等型分析（等1 介入点 31.4%、等2 30.3%、等3 持有期 29.5%）是第六章"介入时机"（§6.4）的直接数据支撑——**DJA丘等1/2/3 + BSHA5 触发器**。等6/等7（下跌通道）非介入点，应回避。
...
| **管中** | 扩管（管宽≥5+推高顶）/平管（管宽≥5+非推高顶）/窄管（管宽0-5，无有效上涨）| 镜面 |
...
| 模式 | 样本 | 次日P3% | 次日P5% | 均高幅 |
|:--|--:|--:|--:|--:|
| **扩管（管宽≥5+推高顶）** | 2,574,958 | **32.05%** | **17.57%** | **2.66%** |
| **平管（管宽≥5+非推高顶）** | 2,536,637 | 28.51% | 13.60% | 2.35% |
| **窄管（管宽0-5）** | 2,192,087 | **9.92%** | **3.37%** | 1.24% |

> **✅ 扩管（32.05%）> 平管（28.51%）> 窄管（9.92%）**——窄管管宽过小，不会形成有效上涨形态，应回避。**推高顶 = 触顶 + 升排**（2026-09-20 用户纠正定义）。
...
| 模式 | 次日P3% | 次日P5% | 操作 |
|:--|--:|--:|:--|
| **扩管（管宽≥5+推高顶）** | **32.05%** | **17.57%** | 垒升典型，可介入 |
| **平管（管宽≥5+非推高顶）** | 28.51% | 13.60% | 可介入 |
| **窄管（管宽0-5）** | **9.92%** | **3.37%** | **应回避**（管宽过小，无有效上涨形态）|

> **✅ 管中模式区分度大：** 扩管（32.05%）> 平管（28.51%）> 窄管（9.92%），差22.13pp。**窄管应回避**——这是旧准则（DTZA≥3）没有的维度。**推高顶 = 触顶 + 升排**（2026-09-20 用户纠正定义）。
...
> - **第1层 · 选土（§6.2 四类区域，DXZE×CD 二维）：** 判断当前处于哪个区域——**主战场（DXZE>0+CD上/忐/忠）/ 鸡肋（DXZE<0+CD上/忐/忠）/ 执念（(CD中)+(DXZE>0+CD下/忑)）/ 禁区（DXZE<0+CD下/忑）**。只在**主战场**重仓，鸡肋可持有不宜过早介入，执念观察，禁区坚决不碰。
> - **第2层 · 护型（§6.3，甲乙己）：** 在选定的区域内选护型——**只做 甲/乙(ZA>0)/己，其余按角色处理**。
> - **第3层 · 介入时机（§6.4，DJA丘等1/2/3 + BSHA5）：** 叠加微观升管/垒升/剔诱多。
> - **关系：** 第1层选土决定"做不做、重仓还是轻仓"，第2/3层决定"具体买哪个、何时买"。**先选土（大势/土质），再选护型/时机（微观）**——不要在恶土（禁区/执念）里谈微观，这正是 §6.1.1 操作理念"抓大看小"的核心。
...
#### 6.1.3 操作逻辑明确化：沃土 + DXZC>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）

> **课题（用户提出，2026-09-16）：** 操作逻辑是否可以明确为：在沃土，选 DXZC>0 甲乙己，优选升连，不破DJB不出，盈利性如何？
...
在沃土（DXZE>0 + CD上/忐）
  → 选 DXZC>0 甲乙己
  → 优选升连 且 涨幅≥2%（垒升）   ← 关键修正：升连必须≥2%
  → 不破DJB不出（DXZB>0 持有）
```

**🔴 关键修正：升连必须配合涨幅≥2%（垒升），否则适得其反。**

> **§5.3.8 全量验证（7455文件，百万级样本）：**
...
>
> **⚠️ 注意：** 沃土+甲乙己把样本收敛到极小（§6.12.5 每年仅 10-14 次/股），再叠加"升连涨幅≥2%"（样本56万），可操作频率进一步降低——但每次胜率极高（92.75%）。这是"少而精"的极致形态，需接受低交易频率换取高胜率。
...
>
> **⚠️ 忠的归属（2026-09-16 统一）：** 忠 归**主战场**（DXZE>0）/ **鸡肋**（DXZE<0），遵循 VBA 仓日类定义（忠 ∈ {上,忐,忠}，见 §6.2.1）。但忠 = DXZC>0 且 DXZD<0（价格在 DJD=EMA60 之下），**上升受限、升连弱（20-22%）**——操作时需谨慎，**必须叠加"升连≥2%"（§5.3.8）才介入**，否则忠的升连不提升盈利性。上/忐 仍是首选（升连强、持久），忠 是次选（可做但需谨慎）。
>
> **⚠️ 忠 观察慎入（用户，2026-09-19，2026-09-20 统一用词）：** 虽然 VBA 定义忠 ∈ {上,忐,忠} 归主战场/鸡肋，但**操作上对忠 观察慎入**——忠虽然单看 DXZC 恒正，但 DXCD<0（忐忠忑）期间 DXZC 反复交叉（97.4%），**忠 的 DXZC 转正是假信号**；且忠 DXZD<0（价格在 EMA60 之下），**上升受限、升连弱（20-22%）、P≥3% 弱**，所以要**等待 DXCD 变为忐或上 再介入**。**只在已变为上/忐 之后考虑介入，不要在忠 里猜测何时变好。**
...
| DXCD | DJZ、EMA26、EMA60 的排列 | 土质（中周期） |
| DXZE | DJZ vs DJE（EMA120） | 大势（长周期） |

> **DJZ = 收盘价**（VBA 权威，结算.bas:348 位osBTZB）。所有 DXZ* 系列变量都是 **DJZ（收盘价）相对各均线的交叉天数**，不是"价格 P"。
...
| DXCD | 数学条件 | 几何含义 |
|:--|:--|:--|
| **上** | P > EMA26 > EMA60 | 价格最高，完全多头排列 |
| **中** | EMA26 > P > EMA60 | 价格居中，均线多头 |
| **下** | EMA26 > EMA60 > P | 价格最低，均线多头但价格跌破 |
| **忐** | P > EMA60 > EMA26 | 价格最高，但均线空头 |
| **忠** | EMA26 < P < EMA60 | 价格在 EMA26 之上、EMA60 之下（均线空头）|
| **忑** | P < EMA26 < EMA60 | 价格在 EMA26 之下（均线空头）|

> **VBA 判定逻辑（神谕.bas:4806-4823，结算.bas:349-360）：** DXCD 由 BTCD（EMA26 vs EMA60）、BTZC（P vs EMA26）、BTZD（P vs EMA60）三者决定：
> - **BTCD>0（EMA26>EMA60）**：BTZC>0→上，BTZD>0→中，否则→下
> - **BTCD≤0（EMA26<EMA60）**：BTZD>0→忐，BTZC>0→忠，否则→忑
> - **忠 = EMA26 < P < EMA60**（P 在 EMA26 和 EMA60 之间）；**忑 = P < EMA26**（P 在 EMA26 之下，因 EMA26<EMA60 故 P<EMA60 自动成立）。

##### 6.2.1.4 核心洞察：长均线（EMA60）是趋势锚，短均线（EMA26）容易被破

> **用户理论（2026-09-17）：** 短均线（EMA26）更容易被破，而下破长均线（EMA60）时可能已经多次穿插短均线。所以判断方向应**以长均线（EMA60）为锚**，而非短均线（EMA26）。

##### 6.2.1.5 方向判断（P vs EMA60，全量验证，次日ZD>0 = 次日价格仍站上EMA60）

| DXCD | 数学条件 | 方向 | 次日ZD>0%（甲乙己） | 次日ZD>0%（丙丁戊） |
|:--|:--|:--|:--|:--|
| **上** | P>EMA26>EMA60 | **升势** | 96.9-99.3% | 94.8-97.1% |
| **中** | EMA26>P>EMA60 | **升势（偏升）** | 68.3-83.3% | 70.4-81.5% |
| **忐** | P>EMA60>EMA26 | **升势** | 88.4-90.1% | 58.8-63.6% |
| **下** | EMA26>EMA60>P | **跌势** | 35.0-38.2% | 8.8-19.3% |
| **忠** | EMA26<P<EMA60 | **跌势（偏跌）** | 16.4-29.5% | 16.5-31.6% |
| **忑** | P<EMA26<EMA60 | **跌势** | 2.4-4.1% | 0.6-3.3% |

> **⚠️ 数据完全支持用户理论：**
> - **升势（P>EMA60）**：上/中/忐 —— 次日ZD>0 高（68-99%）
> - **跌势（P<EMA60）**：下/忠/忑 —— 次日ZD>0 低（0-38%）
> - **中（EMA26>P>EMA60）偏升**：跌破短均线但站上长均线，次日ZD>0 高达 68-83% —— **用户判断"中更可能升势"正确**。
> - **忠（EMA26<P<EMA60）偏跌**：站上短均线但跌破长均线，次日ZD>0 仅 16-32% —— **用户判断"忠更可能跌势"正确**。
> - **核心：判断方向的关键是价格相对长均线（EMA60）的位置，不是短均线（EMA26）。** 短均线易破，破/站上短均线不是强方向信号；长均线才是趋势锚。

##### 6.2.1.6 待定状态（一条均线之上一条之下）的方向倾向

| 待定 | 数学条件 | 方向倾向 | 原因 |
|:--|:--|:--|:--|
| **中** | EMA26>DJZ>EMA60 | **偏升** | 跌破短均线（短期回调），站上长均线（长期升势）|
| **忠** | EMA60>DJZ>EMA26 | **偏跌** | 站上短均线（短期反弹），跌破长均线（长期跌势）|

**强度（均线排列方向）：**

| 均线排列 | DXCD | 强度 |
|:--|:--|:--|
| 多头（EMA26>EMA60） | 上/中/下 | 升势更强（上>中）|
| 空头（EMA26<EMA60） | 忐/忠/忑 | 升势更弱（忐<上）|
...
> **结论：** 四类区域的数学基础 = **均线系统几何结构**。方向由长均线（EMA60）决定（P>EMA60=升势，P<EMA60=跌势），待定状态（中偏升/忠偏跌）由"跌破/站上短均线但长均线方向相反"决定，强度由均线排列（多头/空头）决定。**DXAB 在各区域内做优选，不改变区域划分。** 此数学框架是 §6.2.1 概率划分的理论基石。
...
> - **核心：DXCD 是决定性变量，DXAB 是次要的**——先选土（DXCD），再选护型（DXAB），不要在恶土里谈护型。

> [!IMPORTANT]
> 这是整个定型操作框架的**决定性一层**：在选区内，唯一需要判断的就是**选择哪个护型**。下表是全部 8 个护型的完整评定（P≥3% + 持续性 + 次日转移 + 操作结论），**操作时应以此表为最终依据**。

> **🔍 DXCD 是决定性变量，DXAB 是次要的（2026-09-20 用户确认）：** 第1层选土（DXCD）决定"做不做"，第2层护型（DXAB）决定"选哪个"。**DXCD 是真正的决定性变量**——在选好的土质（DXCD=上/忐）里，**丙丁戊可忍受**（分角色处理：乙ZA<0/丙持仓或退出、戊ZA>0观察、丁/戊ZA<0放弃）；在恶土（DXCD=中/下/忑）里，甲乙己也几乎消失（乙ZA>0 仅0.03-0.7%）。**先选土（DXCD），再选护型（DXAB）**——不要在恶土里谈护型。
...
##### 6.4.2.2 管中模式（扩管/平管/窄管）

> **升管分类（用户，2026-09-19，2026-09-20 纠正定义）：** 主要分为三类——**窄管**（管宽过小，不会形成有效上涨形态）、**扩管**（管宽≥5 + 柱排推高顶，升宽在扩大）、**平管**（管宽≥5 + 非推高顶，升宽持平）。**推高顶 = 触顶 + 升排**。窄管 = 0 < 管宽哼JA < 5。
...
| **平管（管宽≥5+非推高顶）** | 2,536,637 | 28.51% | 13.60% | 2.35% |
| **窄管（管宽0-5）** | 2,192,087 | **9.92%** | **3.37%** | 1.24% |
| 管宽≤0 | 123,565 | 2.43% | 1.68% | 0.44% |
| 管宽空 | 1,219,686 | 18.24% | 7.46% | 1.71% |

> **✅ 扩管（32.05%）> 平管（28.51%）> 窄管（9.92%）**，差 22.13pp——**扩管 次日P3% 最高（管宽≥5 + 柱排推高顶），窄管 最低（管宽0-5，管宽过小，不会形成有效上涨形态，应回避）**。区分度远强于旧固定阈值分类（4.79pp）。
...
### 6.8 第3层：介入时机（选时机）——升管坐标定位（新框架）

> **框架定位（2026-09-20）：** 第3层在"选区域（第1层 DXCD）→ 选护型（第2层 甲乙己）"之后，解决**何时介入**。核心思路：在升管（DXAB甲乙己 + 升管入管）中，用 **（DXAB + 等型）** 作为坐标定位介入时机。等型坐标：**等1（起点）/等2（早期）/等3（成熟）/等5/6/7（出管）**。
> **管宽分类铁律（2026-09-20 用户纠正）：** 扩管/平管/窄管**必须用管宽哼JA（col22）**，不是宽哼JC（col23）。**窄管 = 0 < 管宽哼JA < 5**；**管宽≥5 时看柱排是否推高顶（推高顶=触顶+升排）——推高顶=扩管，非推高顶=平管**。管宽哼JA 只在升管（日ZA>0）时有值。**升管 = DXAB 甲乙己 + DXZA>0**（用户确认：只有甲乙己才能定义升管）。

#### 6.8.0 框架总览：操盘工具 + 坐标定位工具
...
- **等型坐标：** 等1（起点）/等2（早期）/等3（成熟）/等5/6/7（出管）。
- 等型判定（BT连阳 col44，VBA权威）：等1=日ZA=1，等2=日ZA=2，等3=日ZA≥3，等5=日ZA=-1，等6=日ZA=-2，等7=日ZA≤-3。

> **🔍 升管/跌管定义（2026-09-20 用户确认）：**
> - **升管 = DXAB 甲乙己**（甲/乙/己，正交）。本框架 §6.8.1~6.8.3 的等1/2/3 均限定甲乙己。
> - **跌管 = DXAB 丙丁戊**（丙/丁/戊，非正交），由升管**镜面映射**定义。
> - **管宽哼JA 定义（VBA权威）：** 管宽哼JA = (最近5日最高价 / EMA5 − 1) × 100，即**最近5日最高价相对 EMA5 的偏离百分比**。**管宽哼JA 就是衡量升宽的变量**（用户确认，2026-09-20）。
> - **🔍 管宽分类（用户纠正，2026-09-20）：** 扩管/平管/窄管**不是固定阈值**（>60/0-60/<0），而是：
>   - **窄管**：0 < 管宽哼JA < 5（升宽很小，无有效上涨，应回避）。
>   - **管宽≥5**：看柱排是否**推高顶**（推高顶 = 触顶 + 升排）——**推高顶=扩管**（升宽在扩大），**非推高顶=平管**（升宽持平）。
>   - 全量验证（等3甲乙己）：窄管 9.40%（n=1,614,900）、扩管 31.72%（n=2,220,335）、平管 27.88%（n=1,769,930）——**扩管 vs 窄管差 22.32pp**，区分度远强于旧固定阈值（4.79pp）。
> - **管宽哼JA 无负值**（最高价通常高于均线），升管/跌管分布几乎一致（0-30 占 96%+）。
> - **跌管也有窄管(0-5)**（丙5.22%、戊8.76%，极弱应回避）——窄管定义（0-5）在升管/跌管通用（见 §6.8.6）。

#### 6.8.0.1 升管管宽分类：原则 + 分布统计（2026-09-20 全量验证）

> **升管定义（用户确认）：** 升管 = **DXAB 甲乙己 + DXZA>0**。**只有甲乙己才能定义升管**（戊/丙/丁即使 DXZA>0 也不算升管）。操作流程：**DXZA>0（基本条件）→ 触哼 → 入管 → 入管后区分窄管/扩管/平管**。
...
**② 管宽分类分布（全量 7455 文件，升管 n=8,646,933）：**

| 分类 | n | 占比 |
|:--|--:|--:|
| **窄管（0-5）** | 5,408,454 | **62.55%** |
| **扩管（≥5+推高顶）** | 1,856,187 | **21.47%** |
| **平管（≥5+非推高顶）** | 1,343,702 | **15.54%** |
| 管宽≤0 | 38,589 | 0.45% |

> **升管中 62.55% 是窄管（0-5）**——大部分升管管宽很小，无有效上涨形态，应回避。扩管/平管合计仅 37%。
...
> **管宽哼JA 几乎都在 0-20（99.2%），>60 仅 0.01%**——**旧固定阈值"扩管>60"几乎不存在**，印证用户纠正的正确性（旧阈值分类区分度仅 4.79pp，新分类 24pp+）。

**④ 升管管宽分类 × 次日P3%（全量，基线 23.03%）：**

| 分类 | n | 次日P3% | 均高幅 |
|:--|--:|--:|--:|
| **扩管（≥5+推高顶）** | 1,856,187 | **38.73%** | 3.10% |
| **平管（≥5+非推高顶）** | 1,343,702 | **37.04%** | 2.86% |
| **窄管（0-5）** | 5,408,454 | **14.29%** | 1.49% |
| 管宽≤0 | 38,589 | 4.35% | 0.50% |

> **区分度：扩管 38.73% vs 窄管 14.29%，差 24.44pp**。扩管/平管是介入主战场，窄管应回避。

**⑤ 升管管宽分类 × 等型（次日P3%）：**

| 等型 | 窄管 | 平管 | 扩管 |
|:--|--:|--:|--:|
| 等1 | 19.16% | 42.77% | 40.36% |
| 等2 | 19.00% | 42.52% | 41.71% |
| **等3** | 19.88% | 42.20% | **48.79%** |
| 等5 | 15.52% | 38.05% | 34.94% |
| 等6 | 13.99% | 34.39% | 33.19% |
| 等7 | 7.95% | 30.54% | 27.18% |

> **扩管×等3 = 48.79% 是最强介入点**（n=318,810）。扩管/平管在等1-3 都强（40-49%），窄管在等7 最弱（7.95%，接近出管）。
...
> **扩管主要在甲（11.91%）和乙（8.09%）**，己很少（1.47%）。升管护型分布：甲 48.67%、乙 37.65%、己 13.68%。

**⑦ 升管管宽分类 × 柱排（次日P3%）：**

| 柱排 | 窄管 | 平管 | 扩管 |
|:--|--:|--:|--:|
| 升连 | 14.31% | 40.69% | 39.94% |
| 升吞 | 16.01% | 43.92% | 40.69% |
| 升孕 | 12.78% | 36.77% | 33.68% |
| 人排 | 14.99% | 37.52% | — |
| 跌连 | 12.34% | 33.34% | — |
| 跌孕 | 13.71% | 31.73% | — |
| 跌吞 | 12.39% | 30.04% | — |

> **扩管只有升排**（扩管=触顶+升排，人排/跌排不满足"推高顶"定义）。平管×升吞 43.92% 最强；窄管×升孕 12.78% 最弱。

> **核心结论：** 升管（甲乙己+DXZA>0）中，**扩管/平管（管宽≥5）是介入主战场**（37-39%），**窄管（0-5）占 62.55% 应回避**（14.29%）。**扩管×等3 = 48.79% 是最强介入点**。管宽哼JA 恒≥0，窄管(<0) 在升管语境不存在。

#### 6.8.1 等3：成熟升管（站上DJA 3柱+）

> **等3 是主介入区**（样本最大，n=6,417,364，基线 22.99%）。管宽哼JA 是决定性维度：**扩管（管宽≥5+推高顶）极强**（升连 33.11%），**窄管（0-5）极弱**（6.94-10.89%，应回避）。

**已验证数据（甲乙己，n=6,417,364，基线 22.99%，推高顶=触顶+升排）：**

| 柱排 | 窄管(0-5) | 平管(≥5+非推高顶) | 扩管(≥5+推高顶) |
|:--|:--|:--|:--|
| 升连 | 10.20% (n=655,837) | 26.82% (n=150,478) | **33.11%** (n=1,439,156) |
| 升吞 | 10.89% (n=144,470) | 29.61% (n=47,044) | **32.65%** (n=256,957) |
| 升孕 | 8.65% (n=236,789) | 24.92% (n=69,284) | 27.46% (n=524,222) |
| 人排 | 8.89% (n=424,800) | 28.80% (n=1,041,644) | — |
| 跌连 | 6.94% (n=119,528) | 26.89% (n=360,319) | — |
| 跌孕 | 7.88% (n=20,725) | 25.24% (n=64,111) | — |
| 跌吞 | 7.91% (n=12,751) | 24.09% (n=37,050) | — |

> **结论：**
> ① **窄管(0-5) 所有柱排都极弱**（6.94-10.89%），远低于基线（22.99%）——**窄管应回避**（管宽过小，无有效上涨形态）；
> ② **扩管(≥5+推高顶) 升排最强**：升连 33.11%、升吞 32.65%、升孕 27.46%——**管宽≥5 + 触顶 + 升排 = 最强介入区**；
> ③ **平管(≥5+非推高顶) 次强**（24-30%），人排/跌排也可做但弱于扩管升排；
> ④ **区分度巨大**：扩管升连(33.11%) vs 窄管跌连(6.94%) 差 **26.17pp**——远强于旧固定阈值分类（4.79pp）。
> ⚠️ **扩管只有升排**（扩管=触顶+升排，人排/跌排不满足"推高顶"定义）。

> **🔍 专项验证（2026-09-20）：等3 管宽哼JA空 = 转弱信号。** 等3（甲乙己）中管宽哼JA为空的行（n=716,134）次日P3%=**17.41%**，明显低于基线（22.99%）。**等3 里管宽哼JA为空 → 管宽消失 → 即将出管/转弱**，应回避。这补充了"不触顶阳柱→管宽缩小→下破DJA"链条的末端：管宽缩小到消失（管宽哼JA空）时胜率已降至 17.41%。

**介入点条件（等3）：**
- **首选：** 扩管（管宽≥5 + 触顶 + 升排）→ 升连 33.11%、升吞 32.65%、升孕 27.46%，最强介入区。
- **次选：** 平管（管宽≥5 + 非推高顶）→ 24-30%，人排/跌排也可做但弱于扩管升排。
- **升排推高顶：** 扩管→升排推高顶（触顶+升排），可期待连续冲高。
- **平管平移十字星：** 关注下柱冲高（十字星后方向待确认，专项验证：平管十字星次日P3%=21.48%，接近基线，非强信号）。

**止损条件（等3）：**
- **窄管（管宽哼JA 0-5）→ 6.94-10.89%，极弱，应回避**（管宽过小，无有效上涨形态）。
- 等3 跌排/人排：关注是否维持管宽；**不触顶阳柱 → 管宽缩小 → 下破DJA** 是转坏链条，跌破 DJA 即止损。
- **管宽哼JA空（管宽消失）→ 17.41%，转弱信号，应回避或减仓。**
- 未触顶 + 跌排（跌连/跌孕/跌吞）→ 胜率 24-27%，弱于扩管升排，应回避或减仓。
...
#### 6.8.4 等5/6/7：出管/跌管区域（乙DXZA<0）

> **等5/6/7 整体偏弱（20-23%），非主介入区。** 等7（超卖反弹）反而最高（22.92%），但样本少。
...
> **结论：** 等7 反而最高（22.92%，超卖反弹），但样本少；等5/6/7 整体偏弱（20-23%），**非主介入区**。
...
**止损条件（等5/6/7）：**
- 等5/6/7 是出管/跌管区域，**整体回避为主**，介入仅限反弹试仓。
- 跌破 DJA 且无反弹 → 止损离场。
...
| **等3** | 成熟升管 | **扩管（管宽≥5+触顶+升排）→升连33.11%**；平管（≥5+非推高顶）24-30% | **窄管(0-5)→6.94-10.89%极弱回避**；不触顶阳柱→管宽缩小→下破DJA止损；**管宽哼JA空(17.41%)转弱回避** | 7-33% |
| **等5/6/7** | 出管/跌管 | 首下破反弹；**等6第二柱阳柱+次日再站DJA(37.37%)**；整体回避 | 跌破DJA无反弹离场 | 20-23% |

> **核心结论：** 介入时机坐标 = **等3 扩管（管宽≥5 + 触顶 + 升排）** 是最强介入点（升连 33.11%）；等2 靠触顶增强；等1 靠第2柱确认（维持升管29.39% vs 回落2.58%）；等5/6/7 整体回避（仅等6第二柱阳柱+次日再站DJA 37.37%可介入）。**管宽哼JA（col23）是决定性维度**——窄管(0-5)极弱应回避，扩管（≥5+推高顶）升排最强，管宽哼JA空（管宽消失）是转弱信号。
...
> **管宽哼JA 无负值**（最高价通常高于均线），所以跌管**没有"窄管<0"**。但**窄管定义（0-5）在跌管同样适用**——跌管窄管(0-5) 极弱（丙5.22%、戊8.76%，见下方②），应回避。

**② 跌管各护型管宽分类次日P3%（基线 17.82%，n=5,911,829，推高顶=触顶+升排）：**
| 分类 | 丙 | 戊 |
|:--|:--|:--|
| **窄管(0-5)** | **5.22%** (n=158,812) | **8.76%** (n=37,339) |
| **平管(≥5+非推高顶)** | 25.14% (n=619,238) | 25.68% (n=83,189) |
| **扩管(≥5+推高顶)** | 24.14% (n=6,636) | 26.96% (n=1,172) |
| 管宽≤0 | 0.17% (n=4,137) | 0.45% (n=1,338) |
| 管宽空 | 16.98% (n=468,854) | 17.29% (n=4,531,114) |

> **🔍 跌管管宽分类（2026-09-20 用户纠正版）：** 跌管（丙丁戊）**窄管(0-5) 也极弱**（丙5.22%、戊8.76%），应回避；**平管/扩管（管宽≥5）反而较强**（24-27%，高于基线17.82%）——跌管中管宽≥5 时胜率高于整体。⚠️ **修正：** 旧分类"跌管扩管>60 也强（丙54.68%）"基于固定阈值（>60）且样本极小（n=1,046），新分类下跌管扩管（≥5+推高顶）仅 24.14%（n=6,636），**不再有"超跌反弹"强信号**。

**③ 跌管各等型（都偏弱，无强介入区）：**
| 等型 | 次日P3% | n |
|:--|:--|:--|
| 等1(ZA=1) | 19.24% | 830,100 |
| 等7(ZA≤-3) | 18.53% | 2,598,475 |
| 等6(ZA=-2) | 17.63% | 880,654 |
| 等2(ZA=2) | 16.39% | 348,782 |
| 等5(ZA=-1) | 16.01% | 967,841 |
| 等3(ZA≥3) | 15.74% | 285,977 |

**镜面映射操作逻辑（跌管 丙丁戊）：**
- **升管介入（扩管≥5+推高顶 升排）→ 跌管回避/退出**（跌管整体偏弱，基线 17.82%）。
- **跌管窄管(0-5) → 极弱（5-9%），回避。**
- **跌管平管/扩管（管宽≥5）→ 24-27%，可轻仓试仓**（高于跌管基线，但低于升管扩管33%）。
- **跌管管宽哼JA空 → 转弱**（16.98%/17.29%），回避。
- **跌管各等型均偏弱（15-19%）**，无强介入区，以回避为主。

> **⚠️ 待办：** 跌管（丙丁戊）的**窄管/转弱信号**已用新分类（窄管0-5）定义，但跌管管宽哼JA 无负值，需进一步验证跌管窄管的边界。
