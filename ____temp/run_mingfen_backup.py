...
TEMP = r'd:\@VSwork\VS昭明计划VBA优化\昭明算展\谕组周_备份_20260804'

all_weeks = []
for f in sorted(os.listdir(TEMP)):
    if not f.startswith('谕组周_') or not f.endswith('.csv'):
        continue
    code = f.replace('谕组周_', '').replace('.csv', '')
...
# 输出到CSV
csv_out = r'd:\@VSwork\VS昭明计划VBA优化\_产出物\月基策分结果.csv'
