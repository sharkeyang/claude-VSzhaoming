# -*- coding: utf-8 -*-
"""
DXAB正交/持续时间/阴柱特权 综合分析（高波池）
====================================================
回答用户问题：
1. DXAB初期阴柱特权：DXAB>0初期(值1-4)遇阴柱，次日冲高率？
2. DXAB正交后又短期转负的概率：正交(→/中/0)后N天内转负的概率
3. DXAB>0持续时间统计规律
4. DXAB<0持续时间统计规律
5. DXAB正交是否可作为机会（次日冲高率 vs 基线）
6. DXZC>0与DXAB正交交叉统计
7. 一个DXZC>0区域一般有多少次DXAB交叉

磁盘CSV列序：0日期 5涨幅 9DXAB 10柱排 13日ZA 14日ZC 26次日高幅 28上身
DXAB格式：a甲↗上1.I → 护型代码+汉字+正交符号(↗/→/↘)+方向(上/忐/忠/中/忑/下)+数值
"""
...
# ============ 统计容器 ============
# 问题1: DXAB初期阴柱特权
q1 = defaultdict(lambda: [0,0,0.0])  # key: 护型|DXAB值段|是否阴柱
# 问题2: 正交后转负
q2 = defaultdict(lambda: [0,0])  # key: 护型|N天 | [正交样本数, N天内转负数]
# 问题3/4: 持续时间
dur_pos = defaultdict(lambda: [0,0.0])  # key: 护型 -> [run数, 总天数]
dur_neg = defaultdict(lambda: [0,0.0])
dur_pos_all = [0,0.0]  # 全部护型
dur_neg_all = [0,0.0]
# 问题5: 正交次日冲高
q5 = defaultdict(lambda: [0,0,0.0])  # key: 护型|ZC方向
# 问题6: DXZC>0区域内的DXAB交叉次数
q6 = defaultdict(lambda: [0,0.0])  # key: 护型 -> [区域数, 总交叉次数]
# 问题7: 正交后转负的分布（转负发生在第几天）
q7 = defaultdict(lambda: [0,0.0])  # key: 护型 -> [正交样本数, 转负天数总和]
...
for fname in sorted(os.listdir('昭明算展/谕组日')):
    ...
    zfs = [to_f(rows[i][5]) for i in range(n)]  # 涨幅（判断阴阳柱）

    # ===== 问题1: DXAB初期阴柱特权 =====
    # DXAB>0初期(值1-4)，当前阴柱(zf<0)，次日冲高率
    for i in range(n):
    ...
    for i in range(n):
        ...
        if not hx: continue
        # 正交日 = 方向字符'中' 或 数值0
        is_orth = (dirch == '中') or (val == 0)
        if not is_orth: continue
        hr = hrs[i]
        zc = zcs[i]
    ...
    # ===== 问题6: DXZC>0区域内DXAB交叉次数 =====
    # DXZC>0区域 = 连续日ZC>0；区域内统计DXAB从正转负/负转正的交叉次数
    i = 0
    while i < n:
        zc = zcs[i]
...
# ===== 问题3/4输出 =====
print('='*75)
print('【问题3/4】DXAB>0 与 DXAB<0 持续时间')
print('='*75)
...
# ===== 问题5输出 =====
print('='*75)
print('【问题5】DXAB正交次日冲高率（正交是否可作为机会）')
print('='*75)
...
# ===== 问题7输出 =====
print('='*75)
print('【问题7】DXAB正交后首次转负的平均天数')
print('='*75)
print(f'{"护型":<4} {"正交样本":>10} {"平均转负天数":>14}')
print('-'*40)
for hx in ['甲','乙','己','戊','丙','丁']:
    if hx in q7:
        s = q7[hx]
