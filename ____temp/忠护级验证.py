...
for i, f in enumerate(files):
    df = pd.read_csv(f, encoding='gbk', header=None, usecols=[9,10,13,14,26])
    df.columns = ['DXAB','柱排','日ZA','日ZC','次日高幅']
...
for nh, (n, p3) in agg_trans.items():
    if n >= 1000: rows.append((nh, n, p3/n*100))
rows.sort(key=lambda r: -r[1])
print(f"{'次日护级':<6}{'样本':>10}{'占比%':>10}{'5天≥3%':>10}")
total = sum(n for _, (n, _) in agg_trans.items())
for nh, n, p3 in rows:
    print(f"{nh:<6}{n:>10}{n/total*100:>10.2f}{p3:>10.2f}")
print('\n' + '='*70)
print('3. 忠 × 柱排')
print('='*70)
rows = []
