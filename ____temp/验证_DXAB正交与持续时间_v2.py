# -*- coding: utf-8 -*-
"""
DXAB正交/持续时间/阴柱特权 综合分析 v2（高波池）
====================================================
DXAB字符串结构（VBA层护型编码）：
  a甲↗上1.I
  位1(0)=层护段AB代码(a/b/c/r/y/z)  位2(1)=护型汉字(甲乙丙己戊丁)
  位3(2)=正交符号(↗=甲乙己+BTZA≥0, →=甲乙己+BTZA<0, ↘=丙丁戊)
  位4(3)=层护级AB=DXCD护级(上/忐/忠/中/忑/下)
  位5=BTAB数值(=日ZC)  .  位6=BTZA关系(A/B/I/Y/Z+BTZA值)

关键：DXAB数值 = BTAB = 日ZC（价格vs DJC=EMA26交叉天数）
      DXAB正交 = 正交符号'→' = 甲乙己护型 + BTZA<0（价格在DJA之下）
      DXAB>0 = BTAB>0 = 日ZC>0 = DJC丘
"""
...
for fname in sorted(os.listdir('昭明算展/谕组日')):
    ...
    for i in range(n):
        ...
        # 问题8: 正交 vs 非正交 次日冲高
        if is_orth:
            q8[f'{hx}|正交'][0]+=1; q8[f'{hx}|正交'][1]+= (1 if hr>=3 else 0); q8[f'{hx}|正交'][2]+=hr
        else:
            q8[f'{hx}|非正交'][0]+=1; q8[f'{hx}|非正交'][1]+= (1 if hr>=3 else 0); q8[f'{hx}|非正交'][2]+=hr
        if is_orth:
...
# ===== 问题1输出 =====
print('='*75)
print('【问题1】DXAB初期阴柱特权（BTAB>0，当前阴柱 vs 阳柱，次日冲高率）')
print('='*75)
...
# ===== 问题2输出 =====
print('='*75)
print('【问题2】DXAB正交(→)后N天内BTAB转负的概率')
print('='*75)
...
# ===== 问题3/4输出 =====
print('='*75)
print('【问题3/4】BTAB>0 与 BTAB<0 持续时间（=日ZC>0/<0）')
print('='*75)
...
# ===== 问题5输出 =====
print('='*75)
print('【问题5】DXAB正交(→)次日冲高率（正交是否可作为机会）')
print('='*75)
...
# ===== 问题6输出 =====
print('='*75)
print('【问题6】DXZC>0区域内BTAB交叉次数（主护型）')
print('='*75)
...
# ===== 问题8输出 =====
print('='*75)
print('【问题8】正交(→) vs 非正交 次日冲高率')
print('='*75)
print(f'{"护型":<4} {"状态":<6} {"n":>10} {"P(≥3%)":>8} {"均高幅":>8}')
print('-'*50)
for hx in ['甲','乙','己','戊','丙','丁']:
    for st in ['正交','非正交']:
        key = f'{hx}|{st}'
        if key in q8:
            s = q8[key]
            if s[0] >= 500:
                print(f'{hx:<4} {st:<6} {s[0]:>10,} {s[1]/s[0]*100:>7.2f}% {s[2]/s[0]:>7.2f}%')
print()