# -*- coding: utf-8 -*-
"""
回调分类：阴柱回调 / 下破DJA回调 的细分
====================================================
背景（verify_tent_signal.py）：
...
def classify_yin_zp(zp):
    """阴柱回调：按回调阴柱的柱排分类"""
    s = zp.strip()
    if '人' in s:
        return '阴阳阴(人排)'
    if s.startswith('跌'):
        if '反孕' in s: return '单阴柱(跌孕)'
        if '尾吞' in s or '连后吞' in s: return '单阴柱(跌吞)'
        if '尾连' in s: return '连阴(跌连)'
    return '其他'

def classify_yin_zp_detail(zp):
    """阴柱回调：柱排细分（用于其他桶诊断）"""
    s = zp.strip()
    if '人' in s: return '人排'
    if s.startswith('升'):
...
# 统计
yin_stats = collections.defaultdict(lambda: [0, 0])   # 阴柱回调分类 -> [n, 再次触顶/升排]
yin_detail = collections.defaultdict(lambda: [0, 0])  # 阴柱回调柱排细分
xp_stats = collections.defaultdict(lambda: [0, 0])    # 下破柱数 -> [n, 再次触顶/升排]
xp_zp_stats = collections.defaultdict(lambda: [0, 0]) # 下破期间柱排 -> [n, 再次触顶/升排]
...
for frows in files:
    ...
    for i in range(1, N-1):
        ...
        if cb_type == '阴柱':
            cls = classify_yin_zp(frows[cb_idx][4])
            yin_stats[cls][0] += 1
            if sig: yin_stats[cls][1] += 1
            det = classify_yin_zp_detail(frows[cb_idx][4])
            yin_detail[det][0] += 1
            if sig: yin_detail[det][1] += 1
        else:  # 下破DJA
...
report('① 阴柱回调分类（按回调阴柱柱排）', yin_stats)
report('①b 阴柱回调柱排细分（诊断其他桶）', yin_detail)
