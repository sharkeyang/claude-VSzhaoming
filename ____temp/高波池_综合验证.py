...
# 高波池 = Qic+Qim+Qit（日冲策略主战场）；低波池 = Qd+Qe+Qif（月基策略主战场）
# 已剔除：Qin(非成分)+Qst(ST/退市)+Qbj(北交所) 不参与
import sys as _sys
池类型 = _sys.argv[1] if len(_sys.argv) > 1 else '高波池'
if 池类型 == '高波池':
    目标板块 = {'Qic', 'Qim', 'Qit'}
else:
    目标板块 = {'Qd', 'Qe', 'Qif'}
...
def main():
    ...
    gb_cids = [k for k, v in board_map.items() if v in 目标板块]
    gb_cids_set = set(gb_cids)
    print(f'{池类型}股票: {len(gb_cids)}', flush=True)
    ...
    print(f'  {池类型}文件: {n_files}, 行: {gb_rows:,}', flush=True)

    out = []
    out.append('=' * 80)
    out.append(f'{池类型}综合验证')
    out.append('=' * 80)
    out.append(f'{池类型}文件: {n_files}, 样本行: {gb_rows:,}')
    out.append('')
    ...
    result = '\n'.join(out)
    print(result)
    out_path = f'____temp/{池类型}_综合验证结果.txt'
