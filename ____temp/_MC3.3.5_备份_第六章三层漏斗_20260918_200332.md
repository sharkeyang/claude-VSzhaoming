...
>
> **关键增强信号（§6.4 + §6.1.1.1）：** DJA丘等1（31.4%）+ BSHA5（39.4%）+ **升连≥2%（92.75%）**——升连必须≥2%，否则无优势（首升连无限制68.84%≈基线，涨幅<2% 52.07%反而更差）。
>
> **阅读指引：** 第一~五章是各维度分析（护型/等型/正交动力学/三态理论），第六章是决策收敛（§6.1 定型操作 + §6.11 四类区域 + §6.1.1 操作理念），第七~八章是跨周期验证 + 数据规范。**核心决策在第六章，前面各章是数据支撑。**
...
- [六、操盘计划](#六操盘计划)
    - [6.1 核心框架：四层漏斗 + 真假正交识别](#61-核心框架四层漏斗-真假正交识别)
        - [6.1.1 用户操作理念：沃土→甲乙己不破DJB→升管→垒升→剔诱多（2026-09-16 定稿）](#611-用户操作理念沃土甲乙己不破djb升管垒升剔诱多2026-09-16-定稿)
            - [6.1.1.1 操作逻辑明确化：沃土 + DXZC>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）](#6111-操作逻辑明确化沃土-dxzc0甲乙己-优选升连2-不破djb不出2026-09-16-定稿)
    - [6.2 第1层：DXCD 选择（上/忐 首选，忠 谨慎，中/下/忑 放弃）](#62-第1层dxcd-选择上忐-首选忠-谨慎中下忑-放弃)
    - [6.3 第2层：DXZC>0 区域护型选择——定型操作的核心层](#63-第2层dxzc0-区域护型选择定型操作的核心层)
    - [6.4 第3层：介入时机（DJA丘 + 日等型 + BSHA5）](#64-第3层介入时机dja丘-日等型-bsha5)
    ...
    - [6.6 定型操作期望收益（2026-09-08 验证，BSHA 用列19修正后）：](#66-定型操作期望收益2026-09-08-验证bsha-用列19修正后)
    - [6.7 持有条件样本占总天数比例（2026-09-08 验证）：](#67-持有条件样本占总天数比例2026-09-08-验证)
        - [6.7.1 高波池（Qic+Qim+Qit，3451只）：](#671-高波池qicqimqit3451只)
        - [6.7.2 低波池（Qe+Qin+Qd+Qif+Qst，3732只）：](#672-低波池qeqinqdqifqst3732只)
    - [6.8 操作区域"少而精"原则——不要随便退出，阴柱可介入，仅诱多退出（2026-09-08 验证）：](#68-操作区域少而精原则不要随便退出阴柱可介入仅诱多退出2026-09-08-验证)
    - [6.9 股性分析：持有条件比例是否定义股票性格（2026-09-08 验证）：](#69-股性分析持有条件比例是否定义股票性格2026-09-08-验证)
        - [6.9.1 股性稳定性（关键测试）：](#691-股性稳定性关键测试)
        - [6.9.2 按持有比例分组：收益与波动（高波池）：](#692-按持有比例分组收益与波动高波池)
        - [6.9.3 极端对比（最高10% vs 最低10%）：](#693-极端对比最高10-vs-最低10)
    - [6.10 入管依据验证（旧准则）：DTZA 持续天数（升管/跌管）作为入管信号（2026-09-11 验证）：](#610-入管依据验证旧准则dtza-持续天数升管跌管作为入管信号2026-09-11-验证)
    - [6.11 四类区域：数学基础 + 全集划分](#611-四类区域数学基础-全集划分)
        - [6.11.1 四类区域全集划分：禁区/执念/鸡肋/主战场（2026-09-16 定稿）](#6111-四类区域全集划分禁区执念鸡肋主战场2026-09-16-定稿)
...
> **🔗 本章结论 → 第六章入口：** DJC丘生命周期（甲启动→乙主升→丙预警）是第六章"介入时机"的底层依据——**DJC丘首/初介入选甲/己（§6.4 等1 31.4%），主升持有乙，顶部/尾部 DXZA>0 与 DXZB>0 双降警惕退出（§6.5 规则6）**。DJC丘（DXZC>0）是四类区域"主战场"的前提（§6.11）。
...
> - 己/甲 DXZC>0 时，次日冲高率约 21-23%，与基线（23.6%）接近——**护型本身不保证次日冲高**，需结合介入时机（DJA丘等1/2/3 + BSHA5，§6.4）和升连≥2%（§6.1.1.1）提升胜率。
...
> **🔗 本章结论 → 第六章入口：** 护型分析（甲/乙ZA>0 可做、乙ZA<0 交叉路口、丙持仓或退出、戊ZA>0 观察、丁/戊ZA<0 放弃）是第六章"护型选择"（§6.3）的直接数据支撑——**只做 甲/乙(ZA>0)/己，其余按角色处理**。DXZC>0 限定（上/忐/忠）对应四类区域"主战场"（§6.11）。
...
> **🔗 本章结论 → 第六章入口：** 柱与DJA三态理论 + 操作规则优化是第六章"操作理念"（§6.1.1）的直接数据支撑——**升管（触哼入管，§5.1.4.1）、垒升（柱体≥2%，§5.3）、剔诱多（跌连-非触顶，§6.8.3）、不破DJB不出（DXZB退出规则，§5.2.9）** 正是 §6.1.1 操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）的微观层。**本章是第六章操作理念的"微观细节库"。**
...
>
> **第1步 · 选区（§6.11 四类区域，DXZE×CD 二维）：** 判断当前处于哪个区域——**主战场（DXZE>0+CD上/忐/忠）/ 鸡肋（DXZE<0+CD上/忐/忠）/ 执念（(CD中)+(DXZE>0+CD下/忑)）/ 禁区（DXZE<0+CD下/忑）**。只在**主战场**重仓，鸡肋可持有不宜过早介入，执念观察，禁区坚决不碰。
>
> **第2步 · 选点（§6.1 四层漏斗，DXCD→护型→介入时机）：** 在选定的区域内，用四层漏斗选具体介入点——**第1层 DXCD（上/忐 首选，忠 谨慎）、第2层 护型（甲乙己）、第3层 介入时机（DJA丘等1/2/3 + BSHA5）、第4层 微观（升管/垒升/剔诱多）**。
>
> **关系：** 四类区域（§6.11）是**顶层选区**（决定"做不做、重仓还是轻仓"），四层漏斗（§6.1）是**底层选点**（决定"具体买哪个、何时买"）。**先选区（大势/土质），再选点（护型/时机/微观）**——不要在恶土（禁区/执念）里谈微观，这正是 §6.1 操作理念"抓大看小"的核心。
...
> [!IMPORTANT]
> **四层漏斗总纲（原§6.3 四层漏斗定型操作规则，并入6.1）：** 四层漏斗把可操作范围收敛到极小，与上方核心框架表一致。
...
#### 6.1.1 用户操作理念：沃土→甲乙己不破DJB→升管→垒升→剔诱多（2026-09-16 定稿）
...
| 层 | 理念 | 对应已验证结论 | 数据 |
|:--|:--|:--|:--|
| ① 沃土 | DXZE>0 + DXCD上/忐 | §6.11 主战场 | +4.98pp（23.6%→28.6%）|
...
| ⑤ 剔诱多 | 跌连-非触顶 | §6.8.3 | 24.9% 最明确诱多，退出 |
...
> **⚠️ 唯一需注意的取舍：** 沃土（35.6%）→ 甲乙己 → 升管 → 垒升 → 剔诱多，可操作范围收敛到极小，**交易频率低（§6.6 每年仅 10-14 次/股），但每次胜率高**。这是"少而精"的代价，需接受。

> **一句话：** 此理念 = §5.2.9"抓大看小"五层决策框架的完整落地——**先大势（DXZE>0）→ 沃土（DXCD上/忐）→ 主升区（DXZC>0）→ 护型（甲乙己，不破DJB不出）→ 微观（升管/垒升/剔诱多）**，每层都已有全量数据支撑，非新理念。

##### 6.1.1.1 操作逻辑明确化：沃土 + DXZC>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）
...
| + 沃土（DXZE>0+CD上/忐） | ~73% | +4.98pp | §6.11 主战场 |
...
>
> **⚠️ 注意：** 沃土+甲乙己把样本收敛到极小（§6.6 每年仅 10-14 次/股），再叠加"升连涨幅≥2%"（样本56万），可操作频率进一步降低——但每次胜率极高（92.75%）。这是"少而精"的极致形态，需接受低交易频率换取高胜率。
...
>
> **⚠️ 忠的归属（2026-09-16 统一）：** 忠 归**主战场**（DXZE>0）/ **鸡肋**（DXZE<0），遵循 VBA 仓日类定义（忠 ∈ {上,忐,忠}，见 §6.11）。但忠 = DXZC>0 且 DXZD<0（价格在 DJD=EMA60 之下），**上升受限、升连弱（20-22%）**——操作时需谨慎，**必须叠加"升连≥2%"（§5.3.9）才介入**，否则忠的升连不提升盈利性。上/忐 仍是首选（升连强、持久），忠 是次选（可做但需谨慎）。

### 6.3 第2层：DXZC>0 区域护型选择——定型操作的核心层
> [!IMPORTANT]
...
> - **DXCD=忠（DXZC>0 但 DXZD<0）未覆盖 7.7%**——**已补全操作建议：谨慎介入**（归主战场/鸡肋，遵循 VBA 定义，见 §6.11）。忠虽是 DXZC>0，但 DXZD<0 意味着价格在 DJD(EMA60) 之下，上升受限、升连弱——**需叠加升连≥2%（§5.3.9）才介入**，否则忠的升连不提升盈利性。
>
> **✅ 补全后流程图覆盖：** DXCD=上/忐（约40%）按护型操作；DXCD=忠（约8%）谨慎介入（需叠加升连≥2%）；DXCD=中/下/忑（约52%）明确回避不属于操作范围。**每个柱都能找到对应操作指南（或明确的不操作指令）。**

### 6.6 定型操作期望收益（2026-09-08 验证，BSHA 用列19修正后）：
...
#### 6.7.2 低波池（Qe+Qin+Qd+Qif+Qst，3732只）：
...
### 6.8 操作区域"少而精"原则——不要随便退出，阴柱可介入，仅诱多退出（2026-09-08 验证）：
...
### 6.9 股性分析：持有条件比例是否定义股票性格（2026-09-08 验证）：
...
### 6.10 入管依据验证（旧准则）：DTZA 持续天数（升管/跌管）作为入管信号（2026-09-11 验证）：
...
> **结论：** 四类区域的数学基础 = **均线系统几何结构**。方向由长均线（EMA60）决定（P>EMA60=升势，P<EMA60=跌势），待定状态（中偏升/忠偏跌）由"跌破/站上短均线但长均线方向相反"决定，强度由均线排列（多头/空头）决定。**DXAB 在各区域内做优选，不改变区域划分。** 此数学框架是 §6.11 概率划分的理论基石。


#### 6.11.1 四类区域全集划分：禁区/执念/鸡肋/主战场（2026-09-16 定稿）
...
> [!IMPORTANT]
> **升连位置护型发现（2026-09-08，见2.4.2.10）：** DXZC>0 时升连位置，甲（47.7%）最多、乙（41.0%）次之。**但按 DXCD 分化：DXCD=上 时乙反而最多（48.4% > 甲38.3%），且甲/乙都强（33%+）可介入；DXCD=忐 时甲更强（27.6%）；DXCD=忠 时升连弱（20-22%），不可靠。** 升连介入优先选 DXCD=上（甲/乙都强）；DXCD=忠 谨慎（归主战场/鸡肋，但升连弱，需叠加升连≥2%才介入，见 §6.11）。
