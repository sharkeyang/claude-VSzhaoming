...
| 模式 | 样本 | 次日P3% | 次日P5% | 均高幅 |
|:--|--:|--:|--:|--:|
| **扩管（管宽>60）** | 5,480,586 | **23.66%** | **11.77%** | **2.09%** |
| **平管（管宽0~60）** | 8,060,684 | 19.72% | 8.23% | 1.79% |
| **窄管（管宽<0）** | 6,738,892 | **18.87%** | **7.51%** | 1.79% |

> **✅ 扩管（23.66%）> 平管（19.72%）> 窄管（18.87%）**——窄管管宽过小，不会形成有效上涨形态，应回避。
...
| 模式 | 次日P3% | 次日P5% | 操作 |
|:--|--:|--:|:--|
| **扩管（管宽>60）** | **23.66%** | **11.77%** | 垒升典型，可介入 |
| **平管（管宽0~60）** | 19.72% | 8.23% | 可介入 |
| **窄管（管宽<0）** | **18.87%** | **7.51%** | **应回避**（管宽过小，无有效上涨形态）|

> **✅ 管中模式区分度大：** 扩管（23.66%）> 平管（19.72%）> 窄管（18.87%），差4.79pp。**窄管应回避**——这是旧准则（DTZA≥3）没有的维度。
...
> **⚠️ 三层漏斗（2026-09-18 统一）：** 原"四层漏斗"（真假正交/大区域/护型过滤/介入时机）中，**真假正交（DXCD）与大区域（DJC丘DXZC>0）是同一维度**（DXCD隐含DXZC），合并为**第1层 选土（四类区域）**。三层漏斗 = **第1层选土（四类区域，DXZE×CD）→ 第2层护型选择（选护型）→ 第3层介入时机（选时机）**。

> **🎯 使用顺序（2026-09-16 澄清，2026-09-20 合并）：** 第六章是一个**三层漏斗**，第1层就是选区（四类区域），使用顺序是漏斗内部的自然顺序：
...
> - **关系：** 第1层选土（§6.2 四类区域）决定"做不做、重仓还是轻仓"，第2/3层（护型/时机）决定"具体买哪个、何时买"。**先选土（大势/土质），再选护型/时机（微观）**——不要在恶土（禁区/执念）里谈微观，这正是 §6.1.1 操作理念"抓大看小"的核心。

**操作细则（三层漏斗把可操作范围收敛到极小）：**

```
第1层 选土：四类区域（DXZE×CD），主战场重仓/鸡肋持有/执念观察/禁区不碰
   ↓
第2层 护型选择：DXAB广义正交，只做己/甲/乙(ZA>0)；乙(ZA<0)/丙 持仓或退出；戊(ZA>0) 观察；丁/戊(ZA<0) 放弃
   ↓
第3层 介入时机：DJA丘（日ZA>0）+ 日等型 + BSHA5 触发器
```

> **三层漏斗 vs 五层理念（§6.1.1 抓大看小）的关系：** 两套框架互补，不冲突。
> - **三层漏斗**（第1层选土→第2层护型选择→第3层介入时机）= 操盘计划的**选点框架**（决定具体买哪个、何时买）。
> - **五层理念**（§6.1.1 抓大看小：大势→沃土→主升区→护型→微观）= **决策顺序**（先大势再微观，抓大看小）。
> - **关系：** 五层理念是"抓大看小"的完整决策链条（从大势到微观），三层漏斗是"选点"的具体框架（选区域→选护型→选时机）。**先按五层理念抓大看小（大势→沃土→主升区），再按三层漏斗选点（护型→时机）**。详见 §6.1.1。
...
> **升管分类（用户，2026-09-19）：** 主要分为三类——**窄管**（管宽过小，不会形成有效上涨形态）、**扩管**（需要升管不断连升，从而推高顶）、**平管**（主要平移，包括离顶未破DJA或贴近DJA、阴阳交错等）。
...
> **✅ 扩管（23.66%）> 平管（19.72%）> 窄管（18.87%）**，差 4.79pp——**扩管 次日P3% 最高（需升管连升推高顶），窄管 最低（管宽过小，不会形成有效上涨形态，应回避）**。
...
> **管宽分类铁律：** 扩管（管宽>60）/平管（管宽0~60）/窄管（管宽<0）**必须用管宽哼JA（col23）**，不是宽哼JC（col24）。管宽哼JA 只在升管（日ZA>0）时有值。
...
> - **管宽哼JA 定义（VBA权威）：** 管宽哼JA = (最近5日最高价 / EMA5 − 1) × 100，即**最近5日最高价相对 EMA5 的偏离百分比**。**管宽哼JA 就是衡量升宽的变量**（用户确认，2026-09-20）——扩管>60/平管0-60/窄管<0 用它分类。极端值（>60）主要来自北交所高波动股，不影响升宽衡量逻辑。
> - **管宽哼JA 无负值**（最高价通常高于均线），升管/跌管分布几乎一致（0-30 占 96%+），**阈值可共用**（扩管>60/平管0-60）。
> - **跌管无"窄管<0"**——跌管的窄管/转弱信号需重新定义（**待办**，见 §6.8.4）。

#### 6.8.1 等3：成熟升管（站上DJA 3柱+）

> **等3 是主介入区**（样本最大，n=6,417,364，基线 22.99%）。管宽哼JA 是决定性维度：**扩管>60 极强**（升连 76.49%），平管0-60 接近基线。

**已验证数据（甲乙己，n=6,417,364，基线 22.99%）：**

| 柱排 | 平管0-60 | 扩管>60 | 触顶 | 未触顶 |
|:--|:--|:--|:--|:--|
| 升连 | 24.99% (n=2,292,014) | **76.49%** (n=15,140) | 25.45% (n=2,220,735) | 19.76% (n=335,978) |
| 升吞 | 24.96% (n=454,172) | 62.23% (n=548) | 24.93% (n=417,192) | 21.19% (n=106,456) |
| 升孕 | 21.60% (n=836,076) | 58.30% (n=1,959) | 21.45% (n=805,286) | 17.94% (n=147,862) |
| 人排 | 22.67% (n=1,477,922) | 53.54% (n=5,183) | 22.62% (n=1,419,269) | 19.20% (n=266,801) |
| 跌连 | 21.68% (n=480,505) | 49.81% (n=2,058) | 21.57% (n=473,645) | 18.32% (n=72,537) |
| 跌孕 | 20.78% (n=85,256) | 46.12% (n=232) | 20.60% (n=83,856) | 18.08% (n=11,483) |
| 跌吞 | 19.70% (n=50,020) | 55.17% (n=145) | 19.62% (n=50,130) | 16.53% (n=6,134) |

> **结论：**
> ① **扩管>60 极强**（升连 76.49%）——管宽哼JA 是决定性维度，扩管时升排介入胜率极高；
> ② **触顶 vs 未触顶差约5pp**（升连 25.45 vs 19.76）——触顶是等3 的增强信号；
> ③ **升排 > 人排 > 跌排**——柱排方向在等3 仍有效。
> ⚠️ **小样本谨慎：** 扩管>60 的跌吞（n=145）、跌孕（n=232）样本极小，结论仅供参考。
...
**介入点条件（等3）：**
- **首选：** 扩管>60 + 升排（升连/升吞/升孕）→ 胜率 58-76%，最强介入点。
- **次选：** 平管0-60 + 触顶 + 升排 → 胜率 21-25%，接近基线但升排仍占优。
- **升排推高顶：** 扩管→升排推高顶，可期待连续冲高。
...
- 未触顶 + 跌排（跌连/跌孕/跌吞）→ 胜率 16-18%，最弱，应回避或减仓。
...
| **等3** | 成熟升管 | **扩管>60+升排（58-76%）**；平管+触顶+升排 | 不触顶阳柱→管宽缩小→下破DJA止损；**管宽哼JA空(17.41%)转弱回避** | 17-76% |
| **等4** | 出管/跌管 | 首下破反弹；**等6第二柱阳柱+次日再站DJA(37.37%)**；整体回避 | 跌破DJA无反弹离场 | 20-23% |

> **核心结论：** 介入时机坐标 = **等3 扩管>60 + 升排** 是最强介入点（升连 76.49%）；等2 靠触顶增强；等1 靠第2柱确认（维持升管29.39% vs 回落2.58%）；等4 整体回避（仅等6第二柱阳柱+次日再站DJA 37.37%可介入）。**管宽哼JA（col23）是决定性维度**，扩管>60 时升排介入胜率极高，管宽哼JA空（管宽消失）是转弱信号。
...
> **管宽哼JA 无负值**（最高价通常高于均线），所以跌管**没有"窄管<0"**。跌管的窄管/转弱信号需重新定义（**待办**）。

**② 跌管各护型管宽分类次日P3%（基线 17.82%，n=5,911,829）：**
| 分类 | 丙 | 戊 |
|:--|:--|:--|
| **扩管>60** | **54.68%** (n=1,046) | 47.37% (n=19) |
| 平管0-60 | 20.95% (n=787,777) | 20.28% (n=123,019) |
| 管宽空 | 16.98% (n=468,854) | 17.29% (n=4,531,114) |

> **🔍 反直觉发现：跌管（丙丁戊）扩管>60 也极强（丙 54.68%），与升管（甲乙己扩管>60 升连 76.49%）类似。** 但跌管基线仅 17.82%（vs 升管 22.99%），整体偏弱。**跌管扩管>60 可能是"超跌反弹"机会**——用户确认：跌管扩管>60 也介入反弹。⚠️ 丙扩管>60 样本小（n=1,046）、戊更小（n=19），谨慎。
...
**镜面映射操作逻辑（跌管 丙丁戊）：**
- **升管介入（扩管>60 升排）→ 跌管回避/退出**（跌管整体偏弱，基线 17.82%）。
- **跌管扩管>60 → 超跌反弹介入**（丙 54.68%），轻仓试仓反弹。
- **跌管管宽哼JA空 → 转弱**（16.98%/17.29%），回避。
- **跌管各等型均偏弱（15-19%）**，无强介入区，以回避为主。

> **⚠️ 待办：** 跌管（丙丁戊）的**窄管/转弱信号**需重新定义（管宽哼JA 无负值，无"窄管<0"）。当前用"管宽哼JA空"近似，后续需跑数据验证跌管窄管的定义。
