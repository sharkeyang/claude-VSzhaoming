...
for i, f in enumerate(files):
    ...
    for j in range(len(df)):
        ...
        if cond and not in_cycle:
            in_cycle = True
            # 均线价 ≈ 前收盘价（条件满足时价格刚站上均线）
            if j > 0 and close[j-1] > 0:
        ...
        if in_cycle and (not cond or j == len(df)-1):
            # 均线价 ≈ 前收盘价（条件不满足时价格刚跌破均线）
            if j > 0 and close[j-1] > 0:
                exit_price = close[j-1]
            else:
                exit_price = open_p[j] if open_p[j] > 0 else close[j]
...
with open('____temp/board_full_ma.txt', 'w', encoding='utf-8') as f:
    f.write(result)
print(f'\n结果已保存: ____temp/board_full_ma.txt')