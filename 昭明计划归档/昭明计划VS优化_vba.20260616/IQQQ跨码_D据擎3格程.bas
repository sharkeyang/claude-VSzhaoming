...
'                End If
                ...
                With .Cells(1, 位谕of月道尊比柱)
                        ...
                        End If
                End With
                With .Cells(1, 位谕of月道横类)
                        If InStr(.Value, "安") > 0 Then
...
'########################################################################################
        ...
        With WS.Cells(行号遍历, 1)
                ...
                With .Cells(1, 位谕of周波临涨幅)
                        ...
                        If .Value >= WS.Cells(行号遍历, 位谕of周波均涨幅五) Then .HorizontalAlignment = xlRight
                End With
                With .Cells(1, 位谕of周波临高幅)
                        .HorizontalAlignment = xlLeft
                        If .Value >= WS.Cells(行号遍历, 位谕of周波均高幅十) Then .HorizontalAlignment = xlRight
                End With
...
'                    End If
                    '------------------------------------------------------------------------
                    '整体上色：STBASE结算工具_衍生色度按鳄鱼
                    '------------------------------------------------------------------------
                            For Y = 位谕始of族周层 To 位谕终of族周层
                                With .Cells(1, Y)
                                    值TintAndShade = .Interior.TintAndShade
                                    .Interior.Color = STBASE结算工具_衍生色度按鳄鱼(WS.Cells(行号遍历, 位谕of周层三鳄).Value)
                                    .Interior.TintAndShade = 值TintAndShade
                                End With
                            Next
                '========================================================================
                ...
                If .Cells(1, 位谕of周层类IS管) <> "" Then
                        ...
                        If .Cells(1, 位谕of周管宽哼JA).Value >= 15 Then
                            ...
                            With .Cells(1, 位谕of周管撤哼JA)
                                '相对回撤
                                If .Value >= WS.Cells(行号遍历, 位谕of周管宽哼JA) Then
                                    .Font.Color = 常色八红
                                ElseIf .Value >= 0.5 * WS.Cells(行号遍历, 位谕of周管宽哼JA) Then .Font.Italic = True
                                    .Font.Color = 常色八黄
                                End If
                                '绝对回撤
...
'########################################################################################
        ...
        With WS.Cells(行号遍历, 1)
                ...
                If .Cells(1, 位谕of日类BTZE) < 0 Then
                        ...
                        If .Cells(1, 位谕of日层宽哼JC).Value >= 15 Then
                            ...
                            With .Cells(1, 位谕of日层撤哼JC)
                                '相对回撤
                                If .Value >= WS.Cells(行号遍历, 位谕of日层宽哼JC) Then
                                    .Font.Color = 常色八红
                                ElseIf .Value >= 0.5 * WS.Cells(行号遍历, 位谕of日层宽哼JC) Then .Font.Italic = True
                                    .Font.Color = 常色八黄
                                End If
                                '绝对回撤
                                If .Value >= 10 Then .Font.Italic = True
                ...
                With .Cells(1, 位谕of日波上身)
                        If WS.Cells(行号遍历, 位谕of日波BSHR).Value < 1.5 Then
                                .Interior.Color = 常色二灰
                        Else
                            If .Value < 0 Then
                                .Interior.Color = 常色四蓝
...
'                                If Left(WS.Cells(行号遍历, 位谕of周层界), 1) = "_" Then
'                                    .Font.Color = .Interior.Color
'                                    .Font.TintAndShade = -0.1
'                                End If
'                        End With
                ...
                '三鳄
                '========================================================================
'                With .Cells(1, 位谕of周层三鳄)
'                        .Interior.Color = STBASE结算工具_衍生色度按鳄鱼(.Value)
'                End With
                '========================================================================
        End With
        '================================================================================
...
'                '标记：操盘类别
'                With .Cells(1, 位谕of仓类)
'                    .Interior.Color = STBASE结算工具_衍生色度按仓类(.Value)
'                    .Font.Color = .Interior.Color
'                    .Font.TintAndShade = -0.1
'                End With
                '------------------------------------------------------------------------
...
'                        .Comment.text text:=WS.Cells(行号遍历, 位谕of周道乾坤).Value
...
'                '迷你图
'                '----------------------------------------------------------------
'                With WS.Cells(行号遍历, 位谕of周波涨幅迷)
'                        With .SparklineGroups
'                            .Add Type:=xlSparkColumn, SourceData:=WS.Cells(行号遍历, 位谕of周波涨幅4).Resize(1, 5).Address
...
'----------------------------------------------------------------------------------------
'                '========================================================================
'                '废弃原因20251020：该设色方案只关注了WJB（周类范围）与WXAB（上下忐忑）与WJA（区分上与中下）三个因素，而忽略了WJC与更高维的WXCD，导致很可能进入底诱且易被WXZA等小波动影响持仓，导致抓不住主升。
'                '标注：周管
'                '醍醐灌顶20240204：所有【周管族】【周暴族】仅针对【A5】管提供参考，其余可以忽略。
'                '========================================================================
'                '------------------------------------------------------------------------
'                '周类范围：符合
'                '------------------------------------------------------------------------
'                If Left(.Cells(1, 位谕of周层护型), 1) = "√" Or Left(.Cells(1, 位谕of周层护型), 1) = "口"  Then
'                        '----------------------------------------------------------------
'                        '针对【管系】【粒系】设置错位颜色
'                        '----------------------------------------------------------------
'                        If .Cells(1, 位谕of周层类IS管) <> "" Then
...
''                                    With .Cells(1, Y)
''                                        值TintAndShade = .Interior.TintAndShade
''                                        .Interior.TintAndShade = Application.Max(-0.9, 值TintAndShade - 0.3)
''                                    End With
''                                Next
'                        Else
'                                For Y = 位谕始of族周管 To 位谕终of族周管
...
'                        '针对WXAB设置【周层】
'                        '----------------------------------------------------------------
'                        If InStr(.Cells(1, 位谕of周层护型), "忐") > 0 Or InStr(.Cells(1, 位谕of周层护型), "忑") > 0 Or InStr(.Cells(1, 位谕of周层护型), "忠") > 0 Then
'                                For Y = 位谕始of族周层 To 位谕终of族周层
'                                    With .Cells(1, Y)
'                                        值TintAndShade = .Interior.TintAndShade
'                                        .Interior.Color = 常色五青
...
'                                    End With
'                                Next
'                        ElseIf InStr(.Cells(1, 位谕of周层护型), "下") > 0 Or InStr(.Cells(1, 位谕of周层护型), "中") > 0 Then
'                                For Y = 位谕始of族周层 To 位谕终of族周层
'                                    With .Cells(1, Y)
'                                        值TintAndShade = .Interior.TintAndShade
'                                        值TintAndShade = 值TintAndShade - 0.3
...
'                        '设置底色：根据极折
'                        If Right(WS.Cells(行号遍历, 位谕of周龟顶触), 1) = "高" Then
'                             .Interior.Color = 常色主靛
'                        ElseIf Right(WS.Cells(行号遍历, 位谕of周龟顶触), 1) = "撤" Then
'                             .Interior.Color = 常色六橙
'                        ElseIf Right(WS.Cells(行号遍历, 位谕of周龟顶触), 1) = "弹" Then
'                             .Interior.Color = 常色四灰
'                        ElseIf Right(WS.Cells(行号遍历, 位谕of周龟顶触), 1) = "低" Then
'                             .Interior.Color = 常色二灰
'                        End If
'                        '设置字体
'                        If .Value >= 30 Then
...
'                        '设置底色：根据极折
'                        If Right(WS.Cells(行号遍历, 位谕of日龟顶触), 1) = "高" Then
'                             .Interior.Color = 常色主靛
'                        ElseIf Right(WS.Cells(行号遍历, 位谕of日龟顶触), 1) = "撤" Then
'                             .Interior.Color = 常色六橙
'                        ElseIf Right(WS.Cells(行号遍历, 位谕of日龟顶触), 1) = "弹" Then
'                             .Interior.Color = 常色四灰
'                        ElseIf Right(WS.Cells(行号遍历, 位谕of日龟顶触), 1) = "低" Then
'                             .Interior.Color = 常色二灰
'                        End If
'                        '设置字体
'                        If .Value >= 20 Then
...
'                        '-----------------------------------------------
'                        '单独列示
'                        If Len(.Cells(1, 位谕of周龟顶型)) = 0 Or InStr(.Cells(1, 位谕of周龟顶型), "_") > 0 Then
'                            For Y = 位谕始of族周龟 To 位谕终of族周龟
'                                值TintAndShade = .Cells(1, Y).Interior.TintAndShade
'                                .Cells(1, Y).Interior.Color = 常色二灰
'                                .Cells(1, Y).Interior.TintAndShade = 值TintAndShade
'                            Next
'                        '-----------------------------------------------
'                        ElseIf InStr(.Cells(1, 位谕of周龟顶型), "f武") > 0 Then
'                            For Y = 位谕始of族周龟 To 位谕终of族周龟
'                                值TintAndShade = .Cells(1, Y).Interior.TintAndShade
'                                .Cells(1, Y).Interior.Color = 常色四橙
'                                .Cells(1, Y).Interior.TintAndShade = 值TintAndShade
'                            Next
'                        ElseIf InStr(.Cells(1, 位谕of周龟顶型), "e非") > 0 Then
'                            '周级别：向下趋势
'                            For Y = 位谕始of族周龟 To 位谕终of族周龟
'                                值TintAndShade = .Cells(1, Y).Interior.TintAndShade
'                                .Cells(1, Y).Interior.Color = 常色43绿
'                                .Cells(1, Y).Interior.TintAndShade = 值TintAndShade - 0.4
'                            Next
'                        '-----------------------------------------------
'                        ElseIf InStr(.Cells(1, 位谕of周龟顶型), "龙") > 0 Then
'                            For Y = 位谕始of族周龟 To 位谕终of族周龟
'                                值TintAndShade = .Cells(1, Y).Interior.TintAndShade
'                                .Cells(1, Y).Interior.Color = 常色主靛
'                                .Cells(1, Y).Interior.TintAndShade = 值TintAndShade
'                            Next
'                        ElseIf InStr(.Cells(1, 位谕of周龟顶型), "雀") > 0 _
'                            Or InStr(.Cells(1, 位谕of周龟顶型), "虎") > 0 _
'                            Or InStr(.Cells(1, 位谕of周龟顶型), "篪") > 0 Then
'                            For Y = 位谕始of族周龟 To 位谕终of族周龟
'                                值TintAndShade = .Cells(1, Y).Interior.TintAndShade
'                                .Cells(1, Y).Interior.Color = 常色主绿
'                                .Cells(1, Y).Interior.TintAndShade = 值TintAndShade
