...
print(f'\n{"代码":>10} {"股性分":>8} {"年化":>7} {"胜率":>6} {"盈亏比":>6} {"均单次":>7} {"震仓":>6} {"次数":>5} {"HR/涨":>7} {"评级":>6}')
print('-' * 75)
for r in results[:30]:
...
# 输出到CSV
csv_out = os.path.join(TEMP, '..', '_产出物', '股性评分结果.csv')
with open(csv_out, 'w', encoding='utf-8', newline='') as f:
    w = csv.writer(f)
    w.writerow(['代码','股性分','年化','胜率','盈亏比','均单次','震仓','次数','HR涨比','评级'])
    for r in results:
        w.writerow(r)
print(f'\n已保存: {csv_out}')
