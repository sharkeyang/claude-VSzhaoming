...
"""
生成 vba表日策分坏.txt
基于 DXCD × 日护型 转移矩阵，用修正版转坏去向计算转坏率
编码格式：W(转坏率≥30%危险)/_(<30%安全) + 转坏率取整（如 W72、_17）
数据来源：谕组日 CSV（高波池 Qic/Qim/Qit），与VBA中 IQQQ跨码工具_查日策分坏 对应

用法：
    python _产出物/_工具/vba表日策分坏.py
"""
