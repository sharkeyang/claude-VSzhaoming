    ...
    '================================================================================
        ...
        '========================================================================
            ...
            ElseIf UBCID是中股基(CIDL) = True Then
                    谕组(X, 位谕of仓操类) = "操盘ETF类"
                    谕组(X, 位谕of仓限总) = IIf(谕组(X, 位谕of仓限倍) > 0, Application.Max(1, CInt(10000 / 100 / 值今收)), 0)
            '--------------------------------------------------------------------
            '【类别】池金
            ElseIf 谕组(X, 位qt在池) = 常池名金 Then
                    谕组(X, 位谕of仓操类) = "操盘在池"
                    谕组(X, 位谕of仓限单) = (常值仓限底仓额 / 100 / 值今收)
                    谕组(X, 位谕of仓限总) = Application.Max(1, CInt(谕组(X, 位谕of仓限单) * 谕组(X, 位谕of仓限倍)))
            '【类别】池银 池铜
    ...
    For Each 指CIDL In Array("sh000001", "sz399006", "sh000688")
        ...
        If 典指位.Exists(指CIDL) Then
            ...
            If 指冲 <> "" Then
                ...
                If Val(Mid$(指冲, 3)) >= 50 Then 冲高分 = 冲高分 + 1 Else 冲低分 = 冲低分 + 1
            End If
        End If
    Next
...
'========================================================================================
    ...
    If 谕组(X, 位谕of周策分P3) <> "" Then WSTO.Cells(明细行, 8).Value = 谕组(X, 位谕of周策分P3)
    ...
    ElseIf 谕组(X, 位谕of周策分P3) <> "" And Val(Mid$(谕组(X, 位谕of周策分P3), 3)) >= 50 Then
        WSTO.Cells(明细行, 28).Value = "周冲"
    ElseIf 谕组(X, 位谕of日策分P2) <> "" And Val(Mid$(谕组(X, 位谕of日策分P2), 3)) >= 50 Then
...
'========================================================================================
    末行 = 明细行 + 2
    WSTO.Cells(末行, 1).Value = "七、组合调整建议（最小修改方案）"
    WSTO.Cells(末行, 1).Font.Bold = True
    WSTO.Cells(末行, 1).Font.Size = 14
    末行 = 末行 + 1
