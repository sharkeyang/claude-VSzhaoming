...
Option Explicit
' 结算副本路径：由 Python 在结算前设置，STBASE外簿引擎_打开 据此替换
' 硬编码路径，使宏在副本上运行，避免锁住原文件影响用户使用原文件。
' 结算结束后由 Python 复位（清空），不影响其他功能。
Public 常副本路径花册 As String
Public 常副本路径藏库 As String
Public 常是否python调用结算 As Boolean
Public 常结算数量测试 As Integer
'========================================================================================
'调试日志 — 追加写入 D:\zdata\__settle\vba_debug.log，方便定位问题（验证成功后删除）
'========================================================================================
Sub DBG(消息 As String)
    On Error GoTo 日志失败
    '目录不存在则创建
    If Not UTL判断路径存在("D:\zdata\__settle") Then MkDir "D:\zdata\__settle"
    Open "D:\zdata\__settle\vba_debug.log" For Append As #1
    Print #1, Format(Now, "HH:mm:ss") & " " & 消息
    Close #1
    Exit Sub
日志失败:
    '日志失败不影响主流程（磁盘满、权限等问题时静默跳过）
    On Error GoTo 0
End Sub
'########################################################################################
'########################################################################################
...
'========================================================================================
'常结算数量测试设置 — 由 Python 在测试模式下调用，限制结算条数（0=不限）
'========================================================================================
Sub STBASE外簿引擎_设置常结算数量测试(数量 As Integer)
    DBG "设置常结算数量测试=" & 数量
    常结算数量测试 = 数量
End Sub
'========================================================================================
'结算副本路径设置 — 由 Python 在跑宏前调用，告诉宏去开副本而非原文件
'========================================================================================
Sub STBASE外簿引擎_结算副本路径设置(花册路径 As String, 藏库路径 As String)
    DBG "设置副本路径: 花册=" & 花册路径
    DBG "设置副本路径: 藏库=" & 藏库路径
    常副本路径花册 = 花册路径
    常副本路径藏库 = 藏库路径
    常是否python调用结算 = True
    DBG "常副本路径花册已设置=" & (常副本路径花册 <> "")
    DBG "常副本路径藏库已设置=" & (常副本路径藏库 <> "")
End Sub
'========================================================================================
'结算副本路径复位 — 由 Python 在结算完成后调用，恢复默认（不影响其他功能）
'========================================================================================
Sub STBASE外簿引擎_结算副本路径复位()
    DBG "复位副本路径（清空）"
    常副本路径花册 = ""
    常副本路径藏库 = ""
    常是否python调用结算 = False
...
Function STBASE外簿引擎_定位(簿令 As Excel.Workbook, ByVal 簿名全径 As String) As Boolean
    '★ 结算模式：已设置副本路径，跳过定位，让打开函数去开副本
    If 常副本路径花册 <> "" And 簿名全径 = 常外簿花册 Then Exit Function
    If 常副本路径藏库 <> "" And 簿名全径 = 常外簿藏库 Then Exit Function
    Dim 簿名 As String
    簿名 = Dir(簿名全径)
    ' 前台：当前Excel实例
...
Function STBASE外簿引擎_打开(簿令 As Excel.Workbook, ByVal 簿名全径 As String, Optional 是否后台 As Boolean = False) As Boolean
    ' 结算流程：若设置了副本路径，使用副本而非原文件，避免锁住用户文件
    If 常副本路径花册 <> "" And 簿名全径 = 常外簿花册 Then
        簿名全径 = 常副本路径花册
    ElseIf 常副本路径藏库 <> "" And 簿名全径 = 常外簿藏库 Then
        簿名全径 = 常副本路径藏库
    End If
    DBG "STBASE外簿引擎_打开: 簿名全径=" & 簿名全径 & " 常副本路径花册=" & 常副本路径花册 & " 常副本路径藏库=" & 常副本路径藏库
    STBASE外簿引擎_打开 = False
...
Function STBASE外簿引擎_链接(WS目标 As Worksheet, ByVal 簿名全径 As String, ByVal 表名 As String) As Boolean
    ...
    If 是否打开 = False Then
        ...
        If 是否打开 = False Then
            Application.StatusBar = "[外簿链接] 定位+打开失败：" & Dir(簿名全径) & " → " & 表名
            Debug.Print "[外簿链接] 定位+打开失败：" & 簿名全径 & " → " & 表名
            Exit Function
        End If
    End If
    If UTL判断工表存在(表名, 簿令) = False Then
        Application.StatusBar = "[外簿链接] 表不存在：" & Dir(簿名全径) & " → " & 表名
