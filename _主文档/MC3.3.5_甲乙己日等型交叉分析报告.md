...
>
> **关键增强信号（§6.9 + §6.1.1）：** DJA丘等1（31.4%）+ BSHA5（39.4%）+ **升连≥2%（92.75%）**——升连必须≥2%，否则无优势（首升连无限制68.84%≈基线，涨幅<2% 52.07%反而更差）。
>
> **阅读指引：** 第一~五章是各维度分析（正交动力学/护型/等型/三态理论），第六章是决策收敛（§6.1 定型操作 + §6.2.1 四类区域 + §6.1.1 操作理念），第七~八章是跨周期验证 + 数据规范。**核心决策在第六章，前面各章是数据支撑。**

**目录：**

- [DXAB广义正交护型 × 日等型 交叉分析报告](#dxab广义正交护型--日等型-交叉分析报告)
  - [一、基础框架](#一基础框架)
  ...
  - [二、DXAB正交动力学](#二dxab正交动力学)
    ...
    - [2.5 DXAB 广义正交层是必要的（不是冗余）](#25-dxab-广义正交层是必要的不是冗余)
      ...
      - [2.5.5 结论：DXAB 作为算法一环是必要的：](#255-结论dxab-作为算法一环是必要的)
    - [2.6 BSHA 对 DXZC/DXAB 的交叉关系](#26-bsha-对-dxzcdxab-的交叉关系)
      ...
      - [2.6.4 DXAB\<0 时，BSHA 随 BTAB 值演变：](#264-dxab0-时bsha-随-btab-值演变)
      - [2.6.5 结论：](#265-结论)
    - [2.7 突破日护型 → 升势坡度（长期讨论课题，2026-09-08 首验）](#27-突破日护型--升势坡度长期讨论课题2026-09-08-首验)
      ...
      - [2.7.2 升连位置 DXAB 护型分布（2026-09-08）](#272-升连位置-dxab-护型分布2026-09-08)
        ...
        - [2.7.2.3 结论：升连位置护型分布](#2723-结论升连位置护型分布)
    ...
    - [2.8 DXAB 由负转正信号的综述](#28-dxab-由负转正信号的综述)
      ...
      - [2.8.7 综合结论：DXAB 由负转正信号的操作指南](#287-综合结论dxab-由负转正信号的操作指南)
      - [2.8.8 DXAB 由负转正后的特权（2026-09-08 验证）](#288-dxab-由负转正后的特权2026-09-08-验证)
        ...
        - [2.8.8.4 结论：DXAB 负转正后的特权取决于 DXZE](#2884-结论dxab-负转正后的特权取决于-dxze)
  ...
  - [四、等型分析](#四等型分析)
    ...
    - [4.3 等2：关注柱排方向](#43-等2关注柱排方向)
      ...
      - [4.3.2 等2阳柱的划分（升孕 vs 升排 vs 跌排，2026-09-08 验证）](#432-等2阳柱的划分升孕-vs-升排-vs-跌排2026-09-08-验证)
        ...
        - [4.3.2.5 结论：等2阳柱的划分](#4325-结论等2阳柱的划分)
        - [4.3.2.6 诱多识别（头脑风暴，2026-09-08 验证）：](#4326-诱多识别头脑风暴2026-09-08-验证)
        - [4.3.2.7 结论：等2阳柱的诱多识别](#4327-结论等2阳柱的诱多识别)
      ...
      - [4.3.3 等2内部的中符末位划分：末位C vs 末位DEF（2026-09-10 验证）](#433-等2内部的中符末位划分末位c-vs-末位def2026-09-10-验证)
        ...
        - [4.3.3.4 结论：等2的完整操作框架](#4334-结论等2的完整操作框架)
    ...
    - [4.7 综合结论：各分支涨跌判断](#47-综合结论各分支涨跌判断)
  ...
  - [五、管态理论：升管/跌管/转换态](#五管态理论升管跌管转换态)
    - [5.1 柱与DJA三态理论框架（用户理论，2026-09-10 验证）](#51-柱与dja三态理论框架用户理论2026-09-10-验证)
      ...
      - [5.1.4 入管准则（2026-09-10 旧准则 + 2026-09-17 新准则）](#514-入管准则2026-09-10-旧准则--2026-09-17-新准则)
        - [5.1.4.1 新准则：触哼入管 + 管中模式 + 出管判定（2026-09-17）](#5141-新准则触哼入管--管中模式--出管判定2026-09-17)
        - [5.1.4.2 旧准则：DTZA≥3（2026-09-10）](#5142-旧准则dtza32026-09-10)
          - [5.1.4.2.1 升管入管判定（DTZA≥3）：](#51421-升管入管判定dtza3)
          - [5.1.4.2.2 跌管入管判定（DTZA≤-3，镜面映射）：](#51422-跌管入管判定dtza-3镜面映射)
  ...
  - [六、操盘计划](#六操盘计划)
    ...
    - [6.2 第1层：选土（四类区域，DXZE×CD）](#62-第1层选土四类区域dxzecd)
      - [6.2.1 四类区域：数学基础 + 全集划分](#621-四类区域数学基础--全集划分)
        ...
        - [6.2.1.8 四类区域全集划分：禁区/执念/鸡肋/主战场（2026-09-16 定稿）](#6218-四类区域全集划分禁区执念鸡肋主战场2026-09-16-定稿)
    - [6.3 第2层：护型选择（DXAB）——定型操作的核心层](#63-第2层护型选择dxab定型操作的核心层)
    - [6.5 定型操作规则（最终收敛）](#65-定型操作规则最终收敛)
      - [6.5.1 护型操作决策流程图（可视化总结，2026-09-10，2026-09-11 补全DXCD分支）：](#651-护型操作决策流程图可视化总结2026-09-102026-09-11-补全dxcd分支)
      - [6.5.2 流程图遍历验证（随机10个代码，2026-09-11）：](#652-流程图遍历验证随机10个代码2026-09-11)
      - [6.5.3 操作区域"少而精"原则——不要随便退出，阴柱可介入，仅诱多退出（2026-09-08 验证）](#653-操作区域少而精原则不要随便退出阴柱可介入仅诱多退出2026-09-08-验证)
    - [6.6 算法期望分析](#66-算法期望分析)
      - [6.6.1 收益估算与风险提示](#661-收益估算与风险提示)
      - [6.6.2 定型操作期望收益（2026-09-08 验证，BSHA 用列19修正后）](#662-定型操作期望收益2026-09-08-验证bsha-用列19修正后)
      - [6.6.3 持有条件样本占总天数比例（2026-09-08 验证）](#663-持有条件样本占总天数比例2026-09-08-验证)
        - [6.6.3.1 高波池（Qic+Qim+Qit，3451只）](#6631-高波池qicqimqit3451只)
        - [6.6.3.2 低波池（Qe+Qin+Qd+Qif+Qst，3732只）](#6632-低波池qeqinqdqifqst3732只)
    - [6.7 第3层：工具汇总](#67-第3层工具汇总)
      - [6.7.0 框架总览：操盘工具 + 坐标定位工具](#670-框架总览操盘工具--坐标定位工具)
      - [6.7.1 升管管宽分类：原则 + 分布统计（2026-09-20 全量验证）](#671-升管管宽分类原则--分布统计2026-09-20-全量验证)
      - [6.7.2 升管判断（状态机口径，触哼入升管）](#672-升管判断状态机口径触哼入升管)
        - [6.7.2.1 升管定义（状态机口径，只针对 DXAB 甲乙己，不考虑 DXZC）](#6721-升管定义状态机口径只针对-dxab-甲乙己不考虑-dxzc)
        - [6.7.2.2 管中模式（扩管/平管/窄管）](#6722-管中模式扩管平管窄管)
        - [6.7.2.3 出管判定（状态机口径）](#6723-出管判定状态机口径)
      - [6.7.3 垒升判断（柱体≥2%，§5.3）](#673-垒升判断柱体253)
        - [6.7.3.1 垒升判定：在垒升位置介入的获利概率](#6731-垒升判定在垒升位置介入的获利概率)
        - [6.7.3.2 持有策略：升连成群结队，可期待连续](#6732-持有策略升连成群结队可期待连续)
        - [6.7.3.3 启动信号：升吞转升连是最强启动（§5.3.6）](#6733-启动信号升吞转升连是最强启动536)
        - [6.7.3.4 镜面映射的全局指导：先按 DXZC 分类，再按升连/跌连分类（§5.3.5.2）](#6734-镜面映射的全局指导先按-dxzc-分类再按升连跌连分类5352)
        - [6.7.3.5 持续持有 vs 波段（§5.3.1.1）](#6735-持续持有-vs-波段5311)
    - [6.8 第3层：进出时机（选时机）](#68-第3层进出时机选时机)
      - [6.8.0 介入点/止损条件汇总表](#680-介入点止损条件汇总表)
      - [6.8.1 等3：成熟升管（站上DJA 3柱+）](#681-等3成熟升管站上dja-3柱)
      - [6.8.2 等1：升管起点（上破DXZA=1第一柱）](#682-等1升管起点上破dxza1第一柱)
      - [6.8.3 等2：升管早期（站上DJA 2柱）](#683-等2升管早期站上dja-2柱)
      - [6.8.4 等5：出管/跌管区域（乙DXZA\<0）](#684-等5出管跌管区域乙dxza0)
      - [6.8.5 跌管（丙丁戊）镜面映射](#685-跌管丙丁戊镜面映射)
      - [6.8.6 主升必赢：高抛低吸 + 行乞心态（2026-09-24 验证）](#686-主升必赢高抛低吸-行乞心态2026-09-24-验证)
    - [6.9 旧版介入时机](#69-旧版介入时机)
      - [6.9.1 介入时机（DJA丘 + 日等型 + BSHA5）](#691-介入时机dja丘--日等型--bsha5)
    - [6.12 旧内容：操盘决策速查（介入点/退出/流程/印证/收益）](#612-旧内容操盘决策速查介入点退出流程印证收益)
      - [6.12.0 介入点](#6120-介入点)
        - [6.12.0.1 主介入点（高胜率，可主动介入）](#61201-主介入点高胜率可主动介入)
        - [6.12.0.2 次介入点（中胜率，试仓）](#61202-次介入点中胜率试仓)
        - [6.12.0.3 超跌反弹介入点（5.3.3 独有，2026-09-08 补充）](#61203-超跌反弹介入点533-独有2026-09-08-补充)
        - [6.12.0.4 观察点（低胜率，仅观察）](#61204-观察点低胜率仅观察)
        - [6.12.0.5 与 5.3.3 位置决策表相互印证](#61205-与-533-位置决策表相互印证)
      - [6.12.1 退出情况分类（按转坏信号强度，从轻到重）](#6121-退出情况分类按转坏信号强度从轻到重)
      - [6.12.2 完整操盘流程（分情况指南）](#6122-完整操盘流程分情况指南)
        - [6.12.2.1 情况A：DJC丘内（DXZC\>0）——主战场](#61221-情况adjc丘内dxzc0主战场)
        - [6.12.2.2 情况B：DJC丘外（DXZC\<0）——观察区](#61222-情况bdjc丘外dxzc0观察区)
...
> **🔗 本章结论 → 第六章入口：** DJC丘生命周期（甲启动→乙主升→丙预警）是第六章"介入时机"的底层依据——**DJC丘首/初介入选甲/己（§6.9 等1 31.4%），主升持有乙，顶部/尾部 DXZA>0 与 DXZB>0 双降警惕退出（§6.5 规则6）**。DJC丘（DXZC>0）是四类区域"主战场"的前提（§6.2.1）。
...
#### 2.5.4 假正交识别规则（DXAB>0 + DXCD 中/下/忑）：
...
#### 2.6.4 DXAB<0 时，BSHA 随 BTAB 值演变：
...
> - **操作含义：** 升连位置 DXCD=上 时乙/甲都强（33%+）可介入；DXCD=忐 时甲更强（27.6%）；DXCD=忠 时升连弱（20-22%），不可靠。
>
> **⚠️ 涨幅限制（2026-09-16 补充）：** 本节"升连位置"数据**未区分涨幅**。实际介入必须叠加"升连≥2%"（§5.3.8）——首升连无限制（68.84%）≈ 基线（68.86%）无优势，涨幅<2%的升连（52.07%）反而更差，**升连+涨幅≥2% 才达 92.75%**。上表 P(≥3%) 是"升连位置"口径，实际介入需在升连基础上再筛涨幅≥2%。
...
> **🔗 本章结论 → 第六章入口：** DXAB 正交动力学（转负概率、持续时间、快速负交）是第六章"护型选择 + 退出信号"的数据支撑——**DXAB>0 正交是护型可做的前提（§6.3），快速负交（DXCD中/下/忑）是退出预警（§6.5 规则）**。突破日护型坡度（甲最强）辅助介入时机选择。
...
##### 2.8.8.4 结论：DXAB 负转正后的特权取决于 DXZE
...
#### 3.3.1 乙(ZA<0) 转好率的最强预测器 = 日ZA深度（完全单调递增）：
...
> **✅ 管宽列已验证正确（2026-09-16）：** 本节为 2026-09-07 数据，曾担心早于 09-08 列审计（管宽误用 col24=上身，应为 col22=宽哼JC）而用错列。**已用谕组日 CSV 全量重跑验证：col22（宽哼JC）管宽<5 转好率 33.4%、管宽≥10 22.1%，与文档完全一致**——本节管宽数据用的是正确列（col22），**无需重跑**。核心结论（日ZA深度是转好率最强预测器，完全单调递增）用 col13=日ZA，一直正确。
...
> - 己 DXZC>0 时升连占比 **66.1%**（与 §6.5.1 的 66.8% 一致）——**不是"都是升连"**，但升系（升连+升孕+升吞）合计 **82.9%**，绝大多数是升系柱排。
...
> - 己/甲 DXZC>0 时，次日冲高率约 21-23%，与基线（23.6%）接近——**护型本身不保证次日冲高**，需结合介入时机（DJA丘等1/2/3 + BSHA5，§6.9）和升连≥2%（§6.1.1）提升胜率。
...
> **🔗 本章结论 → 第六章入口：** 护型分析（甲/乙ZA>0 可做、乙ZA<0 交叉路口、丙持仓或退出、戊ZA>0 观察、丁/戊ZA<0 放弃）是第六章"护型选择"（§6.3）的直接数据支撑——**只做 甲/乙(ZA>0)/己，其余按角色处理**。DXZC>0 限定（上/忐/忠）对应四类区域"主战场"（§6.2.1）。


## 四、等型分析

> **等型的作用（2026-09-19）：** 等型（日等型/周等型）主要是**与 DXAB 结合，作为坐标工具**——用等型定位 DXAB 护型在 DJA 丘上的位置（介入点/持有期/退出点），辅助判断介入时机。
...
> **🔗 本章结论 → 第六章入口：** 等型分析（等1 介入点 31.4%、等2 30.3%、等3 持有期 29.5%）是第六章"介入时机"（§6.9）的直接数据支撑——**DJA丘等1/2/3 + BSHA5 触发器**。等6/等7（下跌通道）非介入点，应回避。
...
> **课题（用户提出，2026-09-10）：** 研究柱与DJA关系时，要明确三种状态：**升管（DJA之上，DXZA>0）、跌管（DJA之下，DXZA<0）、转换态（上破/下破DJA 1-2柱）**。升管中柱排区分方向（升排=远离DJA，跌排/跌连/跌吞=靠近DJA）；跌管与之镜面映射。入管判定用DTZA≥3；转换态需结合DXAB判别。数据：高波池全量（Qic+Qim+Qit，3451只，2022万行）。**⚠️ 入管判定已升级为触哼入管（§5.1.4.1 新准则），DTZA≥3 为旧准则（§5.1.4.2）。**

#### 5.1.1 三态定义

> **课题（用户提出，2026-09-10）：** 研究柱与DJA关系时，要明确三种状态：**升管（DJA之上，DXZA>0）、跌管（DJA之下，DXZA<0）、转换态（上破/下破DJA 1-2柱）**。升管中柱排区分方向（升排=远离DJA，跌排/跌连/跌吞=靠近DJA）；跌管与之镜面映射。入管判定用DTZA≥3；转换态需结合DXAB判别。**⚠️ 入管判定已升级为触哼入管（§5.1.4.1 新准则），DTZA≥3 为旧准则（§5.1.4.2）。**

| 状态 | 判定 | 说明 |
|:--|:--|:--|
| **升管** | DXAB 甲乙己（DXZB>0）且 形成甲乙己后曾触哼 | 价格在DJA(EMA5)之上。**只有甲乙己才能定义升管**（用户确认，2026-09-20；2026-09-21 状态机口径） |
| **跌管** | DXAB 丙丁戊 + DXZA<0（日ZA<0） | 价格在DJA之下 |
| **转换态** | 上破/下破DJA 1-2柱 | 日ZA=1/2（上破）或日ZA=-1/-2（下破），一般有反复 |

> **升管/跌管判定镜面映射：** 升管入管判定（DTZA≥3）与跌管入管判定成镜面对称。升管中跌排/跌吞=靠近DJA（DJA丘结束），跌管中阳柱/升连/升吞=跌管中DJA丘结束，但**不代表一定转入升管**，升管要等DXZA>0再判定。**⚠️ 入管判定已升级为触哼入管（§5.1.4.1 新准则），DTZA≥3 为旧准则（§5.1.4.2）。**
...
> **本节包含：** 新准则（§5.1.4.1 触哼入管）、旧准则（§5.1.4.2 DTZA≥3）、比较优劣（§5.1.4.3）。**新准则替代旧准则作为入管判定标准。**

##### 5.1.4.1 新准则：触哼入管 + 管中模式 + 出管判定（2026-09-17）

> **课题（用户提出，2026-09-17）：** 在判定升管入管时，会出现虽然满足入管条件，但实际为诱多（或M头）情况，导致很多误判。现对入管判定原则进行重大改变：**不再按DXZA作为入管标准**，而是针对DXAB甲乙己，利用首次触哼作为考虑是否入管的开始，然后判定是否在管中，以及后续是否出管。**只针对DXAB甲乙己（不考虑DXZC）。**

> **🔗 理论渊源：与 Livermore 创新高买入法本质一致（用户指出，2026-09-17）。**
> - **Livermore 创新高买入法核心：** 只在股票创出**新高**（突破前高）时买入——因为创新高意味着上方无套牢盘、无压力，趋势延续，是"顺势"交易的核心。
> - **新准则的本质一致性：** 触哼（触到管上沿 DJA 附近）≈ 突破/创新高；触顶（上符串含A）≈ 创新高。新准则强调"**触哼确认入管**"、"**触顶维持管宽**"——本质是"突破/创新高后顺势买入"，与 Livermore 的"只在创新高时买入"完全一致。
> - **差异：** Livermore 用"前高"（价格创新高），本系统用"触哼/触顶"（价格触到均线管上沿）作为突破确认。**两者都是"突破后顺势"的逻辑，本系统用均线管宽量化了 Livermore 的"新高"概念。**
...
| **管中** | 扩管（管宽≥4+推高顶）/平管（管宽≥4+非推高顶）/窄管（管宽0-4，无有效上涨）| 镜面 |
...
**① 入管判定：升管（状态机口径）vs 旧准则（DTZA≥3）**

| 条件 | 样本 | 次日P3% | 5天P3% | 次日均 | 3天出管% |
|:--|--:|--:|--:|--:|--:|
| 基线_甲乙己 | 10,048,520 | 22.80 | 68.00 | 2.03 | 30.45 |
| 旧准则_DTZA>=3 | 6,414,871 | 23.00 | 70.20 | 2.04 | 23.87 |
| **升管（甲乙己+已触哼）** | 9,771,270 | **22.85** | **68.29** | **2.03** | **29.98** |

> **升管（状态机口径）与基线差异极小**（因97.24%甲乙己都曾触哼）。旧准则（DTZA≥3）次日P3 23.00% 略高。

**② 管中模式：扩管/平管/窄管（升管=甲乙己+曾触哼，全量 7455 文件）**

| 模式 | 样本 | 次日P3% | 均高幅 |
|:--|--:|--:|--:|
| **扩管（管宽≥4+推高顶）** | 2,397,992 | **35.09%** | 2.85% |
| **平管（管宽≥4+非推高顶）** | 2,534,636 | **32.80%** | 2.65% |
| **窄管（管宽0-4）** | 4,797,304 | **11.63%** | 1.32% |

> **✅ 扩管（35.16%）> 平管（33.88%）> 窄管（11.85%）**——窄管管宽过小，不会形成有效上涨形态，应回避。**推高顶 = 触顶 + 升排**（2026-09-20 用户纠正定义）。⚠️ **阈值 5→4（2026-09-21）：** 原数据（扩管38.73%/平管37.04%/窄管14.29%）用阈值5，现改为阈值4——扩管35.16%/平管33.88%/窄管11.85%。
...
**④ 出管边界优化：DJB（EMA12）作为出管边界（2026-09-18 验证）**

> **课题（用户提出，2026-09-18）：** 入管用严格标准（触哼+ZA>0，确保在DJA之上），出管用宽松边界（DJB，容忍更多波动）。即**去掉连阴/阴阳阴提前预警，只用跌破DJB（变丙丁戊）作为出管标准**——既保证入管质量，又避免过早出管。
...
> **✅ DJB出管边界优化可行：**
> - **入管质量高**：入管后5天P3=69.54%（保留旧入管标准，ZA>0确保在DJA之上）。
> - **出管更晚**：DJB 比 DJA 多容忍1.93天，减少频繁出管。
...
>
> **🔗 与 §5.1.4.2 的关系：** §5.1.4.2（旧准则 DTZA≥3）关注**入管后深跌风险**（风险管理），本节关注**入管依据的获利性**（P≥3%）——同一旧准则的两个角度，互补不重复。
>
...
> **课题（用户提出，2026-09-17）：** 在已有旧规则（DTZA≥3）的情况下，为何还要研发新规则（触哼入管）？因为**即使升排、即使上破DJA、即使触顶，还是会出现诱多（或M头）情况**——旧规则无法排除这些诱多。所以研发新规则，并且很大程度解决了这个问题。

**① 为何研发新规则：旧规则无法排除诱多（即使升排/上破DJA/触顶仍会诱多）**
...
> - **这正是研发新规则的原因：** 旧规则（升排/上破DJA/触顶）无法排除诱多，即使最强组合也有17%诱多。新准则（触哼入管）通过"触哼确认 + 管中模式（窄管回避）+ 出管判定（连阴/阴阳阴×触顶）"大幅降低诱多。

**② 入管质量对比（甲乙己）：**

| 指标 | 旧准则（DTZA≥3）| 升管（状态机口径）| 差异 |
|:--|--:|--:|--:|
| 样本 | 6,414,871 | 9,771,270 | 升管覆盖更广 |
| 次日P3% | 23.00 | **22.85** | -0.15pp |
| 5天P3% | 70.20 | **68.29** | -1.91pp |
| 次日均高幅 | 2.04 | **2.03** | -0.01 |
| **3天出管率** | 23.87 | **29.98** | **+6.11pp** |

> **升管（状态机口径）与旧准则（DTZA≥3）对比：** 升管覆盖更广（977万 vs 641万），但次日P3略低（22.85% vs 23.00%），3天出管率更高（29.98% vs 23.87%）——**因升管≈基线（97.24%甲乙己都曾触哼），区分度弱于旧准则**。

**③ 管中模式（新准则特有）：**

| 模式 | 次日P3% | 均高幅 | 操作 |
|:--|--:|--:|:--|
| **扩管（管宽≥4+推高顶）** | **35.16%** | 2.85% | 垒升典型，可介入 |
| **平管（管宽≥4+非推高顶）** | 33.88% | 2.67% | 可介入 |
| **窄管（管宽0-4）** | **11.85%** | 1.34% | **应回避**（管宽过小，无有效上涨形态）|

> **✅ 管中模式区分度大：** 扩管（35.16%）> 平管（33.88%）> 窄管（11.85%），差23.31pp。**窄管应回避**——这是旧准则（DTZA≥3）没有的维度。**推高顶 = 触顶 + 升排**（2026-09-20 用户纠正定义）。⚠️ **阈值 5→4（2026-09-21）：** 原数据（扩管38.73%/平管37.04%/窄管14.29%）用阈值5，现改为阈值4——扩管35.16%/平管33.88%/窄管11.85%。
...
**结论：新准则（触哼入管）在入管质量（P3更高）、误判控制（出管率更低）、管中细分（扩管/平管/窄管）、出管预警（连阴/阴阳阴×触顶）四个方面全面优于旧准则（DTZA≥3）。新准则作为入管判定标准。**
...
> 4. **入管判定：** 升管入管用DTZA≥3（2.9.4），转换态结合DXZB（2.9.5）。**⚠️ 入管判定已升级为触哼入管（§5.1.4.1 新准则），DTZA≥3 为旧准则（§5.1.4.2）。**
...
> 5. **操作规则总纲（与§6.5定型操作规则统一）：** 只做 DXCD=上/忐（DXZC>0）→ 只做 甲/乙(ZA>0)/己 三个护型 → 乙(ZA<0)/丙 持仓不退出等待转确定状态、空仓等待选方向 → 戊(ZA>0) 观察等待转己/甲 → 丁/戊(ZA<0) 直接放弃 → 介入时机 DJA丘+日等型（等1最强）+BSHA5。**§5.2特有的入管判定过滤（DTZA=1不考虑入管/DTZA=2看升连跌孕/DTZA=3排除阴阳阴）+ 升管内跌排减仓 + 乙ZA<0/丙+DTZA≤-2跌管退出 + DXZC<0必须退出** 是§6.5的补充细化（风险管理层）。
>
...
下破DJA后，按DTZA分组（呼应§3.9.4.2跌管入管判定DTZA≤-3）后续走势：
...
> - **与§3.9.4.2完全呼应：** 跌管入管判定DTZA≤-3（跌破DJA三柱）才确认进入跌管。所以**下破DJA后，只要DTZA>-3（未跌破三柱），就还在中间态（选择方向）**，应等确认（可能重新站上），而不是立即退出。
...
> - **升连之后立即跌连 = 0.00%**（所有DTZA区间都是0）——升连链结束后第1柱必然是阴柱形态（跌吞/跌孕），然后过渡到跌连（86.8%）。
> - **DTZA≥4 不影响此规律**——升连后必过渡，与链末DTZA高低无关。
> - **操作含义：** 升连之后不会立刻出现跌连，必然先阴阳相间过渡（跌吞/跌孕）。这与 §5.3.5.1 的"升连后停顿一天（跌孕/跌吞）是升连链延续"一致——升连后的跌孕/跌吞停顿是过渡，不是升连结束。
...
> **镜面映射完全成立：** DXZC>0 时升连成群结队（55.4%）、跌连不成群结队（25.5%）；DXZC<0 时反过来，跌连成群结队（49.4%）。**这印证了阴阳镜面对称原则（MC3.3_C6 §5.0）**——阳区（DXZC>0）升连聚集、跌连孤立；阴区（DXZC<0）跌连聚集、升连孤立。
...
> **🔗 本章结论 → 第六章入口：** 柱与DJA三态理论 + 操作规则优化是第六章"操作理念"（§6.1.1）的直接数据支撑——**升管（触哼入管，§5.1.4.1）、垒升（柱体≥2%，§5.3）、剔诱多（跌连-非触顶，§6.12.4）、不破DJB不出（DXZB退出规则，§6.1.1）** 正是 §6.1.1 操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）的微观层。**本章是第六章操作理念的"微观细节库"。**

### 5.4 阴阳阴（单柱）转跌信号 + 镜面对称甲乙己持有（2026-09-17 全量验证）

> **概念澄清（2026-09-17）：** 单柱阴阳 = **涨幅正负**（col5涨幅>0=阳柱，<0=阴柱），**不是柱排前缀**。柱排前缀 `(升)/(跌)/(人)` 是人排内部偏升/偏跌/中性，不是单柱阴阳。单柱形态 = 柱排字符串形态字母的**最后一个字母**：Q=升连 W=跌连 O=升吞 V=跌吞 o=升孕 v=跌孕（如 `升.尾反孕Qv` 最后字母=v=跌孕，`孕孕vo` 最后字母=o=升孕）。

#### 5.4.1 阴阳阴（单柱）是转跌信号（用户猜想，2026-09-17 验证）

> **课题（用户提出，2026-09-17）：** 判别阴阳阴的类别（单柱：阴柱+阳柱+阴柱）。包括：跌吞+升孕+跌吞（代表向下，类似跌连）、跌孕+升孕+跌吞（代表顶部转向下）、跌孕+升吞+跌孕（更多代表向上）。
...
> **阴阳阴确实存在且是转跌信号：** P3=18.09%（最低之一），均高幅1.72（最低），次日ZA<0=69.10%。阳阳阳最强（P3=25.18%）。**阴阳阴（单柱）是存在的**——之前"阴阳阴=0"的结论是误用柱排分类（跌排/升排）所致，单柱阴阳（涨幅）下阴阳阴样本充足。
...
> - **三种组合转负率差异巨大（93.97% vs 71.25% vs 13.67%），转负概率是核心区分维度。** P3都低于基线（17-17.7% vs 20.49%），说明阴阳阴整体偏弱，但转负率才是关键。
> - **操作含义：** 阴阳阴中，**跌吞+升孕+跌吞 是最强转跌预警**（类似跌连，应回避/退出）；**跌孕+升吞+跌孕 反而是向上信号**（转负仅13.67%，可持有）。判别阴阳阴内部形态，转负概率是决定性维度。

#### 5.4.2 镜面对称：DXZC>0 时甲乙己是持有而非退出（2026-09-17 验证）

> **课题（用户提出，2026-09-17）：** 在 DXZC>0 时主要依赖甲乙（DXZA>0）己，镜面对应于 DXZC<0 时主要依赖丙丁戊（DXZA<0）。已明确 DXZC<0 时绝对不介入丙丁戊，所以按镜面对称原则，当 DXZC>0 时，没有必要在甲乙己退出。
...
> **乙必须拆ZA：** 乙ZA>0 最强（≥3% 24.85%全表最高），乙ZA<0 明显弱（-3.1pp）。**戊拆分意义不大**（差异仅2pp），戊整体就是弱。甲乙己5天持续性优势远大于次日单日（甲71.49%、乙ZA>0 70.41%、己70.75% vs 丁50.46%、戊ZA<0 58.81%）。
...
> **⚠️ 14.71% 是平均掩盖结构，太简单：**
> - **三连跌段内第1柱只有 50.48% 是 DXZA<0**——三连跌第一根阴柱，一半概率 DXZA 还是正的（刚转负或未转负）。到第3柱才 94.42%，第4柱 99.20%，第5柱 99.93%。
> - **按护型：** 甲/己 在三连跌段内 DXZA<0=0%（甲/己 ZA 永不<0，护型定义），丙/丁=100%，戊=91.76%，乙=61.59%。**"三连跌→DXZA<0"只对丙丁成立，对甲己完全不成立，对乙戊是概率性的。**
> - **14.71% 把第1柱（50%）和第3柱（94%）平均了，且没按护型区分。** 真实结构是**位置依赖 + 护型依赖**，单一数字严重失真。


## 六、操盘计划

> **📋 待完成任务（2026-09-20 记录，后续再做）：**
> **把 §6.9（旧版介入时机）、§6.12（旧内容）中有用的部分融合到 §6.8（新框架），其他删掉。**
> - **融合到 §6.8 的内容（5项）：**
>   1. **升管判断**（§6.7.2）：入管判定（触哼入管）、出管判定（连阴/阴阳阴×触顶）、出管边界（DJB宽松）
>   2. **垒升判断**（§6.7.3）：只在垒升位置介入、持有策略、启动信号
>   3. **BSHA5 触发器**（§6.9.1/§6.12.0）：主介入点+BSHA5=39.4%
>   4. **退出规则**（§6.12.1）：放心持有/警惕/升连上影/顶部/提前退出/强制退出 → 融合到 §6.8 止损条件
>   5. **等5超跌反弹**（§6.12.0.3）：等5+ZC>0+乙+BSHA5=33.6%
> - **删除：** §6.9 整体、§6.12 整体（有用的融合到 §6.8 后删除）
> - **被 §6.8 取代删除：** 主介入点（§6.12.0.1）、次介入点（§6.12.0.2）、观察点（§6.12.0.4）
...
> **📊 三层漏斗过滤比例（2026-09-20 补充，§6.6.2 定型操作期望收益逐层递进）：**
>
> | 层 | n | 占基线 | P(≥3%) | 均高幅 |
> |:--|:--|:--|:--|:--|
...
> | **第3层 +DJA丘** | 3,324,398 | 30.0% | 29.7% | 2.5% |
...
> - **交易频率**：每年仅 10-14 次/股（少而精，§6.12.5）
>
> **🔍 三层漏斗的本质（用户定稿 2026-09-22）：**
...
>
> ⚠️ **口径说明：** 上表是"主战场 DXZE>0+上/忐"口径（不含忠）。若按完整三层漏斗（含忠归鸡肋），比例会不同。另 §6.5.3 操作区域（DXCD上/忐 + 甲/乙ZA>0/己/乙ZA<0）占 **35.6%**（含乙ZA<0 的交叉路口）。

> **🎯 使用顺序（2026-09-16 澄清，2026-09-20 合并）：** 第六章是一个**三层漏斗**，第1层就是选土（四类区域），使用顺序是漏斗内部的自然顺序：
> - **第1层 · 选土（§6.2 四类区域，DXZE×CD 二维）：** 判断当前处于哪个区域——**主战场（DXZE>0+CD上/忐）/ 鸡肋（(CD忠，无论ZE)+(DXZE<0+CD上/忐)）/ 执念（(CD中)+(DXZE>0+CD下/忑)）/ 禁区（DXZE<0+CD下/忑）**。只在**主战场**重仓，鸡肋可持有不宜过早介入，执念观察，禁区坚决不碰。
> - **第2层 · 护型（§6.3，甲乙己）：** 在选定的区域内选护型——**只做 甲/乙(ZA>0)/己，其余按角色处理**。
> - **第3层 · 介入时机（§6.9，DJA丘等1/2/3 + BSHA5）：** 叠加微观升管/垒升/剔诱多。
...
> **🎯 四象限分析的目的（2026-09-20 用户确认）：** 把"不破DJB不出"从理念变成**可执行的四象限操作表**——用 DXZA（DJZ 相对 DJA=EMA5 的位置）和 DXZB（DJZ 相对 DJB=EMA12 的位置）决定**持有/退出的具体动作**。核心是区分两个极端：**坚定持有（69.96%）vs 退出（41.62%）**。⚠️ 注意：中间两格区分度弱（60.52% vs 55.90% 差 4.62pp），且"可继续持有"格实际是戊护型（原计划不参与），所以**有效信息主要在"坚定持有 vs 退出"两个极端**，中间两格仅作参考。
...
> **一句话：** 操作逻辑明确为 **沃土 + DXZC>0甲乙己 + 升连涨幅≥2%（垒升）+ 不破DJB不出**，盈利性 5天≥3% 约 **92.75%**（§5.3.8 验证过的最强可执行策略）。**升连必须≥2%** 是决定性修正——不加涨幅限制的升连（68.84%）与基线持平，涨幅<2%的升连（52.07%）反而更差。

### 6.2 第1层：选土（四类区域，DXZE×CD）

> **🎯 选土口诀（2026-09-20 用户定稿）：聚焦上忐，观察中忠，舍弃下忑。**
> - **聚焦上/忐**：沃土首选（升连强、持久，甲/乙ZA>0 占比最高）。
> - **观察中/忠**：中=执念（观察）；忠=观察慎入（归主战场/鸡肋，但升连弱20-22%、DXZC转正是假信号，需叠加升连≥2%才介入，见 §6.2.1）。
> - **舍弃下/忑**：禁区/执念，坚决不碰。

> **第1层选土的核心是四类区域（§6.2.1，DXZE×CD 二维，12格全覆盖）：主战场/鸡肋/执念/禁区。** 只在主战场重仓，鸡肋可持有不宜过早介入，执念观察，禁区坚决不碰。下方 DXCD 选择表是四类区域"土质"维度的数据支撑（上/忐/忠=可做，中/下/忑=不可做，与四类区域一致，**不重复计算**）——DXCD 已隐含在四类区域中，不再作为独立一层。
...
> **数据支撑（详细见 §6.1 核心框架三层漏斗 + §3.9.3 按DXCD拆分）：** DXZC>0（上/忐/忠）时甲/乙ZA>0 主导且持久；DXZC<0（中/下/忑）时丁/戊ZA<0 主导，甲/乙ZA>0 几乎消失（乙ZA>0 仅0.03-0.7%）。**限定 DXZC>0 是有意义的**（见 §3.9.3）。
>
> **⚠️ 忠的归属（2026-09-20 用户定稿）：** 忠 统一归**鸡肋**（无论 ZE），**不归主战场**（2026-09-20 从主战场移到鸡肋）。忠 = DXZC>0 且 DXZD<0（价格在 DJD=EMA60 之下），**上升受限、升连弱（20-22%）**——操作时观察慎入，**必须叠加"升连≥2%"（§5.3.8）才介入**，否则忠的升连不提升盈利性。上/忐 仍是首选（升连强、持久），忠 是次选（可持有但不宜过早介入）。
>
> **⚠️ 忠 观察慎入（用户，2026-09-19，2026-09-20 统一用词）：** 忠 统一归**鸡肋**（无论 ZE），**操作上对忠 观察慎入**——忠虽然单看 DXZC 恒正，但 DXCD<0（忐忠忑）期间 DXZC 反复交叉（97.4%），**忠 的 DXZC 转正是假信号**；且忠 DXZD<0（价格在 EMA60 之下），**上升受限、升连弱（20-22%）、P≥3% 弱**，所以要**等待 DXCD 变为忐或上 再介入**。**只在已变为上/忐 之后考虑介入，不要在忠 里猜测何时变好。**

#### 6.2.1 四类区域：数学基础 + 全集划分
> **课题（用户提出，2026-09-17）：** 之前四类区域是基于概率分析（P≥3% 等统计结果），现在提供基于**数学理论**的分析作为划分基础——即概率分析的基石。核心论断：**只要在 EMA 均线之上就会保持升势**（EMA 数学特性）；用 DJC（EMA26）与 DJD（EMA60）两条均线作为参考判断升势/跌势/待定。
...
> **🔴 DJZ 定义（VBA 权威，结算.bas:212）：** **JZ = (1/3) × JZ + (2/3) × 值结收**——JZ 是**收盘价的 EMA3 平滑值（均基）**，**不是收盘价**。JA/JB/JC/JD/JE 均基于 JZ（均基）迭代。所有 DXZ* 系列变量都是 **DJZ 相对各均线的交叉天数**，不是"价格 P"。
...
> - **忠 = DJC < DJZ < DJD**（DJZ 在 DJC 和 DJD 之间）；**忑 = DJZ < DJC**（DJZ 在 DJC 之下，因 DJC<DJD 故 DJZ<DJD 自动成立）。
...
> - **DXCD<0（忐忠忑）**（161894 段，10546501 天）：段内 DXZC 交叉 **97.4%**，护型变化 **97.4%**——忐/忠/忑 之间反复变换，DXZC 随护型变换而多次交叉。
> - **DXZC 交叉次数分布（DXCD<0）：** 最少 0 次，最多 **138 次**，平均 **5.11 次**，中位 **3 次**；交叉5+次占 **30.2%**——DXZC 转正/转负非常频繁。
> - **DXCD>0（上中下）**（164469 段，9733661 天）：段内 DXZC 交叉 **95.3%**，护型变化 **95.3%**；交叉次数平均 4.81 次，最多 237 次，交叉5+次占 28.2%。
> - **结论：** 用户观点成立——**DXCD<0 时 DXZC 平均交叉 5.11 次（最多138次）**，忠 虽然单看 DXZC 恒正，但在 DXCD<0 整体（忐忠忑）中 DXZC 反复交叉，**忠 的 DXZC 转正是假信号，不可信**。要等变忐（DXCD>0 且 DXZC 稳定）再介入。
>
> **⚠️ 忠 观察慎入：** 忠 虽然单看 DXZC 恒正，但 DXCD<0（忐忠忑）期间 DXZC 反复交叉（97.4%），**忠 的 DXZC 转正是假信号**；且忠 DXZD<0（价格在 EMA60 之下），**上升受限、升连弱（20-22%）、P≥3% 弱**——所以**操作上观察慎入**，**等待 DXCD 变为忐或上 再介入**。**要观察慎入！**
...
> **结论：** 四类区域的数学基础 = **均线系统几何结构**。方向由长均线（DJD）决定（DJZ>DJD=升势，DJZ<DJD=跌势），待定状态（中偏升/忠偏跌）由"跌破/站上短均线但长均线方向相反"决定，强度由均线排列（多头/空头）决定。**DXAB 在各区域内做优选，不改变区域划分。** 此数学框架是 §6.2.1 概率划分的理论基石。
...
> 1. **DXZE>0 + CD下/忑 是执念，不是禁区** —— VBA 里只有 `DXZE<0 + 下/忑` 才是"禁"，`DXZE>0 + 下/忑` 落入"念"（执念）。大势向上时下/忑只是执念，不是绝对禁区。
> 2. **忠 归入鸡肋（无论 ZE），不是主战场** —— 忠 = DXZC>0 但 DXZD<0（价格在 EMA60 之下），上升受限、升连弱（20-22%），**操作上观察慎入**，所以统一归鸡肋（可持有但不宜过早介入），不归主战场（不重仓）。只有"中"才是执念。
>
> **核心逻辑：** CD 决定土质（上/忐=沃土可做，忠=观察慎入，中=执念，下/忑=禁区），DXZE 决定大势（>0 升格，<0 降格）；下/忑+大势向下=禁区，下/忑+大势向上=执念，中=执念，可做+大势向下=鸡肋，可做+大势向上=主战场。
...
> - **操作含义：** 忠 统一归**鸡肋**（无论 ZE，2026-09-20 用户定稿）——**实际介入时对忠 观察慎入**：DXZE>0 时忠 可观察等待转好（37%转上/忐）再介入，不宜直接重仓；DXZE<=0 时忠 转好仅16%，更应回避。**忠 是"值得等待的执念倾向"，不是"直接介入的主战场"。**
>
> **⚠️ 忠 观察慎入（用户，2026-09-19）：** 忠 虽然单看 DXZC 恒正，但 DXCD<0（忐忠忑）期间 DXZC 反复交叉（97.4%），**忠 的 DXZC 转正是假信号**；且忠 DXZD<0（价格在 EMA60 之下），**上升受限、升连弱（20-22%）、P≥3% 弱**，所以**操作上观察慎入**——**等待 DXCD 变为忐或上 再介入**，不要在忠 里猜测何时变好。
...
> - **DXZC<0（中/下/忑）：** 甲乙己几乎消失（乙ZA>0 仅0.03-0.7%），**观察甲乙己**（基本无样本）；**禁入丙丁戊**（丁/戊ZA<0 主导但弱，P≥3% 22-25% 应放弃）。
> - **核心：DXCD 是决定性变量，DXAB 是次要的**——先选土（DXCD），再选护型（DXAB），不要在禁区/执念里谈护型。

> [!IMPORTANT]
> 这是整个定型操作框架的**决定性一层**：在选区内，唯一需要判断的就是**选择哪个护型**。下表是全部 8 个护型的完整评定（P≥3% + 持续性 + 次日转移 + 操作结论），**操作时应以此表为最终依据**。

> **🔍 DXCD 是决定性变量，DXAB 是次要的（2026-09-20 用户确认）：** 第1层选土（DXCD）决定"做不做"，第2层护型（DXAB）决定"选哪个"。**DXCD 是真正的决定性变量**——在选好的土质（DXCD=上/忐）里，**丙丁戊可忍受**（分角色处理：乙ZA<0/丙持仓或退出、戊ZA>0观察、丁/戊ZA<0放弃）；在禁区/执念（DXCD=中/下/忑）里，甲乙己也几乎消失（乙ZA>0 仅0.03-0.7%）。**先选土（DXCD），再选护型（DXAB）**——不要在禁区/执念里谈护型。
...
> **✅ 操作方法定型：**
> 1. **只做 DXCD=上/忐**（DXZC>0，首选）；**忠 观察慎入**（归主战场/鸡肋，但升连弱，需叠加升连≥2%才介入）；排除中/下/忑。
...
#### 6.5.1 护型操作决策流程图（可视化总结，2026-09-10，2026-09-11 补全DXCD分支）：

> **说明：** 下图将 §6.1 定型操作框架的完整决策逻辑可视化为流程图——先按 DXCD 判断区域，DXZC>0（上/忐）区域内按护型判断操作，可做的护型进入介入时机判断，持有的护型监控退出信号。**（若在支持 Mermaid 的查看器中打开可交互渲染；也可打开 `____temp/护型流程图.html` 查看 HTML 版。）**
...
flowchart TD
    ...
    Q0 -->|忠 DXZC>0但DXZD<0| ZHONG[忠<br>DXZD<0 观察慎入]
    ZHONG --> ZHONG1[⚠️ 归主战场/鸡肋<br>但升连弱 需叠加升连≥2%]
    ZHONG1 --> TIMING
...
> | **忠** | DXZC>0 但 DXZD<0 | ⚠️ **观察慎入**（归主战场/鸡肋，但升连弱，需叠加升连≥2%）|
...
>
> **己的升连比例（坡度特征）：** 己在 DXZC>0 时**升连比例最高（66.8%）**——己一旦出现几乎都是连续阳线（坡度陡），印证"己=冲高后急跌"的直觉。但己占区域内升连仅7.5%（"陡但少"），出现频率低，需快进快出（见 §6.3 第2层护型选择）。

#### 6.5.2 流程图遍历验证（随机10个代码，2026-09-11）：
...
> - **DXCD=忠（DXZC>0 但 DXZD<0）未覆盖 7.7%**——**已补全操作建议：观察慎入**（归主战场/鸡肋，遵循 VBA 定义，见 §6.2.1）。忠虽是 DXZC>0，但 DXZD<0 意味着价格在 DJD(EMA60) 之下，上升受限、升连弱——**需叠加升连≥2%（§5.3.8）才介入**，否则忠的升连不提升盈利性。
>
> **✅ 补全后流程图覆盖：** DXCD=上/忐（约40%）按护型操作；DXCD=忠（约8%）观察慎入（需叠加升连≥2%）；DXCD=中/下/忑（约52%）明确回避不属于操作范围。**每个柱都能找到对应操作指南（或明确的不操作指令）。**
...
> **6.5.3.3 诱多识别（操作区域内 柱排×触顶 P≥3%，从低到高）：**
...
#### 6.6.2 定型操作期望收益（2026-09-08 验证，BSHA 用列19修正后）

> **口径说明：** 本节测算的是"定型操作组合"（DXCD=上/忐 × 甲/乙ZA>0/己 × DJA丘 + BSHA5）的收益；**§6.12 测算的是"DXZC>0+剔忠+广义正交"口径（DJC+剔忠）**。两者过滤不同，数据互补，可对照：（§6.12 主介入点 29.8%、年化30-60%）vs（本节定型组合 29.68-42.5%、年化18-32%）——**定型组合更"少而精"，年化更保守但每次胜率更高**。
...
> **各层递进价值：** 第1层选土（主战场 DXZE>0+上/忐，四类区域"主战场"子集）提升 +4.98pp（23.6%→28.6%），护型甲乙己 +1.09pp，BSHA5 触发器 **+12.79pp**（29.7%→42.5%）。**BSHA5 是最大增量。**
>
...
##### 6.6.3.2 低波池（Qe+Qin+Qd+Qif+Qst，3732只）
...
> 核心思路：在升管（DXAB甲乙己 + 升管入管）中，用 **（DXAB + 等型）** 作为坐标定位介入时机。<br>
> 等型坐标：**等1（起点）/等2（早期）/等3（成熟）/等5（出管）**。
> **管宽分类铁律（2026-09-20 用户纠正，2026-09-21 阈值 5→4 + 状态机口径）：** 扩管/平管/窄管**必须用管宽哼JA（col21）**，不是宽哼JC（col23）。<br>
> **窄管 = 0 < 管宽哼JA < 4**；**管宽≥4 时看柱排是否推高顶（推高顶=触顶+升排）——推高顶=扩管，非推高顶=平管**。<br>
> 管宽哼JA 只在升管（日ZA>0）时有值。**升管 = DXAB 甲乙己（DXZB>0）且 形成甲乙己后曾触哼**（用户确认：只有甲乙己才能定义升管）。

#### 6.7.0 框架总览：操盘工具 + 坐标定位工具
...
- 在升管（DXAB甲乙己 + 升管入管）中，用 **（DXAB + 等型）** 定位介入时机。
- **等型坐标：** 等1（起点）/等2（早期）/等3（成熟）/等5（出管）。
- 等型判定（BT连阳 col44，VBA权威）：等1=日ZA=1，等2=日ZA=2，等3=日ZA≥3，等5=日ZA=-1，等6=日ZA=-2，等7=日ZA≤-3。
- **升管/跌管定义 + 管宽分类铁律见 §6.7.1**（升管=甲乙己+曾触哼，跌管=丙丁戊+DXZA<0，窄管=0<管宽哼JA<4，管宽≥4看推高顶分扩管/平管）。

#### 6.7.1 升管管宽分类：原则 + 分布统计（2026-09-20 全量验证）

> **🔍 升管/跌管定义（2026-09-20 用户确认，2026-09-21 状态机口径）：**
> - **升管 = DXAB 甲乙己（DXZB>0）且 形成甲乙己后曾触哼**（甲/乙/己，正交）。**只有甲乙己才能定义升管**（戊/丙/丁即使 DXZA>0 也不算升管）。**一旦触哼就开始算升管**（即使后续不再触哼且保持甲乙己，就仍在升管中）；**如果形成甲乙己后始终未触哼，不算升管**。甲乙己→丙丁戊→又回到甲乙己 = 刚进入甲乙己（重新累积触哼状态）。本框架 §6.8.1~6.8.3 的等1/2/3 均限定甲乙己。
> - **跌管 = DXAB 丙丁戊 + DXZA<0**（丙/丁/戊，非正交），由升管**镜面映射**定义。
> - 操作流程：**甲乙己（DXZB>0）→ 触哼 → 入管 → 入管后区分窄管/扩管/平管**。

> **🔍 升管的两个条件 + 严进宽出判定原则（2026-09-21 用户确认）：**
> - **升管的两个条件：** ① **DXAB 为甲乙己**；② **在上破 DJB 后触哼**（触到管宽哼JA哼线）。
> - **判定原则"严进宽出"：** 放弃原来"一旦上破 DXZA 就考虑是否入管"的旧思维，**只在触哼之后才考虑是否入管**，从而**缩小了考虑范围**（入管用严格标准，出管用宽松边界 DJB，见 §6.7.2.3）。

> **🔍 管宽哼JA 定义（VBA权威）：** 管宽哼JA = (最近5日最高价 / EMA5 − 1) × 100，即**最近5日最高价相对 EMA5 的偏离百分比**。**管宽哼JA 就是衡量升宽的变量**（用户确认，2026-09-20）。**管宽哼JA 无负值**（最高价通常高于均线），升管/跌管分布几乎一致（0-20 占 99%+）。

**① 管宽分类原则（用户纠正，非固定阈值；2026-09-21 阈值 5→4）：**
- **窄管**：0 < 管宽哼JA < 4（升宽很小，无有效上涨，应回避）。
- **管宽≥4**：看柱排是否**推高顶**（推高顶 = 触顶 + 升排）——**推高顶=扩管**（升宽在扩大），**非推高顶=平管**（升宽持平）。
- **管宽哼JA 恒≥0**（最近5日最高价 ≥ EMA5），所以**窄管(<0) 在升管语境下不存在**——升管前提 DXZA>0 已保证管宽哼JA≥0。
- **窄管定义（0-4）在升管/跌管通用**：跌管也有窄管(0-4)（极弱应回避，见 §6.8.5）。
- **阈值 5→4 依据（2026-09-21）：** 窄管0-5 占 62.55% 比例过高，细分显示 **4-5 区间次日P3% 24.27% 接近基线（23.03%）**，并非弱区，不应归入窄管回避。阈值降到 4 后，窄管占 50.26%，扩管/平管合计升至 49.29%。

**② 升管漏斗（全量 7455 文件，n=20,280,162，2026-09-21 状态机口径）：**

> **升管判定（用户确认 2026-09-21）：** 升管 = **甲乙己（DXZB>0）且 形成甲乙己后曾触哼**。**一旦触哼就开始算升管**（即使后续不再触哼且保持甲乙己，就仍在升管中）；**如果形成甲乙己后始终未触哼，不算升管**。甲乙己→丙丁戊→又回到甲乙己 = 刚进入甲乙己（重新累积触哼状态）。

| 层次 | 分类 | n | 占全部 | 占甲乙己 | P3% |
|:--|:--|--:|--:|--:|--:|
| **第1层** | **全部样本** | 20,280,162 | 100% | — | — |
| | ├ 甲乙己（DXZB>0） | 10,048,520 | 49.55% | 100% | — |
| | └ 丙丁戊（DXZB<0） | 10,231,642 | 50.45% | — | — |
| **第2层** | **升管（甲乙己+曾触哼）** | 9,771,270 | 48.18% | 97.24% | — |
| | 甲乙己未触哼 | 277,250 | 1.37% | 2.76% | — |
| **第3层** | **窄管（0-4）** | 4,797,304 | 23.66% | 47.74% | 11.63% |
| | **扩管（≥4+推高顶）** | 2,397,992 | 11.82% | 23.86% | 35.09% |
| | **平管（≥4+非推高顶）** | 2,534,636 | 12.50% | 25.22% | 32.80% |
| | 管宽=0 | 41,337 | 0.20% | 0.41% | 4.10% |

> **漏斗过滤：** 第1层过滤丙丁戊 50.45%（保留甲乙己 49.55%）；第2层过滤甲乙己未触哼 2.76%（保留升管 97.24%）；第3层窄管 47.74% 回避、扩管+平管 49.08% 介入。**最终介入区（扩管+平管）占全部样本 24.32%，占甲乙己 49.08%。**
> **管宽=0 说明（2026-09-21 研究）：** 管宽≤0 全部是**管宽哼JA = 0**（100%，无负数），印证"管宽哼JA 无负值"。**两种原因：** ① **上市首日**（约44%）：EMA5 初始值=首日收盘价，最近5日最高价=首日最高价，两者相等（如 sz300584 2017/1/12 收10.11，之后连续涨停）；② **长期横盘**（约56%）：价格长期不动，EMA5 收敛到横盘价，最近5日最高价=横盘价（如 sh600166 2003/2/24 收0.6，前后多日0.59-0.60）。占比极小（0.41%），不影响结论。
...
> **管宽哼JA 几乎都在 0-20（99%+），>60 仅 0.01%**——**旧固定阈值"扩管>60"几乎不存在**，印证用户纠正的正确性（旧阈值分类区分度仅 4.79pp，新分类 23pp+）。**4-5 区间占 12.41%，次日P3% 24.27% 接近基线**——这是阈值 5→4 的依据（4-5 并非弱区，不应归入窄管回避）。

**④ 升管管宽分类 × 次日P3%（全量，基线 23.03%，状态机口径）：**

| 分类 | n | 次日P3% | 均高幅 |
|:--|--:|--:|--:|
| **扩管（≥4+推高顶）** | 2,397,992 | **35.09%** | 2.85% |
| **平管（≥4+非推高顶）** | 2,534,636 | **32.80%** | 2.65% |
| **窄管（0-4）** | 4,797,304 | **11.63%** | 1.32% |
| 管宽=0 | 41,337 | 4.10% | 0.53% |

> **区分度：扩管 35.09% vs 窄管 11.63%，差 23.46pp**。扩管/平管是介入主战场，窄管应回避。

**⑤ 升管管宽分类 × 等型（次日P3%，状态机口径 + col42判定等型）：**

| 等型 | 窄管 | 平管 | 扩管 |
|:--|--:|--:|--:|
| 等1 | 15.95% | 39.23% | 36.84% |
| 等2 | 15.58% | 38.73% | 38.66% |
| **等3** | 15.96% | 39.28% | **46.68%** |
| 等5 | 12.96% | 34.67% | 31.95% |
| 等6 | 11.88% | 32.15% | 29.62% |
| 等7 | 7.40% | 28.44% | 24.75% |

> **扩管×等3 = 46.68% 是最强介入点**（n=345,740）。扩管/平管在等1-3 都强（36-47%），窄管在等7 最弱（7.40%，接近出管）。

**⑥ 升管管宽分类 × 护型（分布占比，状态机口径，占甲乙己）：**
...
> **扩管主要在甲（13.17%）和乙（8.95%）**，己很少（1.75%）。升管护型分布：甲 48.67%、乙 37.65%、己 13.68%。

**⑦ 升管管宽分类 × 柱排（次日P3%，状态机口径）：**

| 柱排 | 窄管 | 平管 | 扩管 |
|:--|--:|--:|--:|
| 升连 | 11.78% | 35.81% | 36.19% |
| 升吞 | 13.27% | 39.17% | 36.95% |
| 升孕 | 10.43% | 32.56% | 30.56% |
| 人排 | 12.22% | 33.81% | — |
| 跌连 | 9.99% | 30.61% | — |
| 跌孕 | 10.63% | 28.93% | — |
| 跌吞 | 10.10% | 27.37% | — |

> **扩管只有升排**（扩管=触顶+升排，人排/跌排不满足"推高顶"定义）。平管×升吞 39.17% 最强；窄管×跌连 9.99% 最弱。

> **核心结论：** 升管（甲乙己+曾触哼）中，**扩管/平管（管宽≥4）是介入主战场**（33-35%），**窄管（0-4）占 47.74% 应回避**（11.63%）。**扩管×等3 = 46.68% 是最强介入点**。管宽哼JA 恒≥0，窄管(<0) 在升管语境不存在。

#### 6.7.2 升管判断（状态机口径，触哼入升管）

> **升管 = 甲乙己 + 已触哼（状态机口径）。** 由非甲乙己变为甲乙己后，**触哼的那一刻入升管**；保持甲乙己就在升管中；变非甲乙己出升管。完整验证见 §5.1.4.1，核心结论：

##### 6.7.2.1 升管定义（状态机口径，只针对 DXAB 甲乙己，不考虑 DXZC）

> **升管状态机（用户确认 2026-09-22）：** 由非甲乙己变为甲乙己后，**只要未触哼就不考虑入升管**；**只有触哼之后才入升管**；入升管后**保持甲乙己就在升管中**（即使不再触哼）；**变为非甲乙己就是出升管**。

| 条件 | 样本 | 次日P3% | 5天P3% | 3天出管% |
|:--|--:|--:|--:|--:|
| 基线_甲乙己 | 10,048,520 | 22.80 | 68.00 | 30.45 |
| **升管（已触哼）** | 9,771,270 | **22.85** | **68.29** | **29.98** |
| **非升管（未触哼）** | 277,250 | **21.31** | **57.52** | **46.76** |

> **升管（已触哼）占甲乙己 97.24%**。**非升管（未触哼）明显更弱：** 5天P3 57.52%（比升管 **-10.77pp**），3天出管率 46.76%（比升管 **+16.78pp**）——**印证"未触哼就不算升管"的逻辑**（非升管确实更弱，虽只占甲乙己 2.76%）。

##### 6.7.2.2 管中模式（扩管/平管/窄管）

> **升管分类（用户，2026-09-19，2026-09-20 纠正定义，2026-09-21 阈值 5→4 + 状态机口径）：** 升管 = **DXAB 甲乙己（DXZB>0）且 形成甲乙己后曾触哼**（只有甲乙己才能定义升管）。主要分为三类——**窄管**（管宽过小，不会形成有效上涨形态）、**扩管**（管宽≥4 + 柱排推高顶，升宽在扩大）、**平管**（管宽≥4 + 非推高顶，升宽持平）。**推高顶 = 触顶 + 升排**。窄管 = 0 < 管宽哼JA < 4。
...
> **✅ 扩管（35.09%）> 平管（32.80%）> 窄管（11.63%）**，差 23.46pp——**扩管 次日P3% 最高（管宽≥4 + 柱排推高顶），窄管 最低（管宽0-4，管宽过小，不会形成有效上涨形态，应回避）**。区分度远强于旧固定阈值分类（4.79pp）。

##### 6.7.2.3 出管判定（状态机口径）

> **判断是否在管的目的（用户定稿 2026-09-22）：** 判断是否在管，**根本目的是判断是否仍然后续可能会上升**——即**寻找 DXZC>0 时的上升主体部分**。我们介入升管，是为了抓住上升主体，而不是为了"在管"这个状态本身。
>
...
> **垒升 = 柱体明显的升连/升吞/升孕（涨幅≥2%），是比"DXZC>0 且甲乙己"更高级别的要求。** 完整验证见 §5.3。**核心操盘原则：不要因为"是升连"就介入，要看柱体大小**——涨幅≥2%的升连/升吞/升孕才是垒升（可介入），涨幅<2%的升连（十字星）是平移（介入后只能波段不能持有）。

##### 6.7.3.1 垒升判定：在垒升位置介入的获利概率
...
##### 6.7.3.3 镜面映射的全局指导：先按 DXZC 分类，再按升连/跌连分类（§5.3.5.2）
...
> **进出时机（2026-09-20）：** 在 §6.7 工具汇总（升管/跌管定义 + 管宽分类）基础上，用 **（DXAB + 等型）** 定位具体进出时机。等型坐标：**等1（起点）/等2（早期）/等3（成熟）/等5（出管）**。各等型的介入点/止损条件见下方。

#### 6.8.0 介入点/止损条件汇总表

| 等型 | 定位 | 介入点条件 | 止损条件 | 次日P3% |
|:--|:--|:--|:--|:--|
| **等1** | 升管起点 | **等第2柱确认**：第2柱维持升管(ZA>1)→29.39%可介入；人排/升吞轻仓 | 第2柱回落(ZA≤0)→2.58%诱多/出管惯性，止损 | 21-26% |
| **等2** | 升管早期 | 触顶+升排（升连/升吞）；正交初期回落DJA反弹（继续触顶形成垒升） | 未触顶+跌排回避；跌破DJA止损 | 19-24% |
| **等3** | 成熟升管 | **扩管（管宽≥4+触顶+升排）→升连36.71%**；平管（≥4+非推高顶）32.67% | **窄管(0-4)→11.06%极弱回避**；不触顶阳柱→管宽缩小→下破DJA止损 | 20-36% |
| **等5** | 出管/跌管 | 首下破反弹；**等6第二柱阳柱+次日再站DJA(37.37%)**；整体回避 | 跌破DJA无反弹离场 | 20-23% |

> **核心结论：** 介入时机坐标 = **等3 扩管（管宽≥4 + 触顶 + 升排）** 是最强介入点（升连 36.71%）；等2 靠触顶增强；等1 靠第2柱确认（维持升管29.39% vs 回落2.58%）；等5 整体回避（仅等6第二柱阳柱+次日再站DJA 37.37%可介入）。**管宽哼JA（col21）是决定性维度**——窄管(0-4)极弱应回避，扩管（≥4+推高顶）升排最强。⚠️ **阈值 5→4（2026-09-21）：** 原等3 扩管48.79%/平管42.20%/窄管19.88%（用 col23 偏顶JC），修正为 col21 管宽哼JA + 阈值4——扩管35.33%/平管32.67%/窄管11.06%。

#### 6.8.1 等3：成熟升管（站上DJA 3柱+）

> **等3 是主介入区**（升管=甲乙己+曾触哼，n=6,398,833，基线 23.01%）。管宽哼JA 是决定性维度：**扩管（管宽≥4+推高顶）最强**（35.33%），**窄管（0-4）极弱**（11.04%，应回避）。⚠️ **修正（2026-09-21）：** 原数据（等3 n=418,183，基线43.30%，扩管48.79%）误用宽哼JC（col23）当管宽哼JA，正确应为管宽哼JA（col21）+ 阈值4 + 状态机口径——等3 n=6,398,833，基线23.01%，扩管35.33%/平管32.66%/窄管11.04%。

**已验证数据（升管=甲乙己+曾触哼，等3，n=6,398,833，基线 23.01%，推高顶=触顶+升排）：**

| 分类 | n | 占比 | 次日P3% |
|:--|--:|--:|--:|
| **窄管（0-4）** | 3,059,568 | 47.81% | **11.04%** |
| **平管（≥4+非推高顶）** | 1,352,515 | 21.14% | **32.66%** |
| **扩管（≥4+推高顶）** | 1,960,325 | 30.64% | **35.33%** |
| 管宽=0 | 26,425 | 0.41% | 0.12% |

**等3 管宽分类 × 柱排（次日P3%，状态机口径）：**

| 柱排 | 窄管(0-4) | 平管(≥4+非推高顶) | 扩管(≥4+推高顶) |
|:--|:--|:--|:--|
| 升连 | 11.06% | 35.70% | **36.71%** |
| 升吞 | 12.76% | **38.26%** | 36.90% |
| 升孕 | 9.96% | 31.88% | 30.70% |
| 人排 | 11.47% | 33.37% | — |
| 跌连 | 9.75% | 30.62% | — |
| 跌孕 | 10.55% | 28.38% | — |
...
> **结论：**
> ① **窄管(0-4) 极弱**（11.06%），远低于基线（22.99%）——**窄管应回避**（管宽过小，无有效上涨形态）；
> ② **扩管(≥4+推高顶) 最强**（35.33%）——**管宽≥4 + 触顶 + 升排 = 最强介入区**；
> ③ **平管(≥4+非推高顶) 次强**（32.67%），也高于基线；
> ④ **区分度巨大**：扩管(35.33%) vs 窄管(11.06%) 差 **24.27pp**——远强于旧固定阈值分类（4.79pp）。

**介入点条件（等3）：**
- **首选：** 扩管（管宽≥4 + 触顶 + 升排）→ 升连 36.71%，最强介入区。
- **次选：** 平管（管宽≥4 + 非推高顶）→ 32.67%，也高于基线。
- **升排推高顶：** 扩管→升排推高顶（触顶+升排），可期待连续冲高。
- **平管平移十字星：** 关注下柱冲高（十字星后方向待确认，专项验证：平管十字星次日P3%=21.48%，接近基线，非强信号）。

**止损条件（等3）：**
- **窄管（管宽哼JA 0-4）→ 11.06%，极弱，应回避**（管宽过小，无有效上涨形态）。
- **不触顶阳柱 → 管宽缩小 → 下破DJA** 是转坏链条，跌破 DJA 即止损。
- 未触顶 + 跌排 → 弱于扩管升排，应回避或减仓。

#### 6.8.2 等1：升管起点（上破DXZA=1第一柱）
...
> **第2柱维持升管 → 29.39%（可介入）；第2柱回落 → 2.58%（诱多/出管惯性，回避）。** 这直接验证了用户框架"第1柱仅用来判定是否为诱多/出管惯性"——**第1柱本身无区分度，但第2柱是否维持升管是决定性信号**（差 26.81pp）。
...
> **🔍 专项验证（2026-09-20）：等2 触顶是决定性维度。** 等2（日ZA=2）中，触顶（24.00%）vs 未触顶（19.71%）差 4.29pp。**"正交初期回落DJA"难以用前日日ZA≤0 隔离**（日ZA=2 时前一日必为日ZA=1，即刚上穿），因此等2 的核心判别回到**触顶**：触顶（尤其继续触顶形成垒升）→ 可介入；未触顶 → 回避。

**介入点条件（等2）：**
- **首选：** 触顶 + 升排（升连/升吞）→ 胜率 22-24%，等2 最强。
...
> **等6 第二柱阳柱本身胜率低（17.18%），但若次日能再站上DJA，胜率飙升至 37.37%。** 约 41.6%（2277/5478）能再站上。这验证了用户框架"DTZA=-2第二柱阳柱有可能再次站上DJA"——**第二柱阳柱不是直接介入点，而是"观察是否再站DJA"的触发信号**：再站上才介入（37.37%），未站上回避。
...
> **跌管定义（2026-09-20 用户确认）：** 跌管 = DXAB **丙丁戊**（非正交），由升管（甲乙己）**镜面映射**定义。升管的介入时机逻辑（等型坐标 + 管宽哼JA 分类）镜像到跌管，作为跌管的退出/回避/反弹介入逻辑。

**已验证数据（跌管 丙丁戊，全量 7455 文件）：**

**① 管宽哼JA 值分布：升管/跌管几乎一致，阈值可共用（全量 7455 文件）。**
| 区间 | 甲乙己(升管) | 丙丁戊(跌管) |
|:--|--:|--:|
| 0-4 | 49.82% | 53.77% |
| 4-5 | 12.41% | 12.19% |
...
> **管宽哼JA 无负值**（最高价通常高于均线），所以跌管**没有"窄管<0"**。但**窄管定义（0-4）在跌管同样适用**——跌管窄管(0-4) 极弱（见下方②），应回避。

**② 跌管各护型管宽分类次日P3%（跌管=丙丁戊+DXZA<0，全量 7455 文件，推高顶=触顶+升排）：**
| 分类 | 丙 | 戊 | 丁 |
|:--|:--|:--|:--|
| **窄管(0-4)** | **9.66%** (n=577,843) | **10.76%** (n=2,159,545) | **10.73%** (n=1,918,002) |
| **平管(≥4+非推高顶)** | 28.13% (n=665,906) | 27.09% (n=2,122,429) | 26.99% (n=1,253,845) |
| **扩管(≥4+推高顶)** | 23.30% (n=8,716) | 21.15% (n=23,440) | 21.35% (n=9,847) |
| 管宽≤0 | 0.21% (n=5,212) | 5.47% (n=14,399) | 3.11% (n=7,599) |
| 基线 | 19.49% | 18.82% | 17.13% |

> **🔍 跌管管宽分类（2026-09-21 阈值 5→4）：** 跌管（丙丁戊）**窄管(0-4) 也极弱**（9.7-10.8%，低于基线17-19%），应回避；**平管（管宽≥4）反而较强**（27-28%，远高于基线）——跌管中管宽≥4 时胜率高于整体。⚠️ **修正（2026-09-21）：** 原数据（窄管丙11.93%/戊12.27%/丁12.36%、平管30%+、扩管22.8-25.5%）用阈值5，现改为阈值4——窄管(0-4) 9.7-10.8%，平管(≥4) 27-28%，扩管(≥4) 21-23%。旧分类"跌管扩管>60 也强（丙54.68%）"基于固定阈值（>60）且样本极小（n=1,046），**不再有"超跌反弹"强信号**。

**③ 跌管各等型（跌管=丙丁戊+DXZA<0，全量 7455 文件）：**
| 等型 | 次日P3% | n |
|:--|:--|:--|
| 等3(ZA≥3) | 34.34% | 364 |
...
> ⚠️ **修正（2026-09-20）：** 原数据（等1 19.24%/等7 18.53%/等6 17.63%/等2 16.39%/等5 16.01%/等3 15.74%）误用错误列，正确应为 col43（BT连阳等型）+col27（次日高幅）。**等3/等2 样本极小（n=364/11,655），谨慎**；等1/等5/等6/等7 样本较大（12-49万）。跌管各等型整体 18-34%，**等7（超卖）最弱（18.23%）**，等1-3（跌管中反弹）反而较强（29-34%）但样本小。

**镜面映射操作逻辑（跌管 丙丁戊）：**
- **升管介入（扩管≥4+推高顶 升排）→ 跌管回避/退出**（跌管整体偏弱，基线 17-19%）。
- **跌管窄管(0-4) → 极弱（9.7-10.8%），回避。**
- **跌管平管（管宽≥4）→ 27-28%，可轻仓试仓**（高于跌管基线，但低于升管扩管35%）。
- **跌管扩管（≥4+推高顶）→ 21-23%**，弱于平管，样本少（n=8,716~23,440），谨慎。
- **跌管各等型整体偏弱（18-34%）**，等7（超卖）最弱（18.23%）应回避；等1-3（跌管中反弹）较强但样本小，仅作反弹关注。

> **⚠️ 待办：** 跌管（丙丁戊）的**窄管/转弱信号**已用新分类（窄管0-4）定义，但跌管管宽哼JA 无负值，需进一步验证跌管窄管的边界。

#### 6.8.6 主升必赢：高抛低吸 + 行乞心态（2026-09-24 验证）

> **课题（用户思想，2026-09-24）：** 主升区域（多头排列）存在大量震荡，不能因震荡放弃主升，要在此区域**高抛低吸**，依赖 DXZC/DXZB 入口操作；主升**必赢**，以**行乞心态**（只求有机会形成主升，不求走势工整）而非考官心态。
>
> **数据说明：** 全量 7455 文件 / 2028 万行谕组日。主升区域 = **多头排列（JC>JD）= DXCD 上/中/下**（占全样本 48.00%）。跌破 DJA=日ZA<0，跌破 DJB=DXAB 护型为丙丁戊，跌破 DJC=DXCD 中/下，跌破 DJD=DXCD 下。

**① 主升区域（多头排列）内跌破各均线比例（用数据说话）：**
| 跌破 | 比例 | 依据 |
|:--|:--|:--|
...
> **🔍 结论：** 跌破比例**单调递减**（DJA>DJB>DJC>DJD）——**低阶均线容易跌破，高阶均线不易跌破**。主升区域确实存在大量震荡（跌破 DJA 近半、跌破 DJB 四成），但震荡集中在低阶均线；跌破 DJC/DJD 的比例递减。多头排列内部：上=69.02%，中=12.94%，下=18.04%。

**② DXAB 护型（甲乙己/丙丁戊）= DJB 跌破的完美代理：**
...
> **🔍 结论：** 多头排列整体（上+中+下）下日P3=23.20%，**显著高于基线 20.51%**——主升区域整体"必赢"。但跌破 DJD（下）后持有10日累计 -0.17% 转负，不再是主升，应回避。

**④ 高抛低吸（DXCD=上 时，持有N日累计收益）：**
| 状态 | 持有3日 | 持有5日 | 持有10日 |
|:--|:--|:--|:--|
...
> **🔍 结论：** **跌破 DJA/DJB 低吸明显优于站上追高**（持有10日差 2.19pp/2.38pp）。**跌破 DJB 低吸比跌破 DJA 低吸更强**（+7.12% vs +6.67%）——更深回调，反弹空间更大。这正是"依赖 DXZC/DXZB 入口"。
> ⚠️ **注：** 若看**次日冲高**（下日P3），站上均线反而更强（25.84% vs 22.37%）。但"低吸"的本质是**持有赚反弹**，故用持有N日累计收益衡量。

**⑤ 行乞心态（跌破后能否拉回，DXCD=上 时）：**
| 跌破 | 1日 | 3日 | 5日 | 10日 |
|:--|:--|:--|:--|:--|
...
> **🔍 结论：** **跌破 DJA/DJB 后几乎必然被拉回**（5日95-98%，10日99%+）——震荡是主升的一部分，会被趋势修复，"只求有机会形成主升，不求工整"成立。**浅回调（DJA/DJB）恢复快（99%），深回调（DJC/DJD）恢复慢（91%）**。

**⑤b 恢复概率（DXCD>0 多头排列整体，跌破各均线后未来N日内恢复，全量 973 万行）：**
...
> **🔍 结论：** **跌破越深，恢复越慢**（1日 DJA 27.74% > DJD 20.54%），但**20日内几乎必然恢复（全部 99%+）**；10日内恢复率全部 >90%（DJA 95.72% / DJB 92.48% / DJC 92.09% / DJD 91.95%）。**完美验证行乞心态：主升区域跌破任何均线，20日内几乎必然被趋势拉回。**

**⑤c DXZE>0 + DXCD上/忐 是否必赢？（全量 2028 万行）：**
...
> **🔍 结论：** **DXZE>0 + DXCD=上 是"必赢"**（持有20日 +3.61%，正收益 56.06%，P≥3% 43.49%，远高于基线 +1.31%）。**DXZE>0 + DXCD=忐 不是必赢**（持有10日 -0.31% 负，持有20日仅 +0.59%，弱于基线）。**即使加了 DXZE>0（站上EMA120）这个强条件，DXCD=忐 依然弱**——再次印证 **DXCD=上 才是必赢，DXCD=忐（空头反弹）应剔除**。上忐合并因忐样本占比极小（17.8万/671万≈2.7%）拖累小，合并后仍接近"上"（+3.53%）。

**⑤d EMA必赢的数学本质（DXZE>0+DXCD=上，全量 653 万行）：**
...
> **🔍 EMA必赢本质：** 一旦 DXZE>0+DXCD=上（结算价站上EMA120 且多头排列站上EMA26），**未来20日内 98.81% 的时间结算价都保持在 EMA26 上方**（未来20日 日ZC均值 21.9，持续在上方）。**EMA 是滞后指标**——价格在上、均线在下追，均线永远追不上价格，只要价格不跌破均线，持有就必赢。这是**数学特性保证**，不是预测。

**⑤e 为何还要关注微观条件（选取必赢区域的高点）：**
...
> **🔍 结论：** **必赢 ≠ 赢在最高点**。DXZE>0+DXCD=上 是"必赢区域"，但区域内价格围绕均线震荡（跌破DJA/DJB/DJC/DJD 比例 46%/40%/31%/18%）。**关注微观条件（BSHA/等型）是为了选取必赢区域的高点**——BSHA（偏离EMA5）越高，下日P3 单调递增（贴均线 5.84% → 20+ 49.99%）。**"这个高点高于破坏持仓条件时的交叉点"**：BSHA 高的介入点，价格偏离均线远，即使回调跌破均线（破坏持仓条件），介入点位置也高于交叉点，有安全边际。⚠️ 贴均线（BSHA 0-2）反而最差（5.84%），说明必赢区域内要选偏离均线的高点介入，而非贴着均线。
...
- **DXCD=忐（空头反弹）应剔除**：下日P3=17.27% 最弱，持有20日 -0.55% 负收益，不纳入"主升必赢"。

### 6.9 旧版介入时机
#### 6.9.1 介入时机（DJA丘 + 日等型 + BSHA5）

> **介入时机完整数据见 §6.12 主介入点（等1/等2/等3 细分）+ §6.12（BSHA5 触发器 + 等5超跌反弹）。** 下表为收敛结论：
...
### 6.12 旧内容：操盘决策速查（介入点/退出/流程/印证/收益）
#### 6.12.0 介入点

##### 6.12.0.1 主介入点（高胜率，可主动介入）
...
> [!IMPORTANT]
> **升连位置护型发现（2026-09-08，见2.4.2.10）：** DXZC>0 时升连位置，甲（47.7%）最多、乙（41.0%）次之。**但按 DXCD 分化：DXCD=上 时乙反而最多（48.4% > 甲38.3%），且甲/乙都强（33%+）可介入；DXCD=忐 时甲更强（27.6%）；DXCD=忠 时升连弱（20-22%），不可靠。** 升连介入优先选 DXCD=上（甲/乙都强）；DXCD=忠 观察慎入（归主战场/鸡肋，但升连弱，需叠加升连≥2%才介入，见 §6.2.1）。

##### 6.12.0.2 次介入点（中胜率，试仓）
...
##### 6.12.0.3 超跌反弹介入点（5.3.3 独有，2026-09-08 补充）
...
| 位置 | H2(≥2%) | P(≥3%) | 均高幅 | 样本 |
|:--|:--|:--|:--|:--|
| **操盘核心**（DJC+真正交+DJA）| 36.8% | 24.9% | 2.16% | 623.3万 |
| 操盘核心+等1 | 38.7% | 26.2% | 2.26% | 71.1万 |
| 操盘核心+等3 | 36.7% | 24.8% | 2.16% | 481.2万 |
| **5.3.3 等1+ZC>0+甲** | **39.8%** | **28.3%** | 2.47% | 10.1万 |
| 5.3.3 等3+ZC>0+甲 | 34.9% | 23.0% | 2.06% | 347.2万 |
| 5.3.3 等1+ZC>0+甲乙己 | 37.8% | 25.4% | 2.20% | 85.2万 |

**相互印证结论：**
1. **✅ 等1最强（两者一致）**——操盘核心等1（38.7%）≈ 5.3.3等1+ZC>0+甲乙己（37.8%），都指向等1是启动点。
2. **✅ ZC>0是前提（两者一致）**——5.3.3明确"ZC<0时无论护型多好都不建议主动介入"，与操盘计划的DJC丘第一层一致。
3. **✅ 等3样本最大/稳定（两者一致）**——操盘核心等3（481.2万）≈ 5.3.3等3+ZC>0+甲（347.2万），都是样本最大的稳定持有区。
4. **✅ 好护型（甲/乙ZA>0）最优（两者一致）**——5.3.3等1+ZC>0+甲（39.8%）> 操盘核心（36.8%），说明**限定甲护型比广义甲乙己更强**。
5. **⚠️ 5.3.3的等1+ZC>0+甲（39.8%）比操盘核心（36.8%）高3pp**——因为操盘核心包含所有广义护型（含己/戊ZA>0/丙），而5.3.3限定甲。**操盘计划可进一步限定甲/乙(ZA>0)护型提升胜率。**

**BSHA5 触发器（5.3.3 强调，操盘计划补充）：**

| 位置 | H2(≥2%) | P(≥3%) | 均高幅 |
|:--|:--|:--|:--|
| **操盘核心+BSHA5** | **54.8%** | **42.2%** | 3.34% |
| 操盘核心+BSHA<5 | 29.4% | 17.6% | 1.67% |
| **操盘核心等1+BSHA5** | **58.8%** | **46.9%** | 3.71% |
| 操盘核心等1+BSHA<5 | 34.4% | 21.8% | 1.94% |
...
| 位置 | H2(≥2%) | P(≥3%) | 均高幅 |
|:--|:--|:--|:--|
| **等5+ZC>0+乙+BSHA5** | **61.0%** | **46.3%** | 3.22% |
| 等5+ZC>0+乙+BSHA<5 | 34.0% | 20.0% | 1.86% |
...
>
> **操作含义：** DXZC>0 的跌连约2/3发生在DJA之下（其中下破DJA初期占55%），1/3在DJA之上。**DJA之上的跌连(33.8%)是"回调"信号（价格仍在DJA上方），DJA之下的跌连(66.2%)是"转坏"信号（已跌破DJA）。** 结合 §7.2.7：下破DJA时跌连是主要形态(51.6%)，需警惕转坏。
