...
| 模式 | 样本 | 次日P3% | 均高幅 |
|:--|--:|--:|--:|
| **扩管（管宽≥4+推高顶）** | 2,377,406 | **35.16%** | 2.85% |
| **平管（管宽≥4+非推高顶）** | 1,885,143 | **33.88%** | 2.67% |
| **窄管（管宽0-4）** | 4,345,794 | **11.85%** | 1.34% |
...
> **管宽分类铁律（2026-09-20 用户纠正，2026-09-21 阈值 5→4）：** 扩管/平管/窄管**必须用管宽哼JA（col21）**，不是宽哼JC（col23）。**窄管 = 0 < 管宽哼JA < 4**；**管宽≥4 时看柱排是否推高顶（推高顶=触顶+升排）——推高顶=扩管，非推高顶=平管**。管宽哼JA 只在升管（日ZA>0）时有值。**升管 = DXAB 甲乙己 + DXZA>0**（用户确认：只有甲乙己才能定义升管）。
...
- **升管/跌管定义 + 管宽分类铁律见 §6.7.1**（升管=甲乙己+DXZA>0，跌管=丙丁戊+DXZA<0，窄管=0<管宽哼JA<4，管宽≥4看推高顶分扩管/平管）。

#### 6.7.1 升管管宽分类：原则 + 分布统计（2026-09-20 全量验证）

> **🔍 升管/跌管定义（2026-09-20 用户确认）：**
> - **升管 = DXAB 甲乙己 + DXZA>0**（甲/乙/己，正交）。**只有甲乙己才能定义升管**（戊/丙/丁即使 DXZA>0 也不算升管）。本框架 §6.8.1~6.8.3 的等1/2/3 均限定甲乙己。
> - **跌管 = DXAB 丙丁戊 + DXZA<0**（丙/丁/戊，非正交），由升管**镜面映射**定义。
> - 操作流程：**DXZA>0（基本条件）→ 触哼 → 入管 → 入管后区分窄管/扩管/平管**。
...
| 分类 | n | 占比 |
|:--|--:|--:|
| **窄管（0-4）** | 4,345,794 | **50.26%** |
| **扩管（≥4+推高顶）** | 2,377,406 | **27.49%** |
| **平管（≥4+非推高顶）** | 1,885,143 | **21.80%** |
| 管宽≤0 | 38,589 | 0.45% |

> **升管中 50.26% 是窄管（0-4）**——大部分升管管宽很小，无有效上涨形态，应回避。扩管/平管合计 49.29%。⚠️ **阈值 5→4（2026-09-21）：** 原窄管（0-5）占 62.55%，降到 4 后窄管占 50.26%，扩管/平管占比相应上升。
...
| 分类 | n | 次日P3% | 均高幅 |
|:--|--:|--:|--:|
| **扩管（≥4+推高顶）** | 2,377,406 | **35.16%** | 2.85% |
| **平管（≥4+非推高顶）** | 1,885,143 | **33.88%** | 2.67% |
| **窄管（0-4）** | 4,345,794 | **11.85%** | 1.34% |
| 管宽≤0 | 38,589 | 4.35% | 0.50% |

> **区分度：扩管 35.16% vs 窄管 11.85%，差 23.31pp**。扩管/平管是介入主战场，窄管应回避。

**⑤ 升管管宽分类 × 等型（次日P3%，甲乙己 + col42判定等型）：**

| 等型 | 窄管 | 平管 | 扩管 |
|:--|--:|--:|--:|
| 等1 | 16.12% | 39.19% | 36.84% |
| 等2 | 15.78% | 38.32% | 38.66% |
| **等3** | 16.17% | 39.09% | **46.68%** |
| 等5 | 13.07% | 34.66% | 31.95% |
| 等6 | 11.96% | 32.17% | 29.62% |
| 等7 | 7.40% | 28.43% | 24.75% |

> **扩管×等3 = 46.68% 是最强介入点**（n=345,740）。扩管/平管在等1-3 都强（36-47%），窄管在等7 最弱（7.40%，接近出管）。⚠️ **阈值 5→4（2026-09-21）：** 原表（等1 19.16%/等2 19.00%/等3 19.88% 等）用阈值5，现改为阈值4。
...
> **扩管主要在甲（15.30%）和乙（10.16%）**，己很少（2.03%）。升管护型分布：甲 48.67%、乙 37.65%、己 13.68%。

**⑦ 升管管宽分类 × 柱排（次日P3%）：**

| 柱排 | 窄管 | 平管 | 扩管 |
|:--|--:|--:|--:|
| 升连 | 11.90% | 35.98% | 36.28% |
| 升吞 | 13.39% | 39.33% | 36.97% |
| 升孕 | 10.45% | 32.69% | 30.58% |
| 人排 | 12.49% | 34.44% | — |
| 跌连 | 9.78% | 30.63% | — |
| 跌孕 | 11.22% | 29.48% | — |
| 跌吞 | 10.12% | 27.72% | — |

> **扩管只有升排**（扩管=触顶+升排，人排/跌排不满足"推高顶"定义）。平管×升吞 39.33% 最强；窄管×跌连 9.78% 最弱。

> **核心结论：** 升管（甲乙己+DXZA>0）中，**扩管/平管（管宽≥4）是介入主战场**（34-35%），**窄管（0-4）占 50.26% 应回避**（11.85%）。**扩管×等3 = 46.68% 是最强介入点**。管宽哼JA 恒≥0，窄管(<0) 在升管语境不存在。⚠️ **阈值 5→4（2026-09-21）：** 原窄管（0-5）占 62.55%，4-5 区间次日P3% 24.27% 接近基线非弱区，故阈值降到 4。
...
> **升管分类（用户，2026-09-19，2026-09-20 纠正定义，2026-09-21 阈值 5→4）：** 升管 = **DXAB 甲乙己 + DXZA>0**（只有甲乙己才能定义升管）。主要分为三类——**窄管**（管宽过小，不会形成有效上涨形态）、**扩管**（管宽≥4 + 柱排推高顶，升宽在扩大）、**平管**（管宽≥4 + 非推高顶，升宽持平）。**推高顶 = 触顶 + 升排**。窄管 = 0 < 管宽哼JA < 4。
...
> **✅ 扩管（35.16%）> 平管（33.88%）> 窄管（11.85%）**，差 23.31pp——**扩管 次日P3% 最高（管宽≥4 + 柱排推高顶），窄管 最低（管宽0-4，管宽过小，不会形成有效上涨形态，应回避）**。区分度远强于旧固定阈值分类（4.79pp）。⚠️ **阈值 5→4（2026-09-21）：** 原窄管（0-5）占 62.55%，4-5 区间次日P3% 24.27% 接近基线非弱区，故阈值降到 4——窄管占 50.26%，扩管/平管占比上升。
...
> **等3 是主介入区**（升管=甲乙己+DXZA>0，n=6,417,364，基线 22.99%）。管宽哼JA 是决定性维度：**扩管（管宽≥4+推高顶）最强**（35.33%），**窄管（0-4）极弱**（11.06%，应回避）。⚠️ **修正（2026-09-21）：** 原数据（等3 n=418,183，基线43.30%，扩管48.79%）误用宽哼JC（col23）当管宽哼JA，正确应为管宽哼JA（col21）+ 阈值4——等3 n=6,417,364，基线22.99%，扩管35.33%/平管32.67%/窄管11.06%。

**已验证数据（升管=甲乙己+DXZA>0，等3，n=6,417,364，基线 22.99%，推高顶=触顶+升排）：**

| 分类 | n | 占比 | 次日P3% |
|:--|--:|--:|--:|
| **窄管（0-4）** | 3,075,692 | 47.93% | **11.06%** |
| **平管（≥4+非推高顶）** | 1,354,764 | 21.11% | **32.67%** |
| **扩管（≥4+推高顶）** | 1,960,325 | 30.55% | **35.33%** |
| 管宽≤0 | 26,583 | 0.41% | 0.12% |

**等3 管宽分类 × 柱排（次日P3%）：**

| 柱排 | 窄管(0-4) | 平管(≥4+非推高顶) | 扩管(≥4+推高顶) |
|:--|:--|:--|:--|
| 升连 | 11.07% | 35.72% | **36.71%** |
| 升吞 | 12.80% | **38.37%** | 36.90% |
| 升孕 | 9.97% | 31.90% | 30.70% |
| 人排 | 11.49% | 33.38% | — |
| 跌连 | 9.78% | 30.63% | — |
| 跌孕 | 10.57% | 28.38% | — |
