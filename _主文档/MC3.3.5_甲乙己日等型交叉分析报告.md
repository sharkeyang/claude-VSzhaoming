...
- [六、操盘计划](#六操盘计划)
    ...
    - [6.7 第3层：工具汇总](#67-第3层工具汇总)
        ...
        - [6.8.0.1 升管管宽分类：原则 + 分布统计（2026-09-20 全量验证）](#6801-升管管宽分类原则-分布统计2026-09-20-全量验证)
    ...
    - [6.9 旧版介入时机](#69-旧版介入时机)
        ...
        - [6.9.2 升管判断（触哼入管 §5.1.4.1，2026-09-17 新准则）](#692-升管判断触哼入管-51412026-09-17-新准则)
    - [6.9.2.1 入管判定（只针对 DXAB 甲乙己，不考虑 DXZC）](#6921-入管判定只针对-dxab-甲乙己不考虑-dxzc)
    - [6.9.2.2 管中模式（扩管/平管/窄管）](#6922-管中模式扩管平管窄管)
    - [6.9.2.3 出管判定（连阴/阴阳阴 × 下一柱触顶）](#6923-出管判定连阴阴阳阴-下一柱触顶)
    - [6.9.2.4 出管边界优化（DJB 宽松边界，2026-09-18）](#6924-出管边界优化djb-宽松边界2026-09-18)
        - [6.9.3 垒升判断（柱体≥2%，§5.3）](#693-垒升判断柱体253)
    - [6.9.3.1 介入时机：只在垒升位置介入](#6931-介入时机只在垒升位置介入)
    - [6.9.3.2 持有策略：升连成群结队，可期待连续](#6932-持有策略升连成群结队可期待连续)
    - [6.9.3.3 启动信号：升连链首柱是升吞最强（§5.3.6）](#6933-启动信号升连链首柱是升吞最强536)
    - [6.9.3.4 跌连处理：DXZC>0 跌连不成群结队（§5.3.5.2）](#6934-跌连处理dxzc0-跌连不成群结队5352)
    - [6.9.3.5 持续持有 vs 波段（§5.3.1.1）](#6935-持续持有-vs-波段5311)
    - [6.9.3.6 镜面映射的全局指导（§5.3.5.2）](#6936-镜面映射的全局指导5352)
...
| 状态 | 判定 | 说明 |
|:--|:--|:--|
| **升管** | DXAB 甲乙己 + DXZA>0（日ZA>0） | 价格在DJA(EMA5)之上。**只有甲乙己才能定义升管**（用户确认，2026-09-20） |
...
**② 管中模式：扩管/平管/窄管（升管=甲乙己+DXZA>0，全量 7455 文件）**

| 模式 | 样本 | 次日P3% | 均高幅 |
|:--|--:|--:|--:|
| **扩管（管宽≥5+推高顶）** | 1,856,187 | **38.73%** | 3.10% |
| **平管（管宽≥5+非推高顶）** | 1,343,702 | **37.04%** | 2.86% |
| **窄管（管宽0-5）** | 5,408,454 | **14.29%** | 1.49% |

> **✅ 扩管（38.73%）> 平管（37.04%）> 窄管（14.29%）**——窄管管宽过小，不会形成有效上涨形态，应回避。**推高顶 = 触顶 + 升排**（2026-09-20 用户纠正定义）。⚠️ **修正（2026-09-20）：** 原数据（扩管32.05%/平管28.51%/窄管9.92%）误用宽哼JC（col23）当管宽哼JA，正确应为管宽哼JA（col22）——扩管38.73%/平管37.04%/窄管14.29%。
...
| 模式 | 次日P3% | 均高幅 | 操作 |
|:--|--:|--:|:--|
| **扩管（管宽≥5+推高顶）** | **38.73%** | 3.10% | 垒升典型，可介入 |
| **平管（管宽≥5+非推高顶）** | 37.04% | 2.86% | 可介入 |
| **窄管（管宽0-5）** | **14.29%** | 1.49% | **应回避**（管宽过小，无有效上涨形态）|

> **✅ 管中模式区分度大：** 扩管（38.73%）> 平管（37.04%）> 窄管（14.29%），差24.44pp。**窄管应回避**——这是旧准则（DTZA≥3）没有的维度。**推高顶 = 触顶 + 升排**（2026-09-20 用户纠正定义）。⚠️ **修正（2026-09-20）：** 原数据（扩管32.05%/平管28.51%/窄管9.92%）误用宽哼JC（col23）当管宽哼JA，正确应为管宽哼JA（col22）。
...
> - **融合到 §6.8 的内容（5项）：**
>   1. **升管判断**（§6.9.2）：入管判定（触哼入管）、出管判定（连阴/阴阳阴×触顶）、出管边界（DJB宽松）
>   2. **垒升判断**（§6.9.3）：只在垒升位置介入、持有策略、启动信号
...
> - **升管 = DXAB 甲乙己 + DXZA>0**（甲/乙/己，正交）。**只有甲乙己才能定义升管**。本框架 §6.8.1~6.8.3 的等1/2/3 均限定甲乙己。
...
>   - 全量验证（升管=甲乙己+DXZA>0）：窄管 14.29%（n=5,408,454）、扩管 38.73%（n=1,856,187）、平管 37.04%（n=1,343,702）——**扩管 vs 窄管差 24.44pp**，区分度远强于旧固定阈值（4.79pp）。⚠️ **修正：** 原数据（等3甲乙己 窄管9.40%/扩管31.72%/平管27.88%）误用宽哼JC（col23）当管宽哼JA，正确应为管宽哼JA（col22）。
> - **管宽哼JA 无负值**（最高价通常高于均线），升管/跌管分布几乎一致（0-20 占 99%+）。
> - **跌管也有窄管(0-5)**（丙11.93%、戊12.27%、丁12.36%，极弱应回避）——窄管定义（0-5）在升管/跌管通用（见 §6.8.6）。
...
> **等3 是主介入区**（升管=甲乙己+DXZA>0，n=418,183，基线 43.30%）。管宽哼JA 是决定性维度：**扩管（管宽≥5+推高顶）极强**（升连 48.79%），**窄管（0-5）极弱**（19.88%，应回避）。⚠️ **修正（2026-09-20）：** 原数据（等3甲乙己 n=6,417,364，基线22.99%，升连扩管33.11%）误用宽哼JC（col23）当管宽哼JA，正确应为管宽哼JA（col22）。等3（甲乙己+DXZA>0）中 **100% 是升连柱排**（ZA≥3 站上DJA 3柱+，基本都是升连），原表升吞/升孕/人排/跌连数据不成立。

**已验证数据（升管=甲乙己+DXZA>0，等3，n=418,183，基线 43.30%，推高顶=触顶+升排）：**

| 柱排 | 窄管(0-5) | 平管(≥5+非推高顶) | 扩管(≥5+推高顶) |
|:--|:--|:--|:--|
| 升连 | 19.88% (n=73,603) | 42.20% (n=25,770) | **48.79%** (n=318,810) |

> **结论：**
> ① **窄管(0-5) 极弱**（19.88%），远低于基线（43.30%）——**窄管应回避**（管宽过小，无有效上涨形态）；
> ② **扩管(≥5+推高顶) 最强**（48.79%）——**管宽≥5 + 触顶 + 升排 = 最强介入区**；
> ③ **平管(≥5+非推高顶) 次强**（42.20%），也高于基线；
> ④ **区分度巨大**：扩管(48.79%) vs 窄管(19.88%) 差 **28.91pp**——远强于旧固定阈值分类（4.79pp）。

> **🔍 专项验证（2026-09-20）：等3 管宽哼JA空 = 转弱信号。** 等3（甲乙己）中管宽哼JA为空的行次日P3%明显低于基线，**管宽消失 → 即将出管/转弱**，应回避。这补充了"不触顶阳柱→管宽缩小→下破DJA"链条的末端：管宽缩小到消失（管宽哼JA空）时胜率已明显下降。

**介入点条件（等3）：**
- **首选：** 扩管（管宽≥5 + 触顶 + 升排）→ 升连 48.79%，最强介入区。
- **次选：** 平管（管宽≥5 + 非推高顶）→ 42.20%，也高于基线。
- **升排推高顶：** 扩管→升排推高顶（触顶+升排），可期待连续冲高。
- **平管平移十字星：** 关注下柱冲高（十字星后方向待确认，专项验证：平管十字星次日P3%=21.48%，接近基线，非强信号）。

**止损条件（等3）：**
- **窄管（管宽哼JA 0-5）→ 19.88%，极弱，应回避**（管宽过小，无有效上涨形态）。
- **不触顶阳柱 → 管宽缩小 → 下破DJA** 是转坏链条，跌破 DJA 即止损。
- **管宽哼JA空（管宽消失）→ 转弱信号，应回避或减仓。**
...
| **等3** | 成熟升管 | **扩管（管宽≥5+触顶+升排）→升连48.79%**；平管（≥5+非推高顶）42.20% | **窄管(0-5)→19.88%极弱回避**；不触顶阳柱→管宽缩小→下破DJA止损；**管宽哼JA空→转弱回避** | 20-49% |
| **等5** | 出管/跌管 | 首下破反弹；**等6第二柱阳柱+次日再站DJA(37.37%)**；整体回避 | 跌破DJA无反弹离场 | 20-23% |

> **核心结论：** 介入时机坐标 = **等3 扩管（管宽≥5 + 触顶 + 升排）** 是最强介入点（升连 48.79%）；等2 靠触顶增强；等1 靠第2柱确认（维持升管29.39% vs 回落2.58%）；等5 整体回避（仅等6第二柱阳柱+次日再站DJA 37.37%可介入）。**管宽哼JA（col22）是决定性维度**——窄管(0-5)极弱应回避，扩管（≥5+推高顶）升排最强，管宽哼JA空（管宽消失）是转弱信号。
...
> **管宽哼JA 无负值**（最高价通常高于均线），所以跌管**没有"窄管<0"**。但**窄管定义（0-5）在跌管同样适用**——跌管窄管(0-5) 极弱（见下方②），应回避。

**② 跌管各护型管宽分类次日P3%（跌管=丙丁戊+DXZA<0，全量 7455 文件，推高顶=触顶+升排）：**
| 分类 | 丙 | 戊 | 丁 |
|:--|:--|:--|:--|
| **窄管(0-5)** | **11.93%** (n=744,229) | **12.27%** (n=2,322,398) | **12.36%** (n=2,713,999) |
| **平管(≥5+非推高顶)** | 30.84% (n=501,195) | 30.47% (n=851,841) | 30.05% (n=1,571,773) |
| **扩管(≥5+推高顶)** | 25.47% (n=7,041) | 23.31% (n=7,455) | 22.83% (n=19,642) |
| 管宽≤0 | 0.21% (n=5,212) | 3.11% (n=7,599) | 5.47% (n=14,399) |
| 基线 | 19.49% | 17.13% | 18.82% |

> **🔍 跌管管宽分类（2026-09-20 用户纠正版）：** 跌管（丙丁戊）**窄管(0-5) 也极弱**（11.9-12.4%，低于基线17-19%），应回避；**平管（管宽≥5）反而较强**（30%+，远高于基线）——跌管中管宽≥5 时胜率高于整体。⚠️ **修正（2026-09-20）：** 原数据（窄管丙5.22%/戊8.76%、平管25.14%/25.68%、扩管24.14%/26.96%，含大量"管宽空"）误用宽哼JC（col23）当管宽哼JA，正确应为管宽哼JA（col22，无空值）。旧分类"跌管扩管>60 也强（丙54.68%）"基于固定阈值（>60）且样本极小（n=1,046），**不再有"超跌反弹"强信号**。
...
**镜面映射操作逻辑（跌管 丙丁戊）：**
- **升管介入（扩管≥5+推高顶 升排）→ 跌管回避/退出**（跌管整体偏弱，基线 17-19%）。
- **跌管窄管(0-5) → 极弱（11.9-12.4%），回避。**
- **跌管平管（管宽≥5）→ 30%+，可轻仓试仓**（高于跌管基线，但低于升管扩管39%）。
- **跌管扩管（≥5+推高顶）→ 22.8-25.5%**，弱于平管，样本少（n=7,041~19,642），谨慎。
...
#### 6.9.2 升管判断（触哼入管 §5.1.4.1，2026-09-17 新准则）

> **升管入管用新准则（触哼入管）。** 完整验证见 §5.1.4.1，核心结论：

##### 6.9.2.1 入管判定（只针对 DXAB 甲乙己，不考虑 DXZC）
...
##### 6.9.2.2 管中模式（扩管/平管/窄管）

> **升管分类（用户，2026-09-19，2026-09-20 纠正定义）：** 升管 = **DXAB 甲乙己 + DXZA>0**（只有甲乙己才能定义升管）。主要分为三类——**窄管**（管宽过小，不会形成有效上涨形态）、**扩管**（管宽≥5 + 柱排推高顶，升宽在扩大）、**平管**（管宽≥5 + 非推高顶，升宽持平）。**推高顶 = 触顶 + 升排**。窄管 = 0 < 管宽哼JA < 5。
...
> **✅ 扩管（38.73%）> 平管（37.04%）> 窄管（14.29%）**，差 24.44pp——**扩管 次日P3% 最高（管宽≥5 + 柱排推高顶），窄管 最低（管宽0-5，管宽过小，不会形成有效上涨形态，应回避）**。区分度远强于旧固定阈值分类（4.79pp）。⚠️ **修正（2026-09-20）：** 原数据（扩管32.05%/平管28.51%/窄管9.92%，含管宽空121.9万）误用宽哼JC（col23）当管宽哼JA，正确应为管宽哼JA（col22，无空值）——扩管38.73%/平管37.04%/窄管14.29%。

##### 6.9.2.3 出管判定（连阴/阴阳阴 × 下一柱触顶）
...
##### 6.9.2.4 出管边界优化（DJB 宽松边界，2026-09-18）
