...
- [六、操盘计划](#六操盘计划)
    ...
    - [6.7 第3层：工具汇总](#67-第3层工具汇总)
        ...
        - [6.7.2 升管判断（触哼入管 §5.1.4.1，2026-09-17 新准则）](#672-升管判断触哼入管-51412026-09-17-新准则)
...
> **判断是否在管的目的（用户定稿 2026-09-22）：** 判断是否仍然后续可能会上升，即**寻找 DXZC>0 时的上升主体部分**。DXZA<0 虽然离开了（哼，DJA）升管，但**后续可能会反弹，不会立即下破 DJC**。因此：
> - **出管条件（破线）：** 采用 **DXZB<0**（变丙丁戊）作为出管条件——更宽松，多容忍反弹。
> - **主升变震荡标志（形态）：** **DXZA<0、DJA 之上出现跌排（连阴、阴阳阴）**都可以作为由主升变为震荡的标志，从而**逢高退出**。
...
##### 6.7.3.3 启动信号：升连链首柱是升吞最强（§5.3.6）

看到"升吞"后，下一日若转升连，是最强启动（74.07%），应重点介入。

##### 6.7.3.4 跌连处理：DXZC>0 跌连不成群结队（§5.3.5.2）

跌连大概率只有1天，不必恐慌卖出；但跌连当日介入优势不大（67.19%≈基线），最好等转升排确认（22.83%）再介入，不宜跌连当日重仓。

##### 6.7.3.5 持续持有 vs 波段（§5.3.1.1）

实体升连（涨幅≥3%）可一直持有（42.24%），十字星升连（涨幅<1%）只能波段（32.95%）；高波动标的垒升阈值≥3%，低波动标的≥2%。

##### 6.7.3.6 镜面映射的全局指导（§5.3.5.2）

DXZC>0（阳区）升连聚集、跌连可介入；DXZC<0（阴区）跌连聚集、升连不可追。
