...
- [附：数据口径说明](#附数据口径说明)
    ...
    - [9.1.1 日等型定义（BT连阳列，2026-09-12 补充）](#911-日等型定义bt连阳列2026-09-12-补充)
...
| **乙(ZA<0)** | →乙65.7%、→丙31.9%、→丁2.3% | ⚠️ **交叉路口**——转正30.6%/维持34.1%/转丙32.2%，**持仓不退出，等待走出方向**（见2.1.5.1.3） |
...
| ⚠️ **交叉路口** | **乙(ZA<0)** | 转正30.6%/维持34.1%/转丙32.2%，**持仓不退出，等待走出方向**（见2.1.5.1.3） |
...
> 5. **操作规则总纲：** DXCD=上/忐 → DXZC>0 → 只做己甲乙（DXZA>0）→ 优先阳柱接入 → 入管判定过滤（DTZA=1不考虑入管，DTZA=2看升连/跌孕，DTZA=3排除阴阳阴）→ 升管内跌排减仓 → 乙ZA<0/丙 + DTZA≤-2跌管退出 → **DXZC<0必须退出** → 戊ZA>0观察、其他放弃。
>
> **⚠️ 数据支持核对（2026-09-12）：** 操作规则总纲各环节均有数据支持——①入管判定过滤（§2.10.2，下破风险-6.72pp）；②升管内跌排减仓（§2.10.4，下破64.8% vs 升排37.7%，差27.05pp）；③乙ZA<0/丙+DTZA≤-2跌管退出（§2.10.3，深跌68%）；④DXZC<0必须退出（§2.10.8.1，平均亏损-8.86%→-5.78%）。**但"升管内跌排减仓"与策略4（§2.10.8.5）需统一**——策略4是"跌连后不立即退出，下破后等确认（重新站上继续持有，未重新站上才退出）"，而"升管内跌排减仓"是"跌排就减仓"。**统一为：升管内跌连不立即退出，等确认（下破后重新站上继续持有，未重新站上才退出）；非跌连的跌排（如跌吞）可减仓。**
...
**3.4.3.1 快速负交的护型×DXCD交叉样本数（乙拆分DXZA>0/<0，2026-09-12 修正）：**
...
> **⚠️ 计数算法（2026-09-12）：** "升连位置"按**行计数**——每个柱排含"升.尾连"的行算1次。**一个四连升（连续4天升连）算4次**（每天1次），不是"连数-1"（3次）。所以占比反映的是"升连出现的行数占比"，不是"升连段数占比"。
...
| 层 | 变量 | 作用 | 数据支撑 |
|:--|:--|:--|:--|
| **第1层 大区域** | DJC丘（DXZC>0）| 划出主升区域 | +7.20pp（20.1%→27.3%）|
| **第2层 护型过滤** | DXAB广义正交 | 排除下破DJC高发的非广义 | 非广义下破33.6% vs 广义7.5% |
| **第3层 真假正交** | DXCD上/忐 vs 中/下/忑 | 排除假正交 | 真正交28.8% vs 假正交19.7% |
...
| 持有|甲（对照）| — | 27.3% | 40.1% | 2.3% |
| 持有|乙(ZA>0)（对照）| — | **29.6%** | **42.6%** | 2.4% |
...
> **数据支撑（详细见 §4.1 核心框架四层漏斗 + §3.3.1.3 按DXCD拆分）：** DXZC>0（上/忐/忠）时甲/乙ZA>0 主导且持久；DXZC<0（中/下/忑）时丁/戊ZA<0 主导，甲/乙ZA>0 几乎消失（乙ZA>0 仅0.03-0.7%）。**限定 DXZC>0 是有意义的**（见 §3.3.1.3）。用户选上/忐（排除忠）是因为忠"维持但不冲高"，升连弱（20-22%）。
...
| **乙(ZA<0)** | 25.2% | 1.5天 | 转正30.6%/转丙32.2% | ⚠️ **交叉路口**：持仓不退出，等待转确定状态（乙ZA>0或丙）再决定；空仓等待选方向 |
...
>
> **己的升连比例（坡度特征）：** 己在 DXZC>0 时**升连比例最高（66.8%）**——己一旦出现几乎都是连续阳线（坡度陡），印证"己=冲高后急跌"的直觉。但己占区域内升连仅7.5%（"陡但少"），出现频率低，需快进快出（见 §4.10.5）。
...
> **口径说明：** 本节测算的是"定型操作组合"（DXCD=上/忐 × 甲/乙ZA>0/己 × DJA丘 + BSHA5）的收益；**§4.6 测算的是"DXZC>0+剔忠+广义正交"口径（DJC+剔忠）**。两者过滤不同，数据互补，可对照：（§4.6 主介入点 29.8%、年化30-60%）vs（本节定型组合 29.68-42.5%、年化18-32%）——**定型组合更"少而精"，年化更保守但每次胜率更高**。
...
> - **但己需快进快出**：升连高代表坡度陡，但持续短（1.2天）且冲高后急跌（§3.7 峰值回撤最高8.9%）——**升连高≠可长持，己是"陡但少、冲高后急跌"**。
...
>
> **操作含义：** DXZC>0 的跌连约2/3发生在DJA之下（其中下破DJA初期占55%），1/3在DJA之上。**DJA之上的跌连(33.8%)是"回调"信号（价格仍在DJA上方），DJA之下的跌连(66.2%)是"转坏"信号（已跌破DJA）。** 结合 §4.10.6：下破DJA时跌连是主要形态(51.6%)，需警惕转坏。
...
|:--|:--|:--|:--|:--|
| **首(0-10%)** | 60.8万 | 78.7% | **95.6%** | **64.1%** |
| **初(10-30%)** | 100.0万 | 97.2% | 89.8% | **64.9%** |
| **中(30-70%)** | 198.5万 | **98.3%** | 77.7% | 34.2% |
| **顶(70-90%)** | 100.0万 | 95.6% | 57.5% | 13.3% |
| **尾(90-100%)** | 36.8万 | 82.9% | **25.8%** | **4.8%** |
...
### 9.1.1 日等型定义（BT连阳列，2026-09-12 补充）

> **日等型 = 价格 vs DJA(EMA5) 的交叉天数（DTZA）分类**，是日类最基础的等型划分。等1/等2/等3为正ZA（价格在DJA上方），等5/等6/等7为负ZA（价格在DJA下方）。
