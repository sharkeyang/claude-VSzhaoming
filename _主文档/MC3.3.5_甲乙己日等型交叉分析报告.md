...
**① 入管判定：触哼入管 vs 旧准则（DTZA≥3）**

| 条件 | 样本 | 次日P3% | 5天P3% | 次日均 | 3天出管% |
|:--|--:|--:|--:|--:|--:|
| 基线_甲乙己 | 10,044,044 | 22.81 | 68.00 | 2.03 | 30.45 |
| 旧准则_DTZA>=3 | 6,414,871 | 23.00 | 70.20 | 2.04 | 23.87 |
| **新准则_触哼入管** | 2,913,747 | **25.14** | **70.61** | **2.25** | **21.74** |
...
| 指标 | 旧准则（DTZA≥3）| 新准则（触哼入管）| 差异 |
|:--|--:|--:|--:|
| 样本 | 6,414,871 | 2,913,747 | 新准则更聚焦 |
| 次日P3% | 23.00 | **25.14** | **+2.14pp** |
| 5天P3% | 70.20 | **70.61** | +0.41pp |
| 次日均高幅 | 2.04 | **2.25** | +0.21 |
| **3天出管率** | 23.87 | **21.74** | **-2.13pp** |

> **✅ 新准则全面优于旧准则：**
> - **次日P3更高**（25.14% vs 23.00%，+2.14pp）——入管质量更高。
> - **3天出管率更低**（21.74% vs 23.87%，-2.13pp）——**更少误判诱多**（这正是用户要解决的问题）。
> - **样本更聚焦**（291万 vs 641万）——新准则筛选更严格，只保留高质量入管。
...
> | 层 | n | 占基线 | P(≥3%) |
> |:--|:--|:--|:--|
> | 基线（全样本）| 11,090,585 | 100% | 23.6% |
> | **第1层 选土**（主战场 DXZE>0+上/忐）| 4,534,155 | **40.9%** | 28.6% |
> | **第2层 +护型甲乙己** | 3,324,398 | **30.0%** | 29.7% |
> | **第3层 +DJA丘** | 3,324,398 | 30.0% | 29.7% |
> | **+BSHA5 触发器** | 1,190,418 | **10.7%** | 42.5% |
>
> **过滤比例（逐层收敛）：**
...
| **第5层 微观** | 升管？垒升？诱多？ | ③ 升管 + ④ 垒升（**升连+涨幅≥2%**）+ ⑤ 剔诱多 | 升管25.14%；垒升98.07%；剔诱多24.9%；**升连≥2%=92.75%** |
...
> **框架定位（2026-09-20）：** 第3层在"选区域（第1层 DXCD）→ 选护型（第2层 甲乙己）"之后，解决**何时介入**。核心思路：在升管（DXAB甲乙己 + 升管入管）中，用 **（DXAB + 等型）** 作为坐标定位介入时机。等型坐标：**等1（起点）/等2（早期）/等3（成熟）/等5（出管）**。
> **管宽分类铁律（2026-09-20 用户纠正，2026-09-21 阈值 5→4 + 状态机口径）：** 扩管/平管/窄管**必须用管宽哼JA（col21）**，不是宽哼JC（col23）。**窄管 = 0 < 管宽哼JA < 4**；**管宽≥4 时看柱排是否推高顶（推高顶=触顶+升排）——推高顶=扩管，非推高顶=平管**。管宽哼JA 只在升管（日ZA>0）时有值。**升管 = DXAB 甲乙己（DXZB>0）且 形成甲乙己后曾触哼**（用户确认：只有甲乙己才能定义升管）。
...
> - **判定原则"严进宽出"：** 放弃原来"一旦上破 DXZA 就考虑是否入管"的旧思维，**只在触哼之后才考虑是否入管**，从而**缩小了考虑范围**（入管用严格标准，出管用宽松边界 DJB，见 §6.7.2.4）。
...
#### 6.7.2 升管判断（触哼入管 §5.1.4.1，2026-09-17 新准则）

> **升管入管用新准则（触哼入管）。** 完整验证见 §5.1.4.1，核心结论：

##### 6.7.2.1 入管判定（只针对 DXAB 甲乙己，不考虑 DXZC）
...
> **✅ 新准则（触哼入管）有效：** 次日P3 25.14%（比基线 +2.33pp），3天出管率 21.74%（比基线 -8.71pp）——更少误判诱多！
...
##### 6.7.2.4 出管边界优化（DJB 宽松边界，2026-09-18）
