...
>
> **关键增强信号（§6.8.3 + §6.8.11.1）：** DJA丘等1（31.4%）+ BSHA5（39.4%）+ **升连≥2%（92.75%）**——升连必须≥2%，否则无优势（首升连无限制68.84%≈基线，涨幅<2% 52.07%反而更差）。
>
> **阅读指引：** 第一~五章是各维度分析（护型/等型/正交动力学/三态理论），第六章是决策收敛（§6.8 定型操作 + §6.8.10 四类区域 + §6.8.11 操作理念），第七~八章是跨周期验证 + 数据规范。**核心决策在第六章，前面各章是数据支撑。**
...
- [六、操盘计划](#六操盘计划)
    ...
    - [6.8 四层漏斗定型操作规则（综合所有分析，2026-09-08 定稿，2026-09-12 统一为四层）](#68-四层漏斗定型操作规则综合所有分析2026-09-08-定稿2026-09-12-统一为四层)
        ...
        - [6.8.11 用户操作理念：沃土→甲乙己不破DJB→升管→垒升→剔诱多（2026-09-16 定稿）](#6811-用户操作理念沃土甲乙己不破djb升管垒升剔诱多2026-09-16-定稿)
            - [6.8.11.1 操作逻辑明确化：沃土 + DXZC>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）](#68111-操作逻辑明确化沃土-dxzc0甲乙己-优选升连2-不破djb不出2026-09-16-定稿)
...
> **🔗 本章结论 → 第六章入口：** DJC丘生命周期（甲启动→乙主升→丙预警）是第六章"介入时机"的底层依据——**DJC丘首/初介入选甲/己（§6.8.3 等1 31.4%），主升持有乙，顶部/尾部 DXZA>0 与 DXZB>0 双降警惕退出（§6.8.4 规则6）**。DJC丘（DXZC>0）是四类区域"主战场"的前提（§6.8.10）。
...
> - 己/甲 DXZC>0 时，次日冲高率约 21-23%，与基线（23.6%）接近——**护型本身不保证次日冲高**，需结合介入时机（DJA丘等1/2/3 + BSHA5，§6.8.3）和升连≥2%（§6.8.11.1）提升胜率。
...
> **🔗 本章结论 → 第六章入口：** 护型分析（甲/乙ZA>0 可做、乙ZA<0 交叉路口、丙持仓或退出、戊ZA>0 观察、丁/戊ZA<0 放弃）是第六章"护型选择"（§6.8.2）的直接数据支撑——**只做 甲/乙(ZA>0)/己，其余按角色处理**。DXZC>0 限定（上/忐/忠）对应四类区域"主战场"（§6.8.10）。
...
> **🔗 本章结论 → 第六章入口：** 柱与DJA三态理论 + 操作规则优化是第六章"操作理念"（§6.8.11）的直接数据支撑——**升管（DTZA≥3，§6.8.9）、垒升（柱体≥2%，§5.3）、剔诱多（跌连-非触顶，§6.8.7.3）、不破DJB不出（DXZB退出规则，§5.2.9）** 正是 §6.8.11 操作理念（沃土→甲乙己不破DJB→升管→垒升→剔诱多）的微观层。**本章是第六章操作理念的"微观细节库"。**
...
>
> **第1步 · 选区（§6.8.10 四类区域，DXZE×CD 二维）：** 判断当前处于哪个区域——**主战场（DXZE>0+CD上/忐/忠）/ 鸡肋（DXZE<0+CD上/忐/忠）/ 执念（(CD中)+(DXZE>0+CD下/忑)）/ 禁区（DXZE<0+CD下/忑）**。只在**主战场**重仓，鸡肋可持有不宜过早介入，执念观察，禁区坚决不碰。
>
...
>
> **关系：** 四类区域（§6.8.10）是**顶层选区**（决定"做不做、重仓还是轻仓"），四层漏斗（§6.8）是**底层选点**（决定"具体买哪个、何时买"）。**先选区（大势/土质），再选点（护型/时机/微观）**——不要在恶土（禁区/执念）里谈微观，这正是 §6.8.11 操作理念"抓大看小"的核心。
...
> [!IMPORTANT]
> **升连位置护型发现（2026-09-08，见2.4.2.10）：** DXZC>0 时升连位置，甲（47.7%）最多、乙（41.0%）次之。**但按 DXCD 分化：DXCD=上 时乙反而最多（48.4% > 甲38.3%），且甲/乙都强（33%+）可介入；DXCD=忐 时甲更强（27.6%）；DXCD=忠 时升连弱（20-22%），不可靠。** 升连介入优先选 DXCD=上（甲/乙都强）；DXCD=忠 谨慎（归主战场/鸡肋，但升连弱，需叠加升连≥2%才介入，见 §6.8.10）。
...
>
> **⚠️ 忠的归属（2026-09-16 统一）：** 忠 归**主战场**（DXZE>0）/ **鸡肋**（DXZE<0），遵循 VBA 仓日类定义（忠 ∈ {上,忐,忠}，见 §6.8.10）。但忠 = DXZC>0 且 DXZD<0（价格在 DJD=EMA60 之下），**上升受限、升连弱（20-22%）**——操作时需谨慎，**必须叠加"升连≥2%"（§5.3.9）才介入**，否则忠的升连不提升盈利性。上/忐 仍是首选（升连强、持久），忠 是次选（可做但需谨慎）。
...
> - **DXCD=忠（DXZC>0 但 DXZD<0）未覆盖 7.7%**——**已补全操作建议：谨慎介入**（归主战场/鸡肋，遵循 VBA 定义，见 §6.8.10）。忠虽是 DXZC>0，但 DXZD<0 意味着价格在 DJD(EMA60) 之下，上升受限、升连弱——**需叠加升连≥2%（§5.3.9）才介入**，否则忠的升连不提升盈利性。
>
...
##### 6.8.11.1 操作逻辑明确化：沃土 + DXZC>0甲乙己 + 优选升连（≥2%）+ 不破DJB不出（2026-09-16 定稿）
...
| + 沃土（DXZE>0+CD上/忐） | ~73% | +4.98pp | §6.8.10 主战场 |
