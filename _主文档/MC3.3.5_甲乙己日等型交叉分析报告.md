...
> **⚠️ 管宽列需确认（2026-09-16）：** 本节为 2026-09-07 数据，早于 09-08 列审计（发现管宽误用 col24=上身，应为 col22=宽哼JC）。**管宽行数值可能基于错误列，需重跑确认**。但核心结论（日ZA深度是转好率最强预测器，完全单调递增）用 col13=日ZA，一直正确，不受影响。管宽方向（管宽小转好率高）大概率仍成立，具体数值待重跑。
