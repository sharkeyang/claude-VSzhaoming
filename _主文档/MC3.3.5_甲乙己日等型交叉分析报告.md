...
- [二、护型分析](#二护型分析)
    ...
    - [2.9 等1：介入点（DXZA由负转正当天）](#29-等1介入点dxza由负转正当天)
        - [2.9.1 柱型细分（连阳 vs 单个阳柱）](#291-柱型细分连阳-vs-单个阳柱)
    - [2.10 等3：持有期（柱体不与DJA相交）](#210-等3持有期柱体不与dja相交)
        - [2.10.1 柱排分类](#2101-柱排分类)
        - [2.10.2 等3跌排 × DXAB范围（验证「0<DXAB≤4时跌排维持升势」）](#2102-等3跌排-dxab范围验证0dxab4时跌排维持升势)
        - [2.10.3 等3触顶 vs 非触顶](#2103-等3触顶-vs-非触顶)
    - [2.11 等2：关注柱排方向](#211-等2关注柱排方向)
        - [2.11.1 柱排分类](#2111-柱排分类)
        - [2.11.2 等2阳柱的划分（升孕 vs 升排 vs 跌排，2026-09-08 验证）](#2112-等2阳柱的划分升孕-vs-升排-vs-跌排2026-09-08-验证)
        - [2.11.3 等2内部的中符末位划分：末位C vs 末位DEF（2026-09-10 验证）](#2113-等2内部的中符末位划分末位c-vs-末位def2026-09-10-验证)
    - [2.12 等5：已结束甲（DXZA=-1）](#212-等5已结束甲dxza-1)
        - [2.12.1 柱排分类（乙(ZA<0)/丙有样本）](#2121-柱排分类乙za0丙有样本)
        - [2.12.2 柱型细分（连阳/单阳，2026-09-10 修正）](#2122-柱型细分连阳单阳2026-09-10-修正)
    - [2.13 等7：下跌通道（乙(ZA<0)/丙有样本）](#213-等7下跌通道乙za0丙有样本)
        - [2.13.1 管宽 × BSHA](#2131-管宽-bsha)
    - [2.14 等6：打破下跌通道（乙(ZA<0)/丙有样本）](#214-等6打破下跌通道乙za0丙有样本)
        - [2.14.1 管宽 × BSHA](#2141-管宽-bsha)
    - [2.15 综合结论：各分支涨跌判断](#215-综合结论各分支涨跌判断)
        - [2.15.1 可能后续上涨（介入/持有）](#2151-可能后续上涨介入持有)
        - [2.15.2 可能后续下跌（回避/退出）](#2152-可能后续下跌回避退出)
        - [2.15.3 关键修正（相对用户猜想）](#2153-关键修正相对用户猜想)
        - [2.15.4 各护型介入点评级（融合第二章，DXZC>0）](#2154-各护型介入点评级融合第二章dxzc0)
...
**2.3 乙(ZA<0) 的细化：不是一律退出，深跌+升排是超卖反弹机会（2026-09-07 验证）：**
...
### 2.9 等1：介入点（DXZA由负转正当天）

#### 2.9.1 柱型细分（连阳 vs 单个阳柱）
...
#### 2.10.2 等3跌排 × DXAB范围（验证「0<DXAB≤4时跌排维持升势」）
...
#### 2.11.2 等2阳柱的划分（升孕 vs 升排 vs 跌排，2026-09-08 验证）
...
#### 2.11.3 等2内部的中符末位划分：末位C vs 末位DEF（2026-09-10 验证）
...
#### 2.12.2 柱型细分（连阳/单阳，2026-09-10 修正）
